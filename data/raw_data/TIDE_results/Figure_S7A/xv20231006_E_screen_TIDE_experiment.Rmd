...
---
Info: In this markdown, I will analyse the data for gRNA delivery in PMA differentiated cells
...
```{r}
ref.file <- "xv20231006_TIDE_screens_controls.txt"
output.file <- "xv20231006_TIDE_screens_controls.pdf"
folder <- ""
seqstart = 100 #125 before
seqend = 500 #300 before
maxshift = 10
...
mut_list
write.csv(mut_list_b, file = "xv20231006_TIDE_screens_controls.csv")

```

#Discard based on 0.75 R2 filter
```{r}
#Discard samples
discard_samples <- tibble(transfection = c("T1", "T1","T1","T1","T1","T1","T2","T2","T2"),
       replicate = c("Rep1","Rep2","Rep3","Rep3","Rep3","Rep3","Rep2","Rep2","Rep2"),
       plate = c("Plate4","Plate5","Plate1","Plate2","Plate5","Plate6","Plate1","Plate3","Plate6"))
...
melt_editing_products <- mut_list_b %>% 
  ...
  separate(sample, into = c("transfection","replicate","plate")) %>%
  anti_join(discard_samples)

#Total editing frequency
editing_frequency <- melt_editing_products %>%
  filter(indel != 0) %>%
  dplyr::group_by(transfection,replicate,plate) %>%
  dplyr::summarise(editing_freq = sum(percentage)/100)

#Calculate parameters
repair_measurement <- melt_editing_products %>% 
  filter(indel %in% c(0, 1, -7)) %>%
  reshape2::dcast(transfection + replicate + plate ~ indel) %>%
  left_join(editing_frequency) %>%
  mutate(TIF = 100-`0`,
         balance = log2(`-7`/`1`))

# Mean values ranged from: 
mean_values_replicate <- repair_measurement %>%
  dplyr::group_by(transfection, replicate) %>%
  dplyr::summarise(mean_editing_freq = mean(editing_freq))
...
# Based on TIF
pdf(file = "/Users/x.vergara.ucin/surfdrive/Shared/surfdrive_shared_steensel/writing_spree/draft_DSB_CRISPR_screen/short_version/Figures/source_figures/Supplementary_figures/Figure_S6/xv20231006_screening_TIDE_controls.pdf", width = 3, height = 4)
ggplot(repair_measurement,aes(replicate, editing_freq)) + 
