...
---
Info: In this markdown, I will analyse the data for DharmaFect #4 lipofectamine test
...
```{r}
ref.file <- "xv20230615_E2283_pools_test.txt"
output.file <- "xv20230615_E2283_pools_test.pdf"
folder <- ""
seqstart = 100
seqend = 500
maxshift = 10
...
mut_list_b <- data.frame(matrix(unlist(mutation_list), ncol =length(mutation_list), byrow=FALSE))
colnames(mut_list_b) <- table$V4
rownames(mut_list_b) <- -10:10
mut_list
write.csv(mut_list_b, file = "xv20230531_E2274_pools_test_mutation.csv")
dev.off()
...
cell_lines <-  c("K562","RPE1","RPEprof","RPEdef", "U2OS")
melt_editing_products <- mut_list_b %>% 
  rownames_to_column() %>% 
  melt() %>%
  select(indel = rowname, sample = variable, percentage = value) %>%
  separate(sample, into = c("exp","cell","gRNA","drug","rep"))

#Average replicates
average_values <- melt_editing_products %>%
  dplyr::group_by(exp, cell, gRNA, drug, indel) %>%
  dplyr::summarise(m = mean(percentage), counts =  n())

#Calculate parameters
repair_measurement <- melt_editing_products %>% 
  filter(indel %in% c(0, 1, -7)) %>%
  reshape2::dcast(exp + cell + gRNA + drug + rep ~ indel, value.var = "percentage") %>%
  mutate(TIF = 100-`0`,
         balance = log2(`-7`/`1`))

#Drug effect
DMSO_effect <- repair_measurement %>%
  filter(cell %in% cell_lines & drug == "DMSO") %>% dplyr::group_by(cell) %>% dplyr::summarise(DMSO_bal = mean(balance, na.rm = T))

drug_effect <- repair_measurement %>%
  filter(cell %in% cell_lines) %>% left_join(DMSO_effect) %>% mutate(delta_bal = balance - DMSO_bal)

```

#Plot TIF of every sample
```{r}
# Based on TIF
ggplot(repair_measurement) + 
  ggbeeswarm::geom_quasirandom(aes(cell, TIF, color= drug)) + 
  theme_bw() + ylim(c(0,100))
#TIF
ggplot(repair_measurement %>% filter(cell %in% cell_lines)) +
  ggbeeswarm::geom_quasirandom(aes(fct_relevel(cell,cell_lines),TIF, color = drug, shape = exp)) +
  theme_bw() + ylab("Total indel frequency") + xlab("DSB-TRIP cell line") + ylim(c(0,100))

#Filter-out experiments
filter_out <- repair_measurement %>% filter(TIF < 20) %>% select(exp, cell, gRNA,rep,drug) %>% distinct()


#Lab meeting
#Balance
cell_lines <-  c("K562","RPE1","RPEprof","RPEdef", "U2OS")
ggplot(repair_measurement %>% anti_join(filter_out) %>% filter(cell %in% cell_lines)) +
  ggbeeswarm::geom_quasirandom(aes(fct_relevel(cell,cell_lines),balance, color = drug, shape = exp)) +
  theme_bw() + ylab("log2 MMEJ:NHEJ ratio") + xlab("DSB-TRIP cell line")
...
#Delta log2
ggplot(drug_effect %>% anti_join(filter_out)) + 
  ggbeeswarm::geom_quasirandom(aes(drug,delta_bal, color = cell, shape = exp)) +
  theme_bw() + ylab("∆log2 MMEJ:NHEJ") + xlab("DNA repair inhibitor") + 
