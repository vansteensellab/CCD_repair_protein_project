...
indels.merged.dt <- indels.dt %>% separate(exp, into = c("plate","bio.rep","t.rep","well")) %>% anti_join(off.plates) %>% select(-t.rep) %>% dplyr::group_by(plate,bio.rep,well,barcode) %>% dplyr::summarise_all(sum, na.rm = T) %>% mutate(exp = paste(plate,bio.rep,well, sep = "_")) %>% ungroup() %>% select(-plate,-bio.rep,-well)
