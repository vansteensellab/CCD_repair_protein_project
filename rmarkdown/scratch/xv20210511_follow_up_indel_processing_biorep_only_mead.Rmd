...
off.plates <- tibble(plate= c("P1","P2","P1"), t.rep = c("R3","R3","R2"), bio.rep = c("B1","B1","B2"), off = "Yes")

### Pool samples into two bio.reps
indels.merged.dt <- indels.dt %>% separate(exp, into = c("plate","bio.rep","t.rep","well")) %>% anti_join(off.plates) %>% select(-t.rep) %>% dplyr::group_by(plate,bio.rep,well,barcode) %>% dplyr::summarise_all(mean, na.rm = T) %>% mutate(exp = paste(plate,bio.rep,well, sep = "_")) %>% ungroup() %>% select(-plate,-bio.rep,-well)
...
saveRDS(screen.tib, file = "xv20210511_follow_up_indel_data_merged_mean.rds")
```

## Link to next file
```{r}
#Import and modify metadata file
follow.up.metadata.file <- read.table("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/xv20210429_meta_file_follow_up.txt", header = T) %>% mutate(targets = case_when(plate == "P1" & well == "C07" ~ "KMT2C_D_Combo", TRUE ~ targets))
...
screen.data.tib <- screen.tib %>% filter(del_7 + ins_1 > 30) %>% 
                                ...
                                separate(exp, into = c("plate","bio.rep","well")) %>%
                                mutate(freqCut > 0.2)
...
ch.z.scores.calc.mmej <- z.scores.calc.mmej.b %>% left_join(new.domains.c5.bis %>% dplyr::select(barcode,chromatin), by = c("barcode")) %>% dplyr::group_by(plate,bio.rep, well,chromatin,targets) %>% dplyr::mutate(ch.z.score = sum(z.score, na.rm = T)/n()) %>% distinct()
...
#Export all combos
combos.followup <- filter(cchz.scores.calc.mmej, grepl("Combo", targets)) %>% select(targets, mmej.SCscore,plate,chromatin) %>% distinct()
...
POLQ.gRNAs <- filter(cchz.scores.calc.mmej, grepl("POLQ", targets))
LIG4.gRNAs <- filter(cchz.scores.calc.mmej, grepl("LigIV", targets))
KDM6A.gRNAs <- filter(cchz.scores.calc.mmej, grepl("KDM6A", targets))
KMT2A.gRNAs <- filter(cchz.scores.calc.mmej, grepl("KMT2A", targets))
NCOA6.gRNAs <- filter(cchz.scores.calc.mmej, grepl("NCOA6", targets))
