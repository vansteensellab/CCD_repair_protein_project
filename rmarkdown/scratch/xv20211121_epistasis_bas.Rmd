...
ggplot(slope.protein.features %>% filter(term != "(Intercept)" & complete.cases(.))) + 
  geom_tile(aes(fct_relevel(gene,heatmap.gene.order.slope.diff),fct_relevel(feature, heatmap.chromatin.order.slope.diff), fill = slope.log2)) +
...
#Plot as a bargraph the effect of the protein in z-score
gene.effect <- both.screen.detail %>% filter(gene %in% top.100.genes.slope) %>% dplyr::group_by(gene,barcode) %>% dplyr::summarise(IPR.score = sum(mmej.z.score, na.rm = T)/sqrt(n()))

ggplot(gene.effect,aes(fct_relevel(gene,top.100.genes.slope),IPR.score)) + stat_summary(fun = mean, geom = "bar") + stat_summary(fun.data = mean_sd, geom = "errorbar")+ theme_minimal() + theme(axis.text.x = element_text(angle = 90))


```
# 6A: NHEJ (LIG4 and POLL)
```{r}
...
complexes <- bind_rows(NHEJ,FANC,RNF8.RNF168, MRN.complex)
...
#Bind 
ub.complex.data <- slope.proteins.all %>% right_join(ub.complex, by = "gene") %>% select(-term) %>% left_join(feature.class)

#plot
ggplot(ub.complex.data %>% filter(feat_class != "others"),aes(fct_relevel(complex, c("RNF8_RNF168","H2AK15ub","H2AK15ac","H1ub")),slope.log2, fill = complex)) +
  stat_mean(geom = "bar") +
  geom_quasirandom() +
