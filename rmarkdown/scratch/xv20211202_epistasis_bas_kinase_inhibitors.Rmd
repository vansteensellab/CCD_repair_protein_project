...
sel.inhibitor.data <- log2.inhibitor.data %>% select(replicate, drug, barcode, MMEJscore = MMEJ_score) %>% mutate(exp = "PIKK")
...
#PARPi data with vorinostat (pre-treatment)
PARPi.data.processed.vor.pretreat <- filter(PARPi.data.processed, Vorinostat == "pretreat") %>% select(replicate = Replicate, drug = Sample, barcode, MMEJscore)

#PARPi data with vorinostat (co-treatment)
PARPi.data.processed.vor.cotreat <- filter(PARPi.data.processed, Vorinostat == "cotreat") %>% select(replicate = Replicate, drug = Sample, barcode, MMEJscore)
...
# Plot differences
data.for.plotting.inhibitors <- log2.inhibitors %>% dplyr::group_by(drug,barcode,exp) %>% dplyr::summarise(m.log2MMEJ = mean(log2MMEJ, na.rm = T)) %>% left_join(clone5_chrom_tib)
...
mean.log2.distance.mmej.inhibitors <- log2.distance.mmej.inhibitors %>% dplyr::group_by(barcode,drug,exp) %>% dplyr::summarise(m.log2.dist = mean(log2.dist, na.rm = T)) %>% left_join(clone5_chrom_tib) %>% filter(drug != "GFP")
...
for (i in unique(mean.log2.distance.mmej.inhibitors$drug)) {
  for (j in colnames(mean.log2.distance.mmej.inhibitors)[7:31]) {
    ...
    slope.protein.features.inhibitors <- slope.protein.features.inhibitors %>% add_row(drug = i, feature = j, exp = k, slope.log2 = model.epistasis.log2 %>% pull(estimate), term = model.epistasis %>% pull(term))
    }
  }
}
...
ggplot(slope.protein.features.inhibitors %>% filter(term != "(Intercept)" & complete.cases(.) & drug != "DMSO")) + 
  ...
  coord_fixed(expand = F) + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), axis.title = element_blank()) + facet_wrap(~ exp)
...
# olaparib vs PARP1
PARP1 <- slope.protein.features.all %>% filter(term != "(Intercept)" & gene == "PARP1") %>% select(feature, PARP1 = slope.log2)
PARP1.perturbation <- left_join(inhibitors.dcast, PARP1)

#Plots
ggplot(PARP1.perturbation, aes(olaparib, PARP1)) + geom_point() + geom_smooth(method = "lm") + stat_cor(method = "spearman") + geom_text_repel(aes(label = feature)) + theme_bw()
