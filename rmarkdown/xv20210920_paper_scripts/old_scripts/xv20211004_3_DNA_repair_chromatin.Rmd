...
data.for.test.chromatin <- both.screen.detail %>% select(ID_gene, mmej.ch.score,t.rep, chromatin) %>% distinct() %>% filter(ID_gene %in% per_chrhits)
...
for (i in unique(data.for.test.chromatin$ID_gene)) {
  wFtest<-data.for.test.chromatin%>%
    filter(ID_gene==i)%>%
    welch.test(mmej.ch.score ~ chromatin, data = .)
...
tmp.welchtest <- WelchTest.chromatin%>%
