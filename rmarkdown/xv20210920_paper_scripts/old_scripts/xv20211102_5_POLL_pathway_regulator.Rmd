...
# figure 5A: Do all hits correlate with chromatin features
```{r}
# Correlation with chromatin data
clone5_chrom_tib <- readRDS("/DATA/projects/DSBrepair/data/R/rs20200519_clone5_newdoms_chromatin.RDS")
...
# Combination mmej
cor.values <- clone5_chrom_tib %>% select(-grep("dom", colnames(clone5_chrom_tib)),- IPR, -euchromatin, -transcription, -chrom_group, -chromatin) %>% left_join(mmej.freq.ctr %>% select(barcode,z.wt.mmej,z.gene.editing)) %>% column_to_rownames(var = "barcode")
...
# Correlation plot where x & y are not similar
correlation.all <- cor(all.prots,cor.values, method = "pearson")

heatmap <- pheatmap(correlation.all, cell_height = 8, cellwidth = 8)
heatmap.gene.order <- rownames(correlation.all[heatmap$tree_row[["order"]],])
heatmap.chromatin.order <- colnames(correlation.all[,heatmap$tree_col[["order"]]])


correlation.transformed <- correlation.all %>% as.data.frame()%>%  rownames_to_column(var = "gene") %>% reshape2::melt(value.name = "coefficient") %>% mutate(class = case_when(grepl("z", variable) ~ "CRISPR", T ~ "chromatin"))

#p.val.adj <- rcorr(all.prots,cor.values, type = "pearson")$P
#%>% as.data.frame()%>%  rownames_to_column(var = "gene") %>% reshape2::melt(value.name = "coefficient")
#tmp <- cor(chrom.barcodes[25:137,1:24], method = "pearson")

#Get tree structure to arrange tile
dst.cor.hits <- dist((abs(correlation.all) > 0.5)*1)
...
#Plot correlations
ggplot(correlation.transformed) + 
  geom_tile(aes(fct_relevel(variable, heatmap.chromatin.order),fct_relevel(gene,heatmap.gene.order), fill = coefficient)) +
  scale_fill_gradient2(limits = c(-1,1)) + 
  #geom_point(data = subset(correlation.transformed, abs(coefficient) > 0.6), aes(variable, gene)) + 
  coord_fixed(expand = F) + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))

tmp <- rcorr(as.matrix(all.prots),as.matrix(cor.values))
...
ggplot(correlation.transformed.stat) + 
  ...
  scale_fill_gradient2(limits = c(-1,1)) + 
  geom_point(data = subset(correlation.transformed.stat, abs(coefficient) > 0.5), aes(variable, gene)) + 
  coord_fixed(expand = F) + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
