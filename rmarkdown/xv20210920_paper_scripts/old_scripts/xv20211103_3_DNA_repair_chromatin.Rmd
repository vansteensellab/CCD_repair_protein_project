...
```{r}
# Get heatmap by chromatin type (5 types, but maybe it's better to make 4 "same as RS paper")
...
#DATA TO BE USED
data.for.test.chromatin <- both.screen.detail %>% select(ID_gene, mmej.z.score,t.rep,barcode, chromatin) %>% mutate(chrom.b = case_when(chromatin %in% c("euchromatin-transcription","transcription") ~ "euchromatin", T ~ chromatin)) %>% select(-chromatin) %>% distinct() %>% filter(ID_gene %in% per_chrhits)

# Variance test for all these 
variance.chromatin <- data.for.test.chromatin %>% dplyr::group_by(ID_gene, chrom.b) %>% dplyr::summarise(stdev = sd(mmej.z.score), mean.effect = mean(mmej.z.score))
ggplot(variance.chromatin) + geom_point(aes(mean.effect, stdev)) + facet_wrap(~ chrom.b)

ggplot(data.for.test.chromatin) + geom_quasirandom(aes(chrom.b,mmej.z.score))

#aov2
aov2.summary <- tidy(aov(mmej.z.score ~ chrom.b + ID_gene, data = data.for.test.chromatin))
...
tmp.aov.chrom <- aov.chromatin%>% filter(term == "chrom.b") %>% distinct(.) %>%
