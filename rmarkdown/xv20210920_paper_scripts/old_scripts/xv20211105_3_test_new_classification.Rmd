...
variance.hits <- score.per.4chrom %>% 
  ...
  group_by(ID_gene) %>% 
  dplyr::summarise(variance = var(screen.score)) %>% 
  arrange(desc(variance))
...
p.val.corr <- rcorr(as.matrix(prots.with.stats),as.matrix(cor.values), type = "spearman")$P[1:23,24:47]
...
#Plot
ggplot(FA.cc.data,aes(value,IPR.MMEJscore)) +
  geom_point() +
