...
ggplot(step1.epistasis %>% filter(gene %in% c("WT","KAT5"))) + geom_point(aes(fct_reorder(barcode,m.log2MMEJ), m.log2MMEJ, color = gene)) + scale_color_manual(values = c("#EF6817","#21201F")) + theme_bw() +  ylab("log2(MMEJ:NHEJ balance)") + theme(axis.text.x = element_blank(), legend.position = "top")
...
slope.prot.features.dcast.hits <- slope.protein.features.all %>% filter(gene %in% hit.genes & term != "(Intercept)") %>% reshape2::dcast(gene ~ feature, value.var = "slope.log2") %>% column_to_rownames(var = "gene")


#Heatmap
heatmap.slope.diff.hits <- pheatmap(slope.prot.features.dcast.hits, silent = T)
...
```{r}
slope.library <- slope.proteins.all %>% left_join(hits.both.screens %>% separate(ID_gene, into = "gene", remove = T) %>% select(gene,library,pathway) %>% distinct()) %>% filter(library %in% c("Chromatin","DNA_repair"))

# Plot per library(all)
ggplot(slope.library) + geom_quasirandom(aes(library,abs(slope.log2), color = library)) + theme_bw() + scale_color_manual(values = levels(as_factor(library.colors$color))) + theme(legend.position = "top")

#test
chromatin.genes.slope <- slope.library %>% filter(library == "Chromatin") %>% pull(slope.log2) %>% abs()
dna_repair.genes.slope <- slope.library %>% filter(library == "DNA_repair") %>% pull(slope.log2) %>% abs()
...
#test
NHEJ.genes.slope <- slope.library %>% filter(pathway == "NHEJ") %>% pull(slope.log2) %>% abs()
MMEJ.genes.slope <- slope.library %>% filter(pathway == "MMEJ") %>% pull(slope.log2) %>% abs()
...
ggplot(slope.proteins.all) + 
  geom_tile(aes(fct_relevel(gene,heatmap.gene.order.slope.diff.all),fct_relevel(feature, heatmap.chromatin.order.slope.diff.hits), fill = slope.log2)) +
...
```{r}
#
effect.size <- slope.protein.features.all %>% filter(complete.cases(.) & gene %in% hit.genes) %>% dcast(gene + feature ~ term, value.var = "slope.log2") %>% select(gene, feature, intercept= '(Intercept)', slope = "unlist(model.dt[j])")
