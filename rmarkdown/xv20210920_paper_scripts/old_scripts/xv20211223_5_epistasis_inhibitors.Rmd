...
```{r}
#Prepare data tables
selected.genes.drugs <- slope.protein.features.all %>% filter(term != "(Intercept)" & gene %in% c("PARP1","ATM","ATR","PRKDC")) %>% select("gene", "feature","slope.log2") %>% mutate(class = "protein", perturbation = paste(gene, "KO", sep = "_")) 

drug.comparison <- mean.slope.protein.featurese.inh %>% select("perturbation" = "drug", "feature","slope.log2") %>% mutate(class = "drug", gene = case_when(grepl("ATM", perturbation) ~ "ATM", grepl("ATR",perturbation) ~ "ATR", gene = grepl("DNAPK", perturbation) ~ "PRKDC", T ~ "PARP1"))
...
test.genes <- comparison.plot %>%
  dplyr::group_by(gene) %>%
  wilcox_test(slope.log2 ~ perturbation, alternative = "greater", paired = T) %>%
  filter(grepl("KO",group1) | grepl("KO",group2))

#supplementary effects

#Prepare data tables
selected.genes.effect <- per_19IPR_data %>% filter(gene %in% c("PARP1","ATM","ATR","PRKDC")) %>% mutate(class = "protein", perturbation = paste(gene, "KO", sep = "_")) 
...
# olaparib vs PARP1
genes <- slope.protein.features.all %>% filter(term != "(Intercept)" & gene %in% c("PARP1","ATM","ATR","PRKDC")) %>% reshape2::dcast(feature ~ gene, value.var = "slope.log2")
gene.drug.perturbation <- left_join(inhibitors.dcast , genes)

#Plots (main figure)
ggplot(PARP1.perturbation, aes(olaparib, PARP1)) + geom_point() + geom_smooth(method = "lm") + stat_cor() + theme_bw()

ggplot(PARP1.perturbation, aes(veliparib, PARP1)) + geom_point() + geom_smooth(method = "lm") + stat_cor() + geom_text_repel(aes(label = feature)) + theme_bw()

ggplot(PARP1.perturbation, aes(talazoparib, PARP1)) + geom_point() + geom_smooth(method = "lm") + stat_cor() + geom_text_repel(aes(label = feature)) + theme_bw()
...
```{r}
# POLL & DNA PKi
POLL <- slope.protein.features.all %>% filter(term != "(Intercept)" & gene == "POLL") %>% select(feature, slope.log2, perturbation = gene)
...
ggplot(combined.POLL, aes(POLL,DNAPKi)) + geom_point() + geom_smooth(method = "lm") + stat_cor(method = "spearman") + theme_bw()
