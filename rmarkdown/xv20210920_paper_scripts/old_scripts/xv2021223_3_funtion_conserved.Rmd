...
# Hit selection (same as in figure 1)
...
# Select hits 
hit.genes <- both.screen.gene.value %>% filter(pathway != "NA") %>% separate(ID_gene, into = "gene", remove = T)
...
```{r}
# Hits per IPR
per_19IPR_data <- both.screen.detail %>% dplyr::group_by(barcode, gene) %>% dplyr::summarise(IPR.score = mean(mmej.z.score), count = n())
...
```{r}
# Per 19 IPR heatmap
balance.IPR.hits <-per_19IPR_data %>% filter(gene %in% hit.genes$gene) %>% select(gene, barcode,IPR.score)

#dcast for heatmap
dcast.balance.IPR.hits <- balance.IPR.hits  %>% reshape2::dcast(gene ~ barcode, value.var = "IPR.score") %>% column_to_rownames(var = "gene")

#plot heatmap silently
heatmap.balance.IPR.hits <- pheatmap(dcast.balance.IPR.hits)
...
ggplot(balance.IPR.hits) + 
  geom_tile(aes(fct_relevel(gene,gene.o.balance.IPR.hits),fct_relevel(barcode, IPR.o.balance.IPR.hits), fill = IPR.score)) +
...
per_19IPR_data_hits <- filter(per_19IPR_data, abs(IPR.score) > 1.95 & count == 3) %>% pull(gene) %>% unique()

# Per 19 IPR heatmap
balance.IPR.hits.all <-per_19IPR_data %>% filter(gene %in% per_19IPR_data_hits) %>% select(gene, barcode,IPR.score)

#dcast for heatmap
dcast.balance.IPR.hits.all <- balance.IPR.hits.all  %>% reshape2::dcast(gene ~ barcode, value.var = "IPR.score") %>% column_to_rownames(var = "gene")

#plot heatmap silently
heatmap.balance.IPR.hits.all <- pheatmap(dcast.balance.IPR.hits.all)
...
ggplot(balance.IPR.hits.all) + 
  geom_tile(aes(fct_relevel(gene,gene.o.balance.IPR.hits.all),fct_relevel(barcode, IPR.o.balance.IPR.hits.all), fill = IPR.score)) +
  geom_point(data = subset(balance.IPR.hits.all, abs(IPR.score) > 1.95),aes(fct_relevel(gene,gene.o.balance.IPR.hits.all),fct_relevel(barcode, IPR.o.balance.IPR.hits.all), fill = IPR.score)) +
  scale_fill_gradient2( low = "#EB2030",mid = "grey90", high = "#2E358F", limits = c(-15,15), oob = squish)  + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), axis.title = element_blank(), legend.position = "top", axis.text.y = element_blank()) + coord_fixed(expand = F,ratio = 1)

# Is there any that has significant perturbation in both situation
genes.both.dir <- balance.IPR.hits %>% mutate(pos = IPR.score >1.95, neg = IPR.score < -1.95) %>% dplyr::group_by(gene) %>% dplyr::summarise(pos = sum(pos), neg = sum(neg))
