...
ggplot(step1.epistasis %>% filter(gene %in% c("WT","KAT5"))) + geom_point(aes(fct_reorder(barcode,m.log2ratio), m.log2ratio, color = gene)) + scale_color_manual(values = c("#EF6817","#21201F")) + theme_bw() +  ylab("log2(MMEJ:NHEJ ratio)") + theme(axis.text.x = element_blank(), legend.position = "top")
...
ggplot(slope.protein.features.all %>% filter(gene == "KAT5")) +
  geom_col(aes(fct_relevel(feature,heatmap.chromatin.order.slope.diff.hits),epistasis, fill = gene)) + geom_vline(aes(xintercept = 0)) + theme_bw() + coord_flip() + scale_fill_manual(values = "#EF6817")
