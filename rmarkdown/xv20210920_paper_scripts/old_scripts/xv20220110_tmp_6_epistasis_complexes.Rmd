...
complexes <- bind_rows(NHEJ,FANC,MRN.complex,RNF8.RNF168)

# filter all genes and re-run this
complex.gen.inter <- slope.protein.features.all %>% right_join(complexes) %>% bind_rows(mean.slope.protein.featurese.inh %>% filter(drug == "DNAPKi") %>% select(gene = drug, feature, epistasis) %>% mutate(complex = "NHEJ")) %>% mutate(gc = paste(gene,complex, sep = "_"))

ggplot(complex.gen.inter) +
  stat_summary(aes(fct_relevel(feature,heatmap.chromatin.order.slope.diff.hits),epistasis, group = complex, fill = complex), fun = mean, na.rm = T, geom = "bar") + geom_quasirandom(aes(fct_relevel(feature,heatmap.chromatin.order.slope.diff.hits),epistasis,group = gene), alpha = 0.5) +
...
# olaparib vs PARP1
genes <- slope.protein.features.all %>% filter(gene %in% c("PARP1","ATM","ATR","PRKDC","POLL")) %>% reshape2::dcast(feature ~ gene, value.var = "epistasis")
...
#Bind 
ub.complex.data <- ub.complex %>% left_join(slope.protein.features.all, by = "gene")  %>% left_join(feature.class)

#plot
ggplot(ub.complex.data %>% filter(feat_class != "others"),aes(fct_relevel(complex, unique(ubq.response.colors$label)),epistasis, fill = complex)) +
  stat_mean(geom = "bar") +
  geom_quasirandom() +
