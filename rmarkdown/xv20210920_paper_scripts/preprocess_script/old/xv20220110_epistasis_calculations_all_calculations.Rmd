...
```{r}
KAT5.values.ratio <- filter(log2.ratio.screen.detail.all, gene %in% c("PARP1","PARP3","FANCB")) %>% group_by(gene,library, barcode) %>% dplyr::summarise(m.log2ratio = mean(log2ratio, na.rm = T))
...
for (i in unique(KAT5.WT.example$gene)) {
  for (k in colnames(KAT5.WT.example)[5:37]){
    for (j in colnames(KAT5.WT.example)[40:64]) {
...
for(j in colnames(KAT5.WT.example)[57:81]){
  ...
  ks_test <- ks.test(WT.values, KAT5.values, alternative = "greater") %>% tidy()
  print(ks_test)
...
epistasis.ratio.sign.tmp <- slope.protein.features.all.ratio  %>% filter(complete.cases(.)) %>% left_join(mean.log2.fc) %>% dcast(gene + feature + mean.fc ~ term, value.var = "slope.log2") %>% select(gene,feature,intercept = '(Intercept)', slope = 'unlist(model.dt[j])', mean.fc) %>% left_join(slopes.null.summary) %>% left_join(max.values) %>% mutate(z.score = (slope - m)/s,sign.presence = slope*0.5 + intercept,cross.x = -intercept/slope, epistasis.sign = slope*sign(sign.presence), epistasis.final = case_when(cross.x < 1 ~ slope, cross.x > 1 ~ 0)) %>% mutate(p.val.z = 2*pnorm(abs(z.score), lower.tail = F), p.adj = p.adjust(p.val.z, method = "fdr"))
...
saveRDS(epistasis.ratio.sign, file = "~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/xv20220128_ratio_epistasis_all_genes.rds")
