...
tmp.dcast.ratio <- log2.ratio.screen.detail.all %>% reshape2::dcast(gene + library + barcode + sample + well + plate ~ t.rep, value.var = "log2ratio")
...
tmp.dcast <- log2.distance.ratio.all %>% reshape2::dcast(gene + library + barcode + sample + well + plate ~ t.rep, value.var = "log2.ratio")

# Test reproducibility across replicates
tmp.dcast.mean <- log2.distance.ratio.all %>% dplyr::group_by(t.rep, gene, library, sample, well, plate) %>% dplyr::summarise(m = mean(log2.ratio, na.rm = T)) %>% reshape2::dcast(gene + library + sample + well + plate ~ t.rep, value.var = "m")
...
wt.log2.distance <- log2.ratio.screen.detail.all %>% filter(sample == "WT") %>% dplyr::group_by(well,plate,t.rep,barcode) %>% dplyr::summarise(m.log2.ratio = mean(log2ratio, na.rm = T))  %>% left_join(wt.set.ratio) %>% mutate(m.log2.distance = m.log2.ratio - wt.log2ratio)


%>% na.omit() %>% left_join(clone5_z.score_chrom_tib, by = c("barcode" = "ID"))
