...
KO.distribution.per.replicate <- log2.ratio.screen.detail.all %>% filter(sample == "KO") %>% left_join(wt.set.ratio) %>% mutate(log2.ratio = log2ratio - wt.log2ratio) %>% left_join(clone5_z.score_chrom_tib, by = c("barcode" = "ID"))
...
#Compute z-score per IPR (at least 2 replicates)
per_19IPR_data <- both.screen.detail %>% 
  dplyr::group_by(gene,barcode,library) %>% 
  dplyr::summarise(IPR.z.score = sum(mmej.z.score, na.rm = T)/sqrt(n()), count = n()) %>% ungroup() %>% filter(count >= 2)
...
KO.slope.epistasis.filt.slopes <-  KO.slope.protein.features.all.ratio %>% left_join(IPR.significance, by = "gene") %>% na.omit() %>% mutate(slope.log2.b = case_when(signif.pert == "MMEJ" & slope.log2 < 0 ~ 0, signif.pert == "NHEJ" & epistasis.final > 0 ~ 0, T ~ epistasis.final))
...
#Dcast example
dcast.slope.epistasis <- KO.slope.epistasis.filt %>% reshape2::dcast(library + gene + feature ~ t.rep, value.var = "epistasis.final.b")

#Plot the chromatin specific interactions
ggplot(dcast.slope.epistasis %>% filter(library == "DNA_repair"), aes(R1,R4)) + geom_point() + xlab("R1 (chromatin specific effects))") + ylab("R4 (chromatin specific effects)") + geom_abline() + theme_bw() + stat_cor()
ggplot(dcast.slope.epistasis %>% filter(library == "DNA_repair"), aes(R4,R5)) + geom_point() + xlab("R4 (chromatin specific effects))") + ylab("R5 (chromatin specific effects)") + geom_abline() + theme_bw() + stat_cor()
ggplot(dcast.slope.epistasis %>% filter(library == "DNA_repair"),aes(R1,R5))  + geom_point() + xlab("R1 (chromatin specific effects))") + ylab("R5 (chromatin specific effects)") + geom_abline() + theme_bw() + stat_cor()

#Plot the chromatin specific interactions
ggplot(dcast.slope.epistasis %>% filter(library == "Chromatin"), aes(R1,R2)) + geom_point() + xlab("R1 (chromatin specific effects))") + ylab("R4 (chromatin specific effects)") + geom_abline() + theme_bw() + stat_cor()
ggplot(dcast.slope.epistasis %>% filter(library == "Chromatin"), aes(R2,R3)) + geom_point() + xlab("R4 (chromatin specific effects))") + ylab("R5 (chromatin specific effects)") + geom_abline() + theme_bw() + stat_cor()
ggplot(dcast.slope.epistasis %>% filter(library == "Chromatin"),aes(R1,R3))  + geom_point() + xlab("R1 (chromatin specific effects))") + ylab("R5 (chromatin specific effects)") + geom_abline() + theme_bw() + stat_cor()
...
dcast.slope.epistasis.mean <- dcast.slope.epistasis %>% left_join(mean.slope.epistasis)


# Correlation with mean
ggplot(dcast.slope.epistasis.mean %>% filter(library == "DNA_repair"), aes(R1,mean.epistasis)) + geom_point() + xlab("R1 (chromatin specific effects))") + ylab("R4 (chromatin specific effects)") + geom_abline() + theme_bw() + stat_cor()
ggplot(dcast.slope.epistasis.mean %>% filter(library == "DNA_repair"), aes(R4,mean.epistasis)) + geom_point() + xlab("R1 (chromatin specific effects))") + ylab("R4 (chromatin specific effects)") + geom_abline() + theme_bw() + stat_cor()
ggplot(dcast.slope.epistasis.mean %>% filter(library == "DNA_repair"), aes(R5,mean.epistasis)) + geom_point() + xlab("R1 (chromatin specific effects))") + ylab("R4 (chromatin specific effects)") + geom_abline() + theme_bw() + stat_cor()
