...
# What happens if I do this with WT (how does this look?)

```{r}
# Compute z-score with wt log2(ratio)
null.distribtution.log2Ratio <- filter(log2.ratio.screen.detail.all , sample == "WT") %>% dplyr::group_by(barcode, t.rep,library) %>% dplyr::summarise(wt.log2ratio = mean(log2ratio, na.rm = T),wt.log2ratio.inv = mean(log2ratioinv, na.rm = T), sd.wt.log2ratio = sd(log2ratio, na.rm = T), sd.wt.log2ratio.inv = sd(log2ratioinv, na.rm = T)) 

# Calculate z-scores based on this values
z.scores.ratio.IPR <- filter(log2.ratio.screen.detail.all, sample != "WT") %>% left_join(null.distribtution.log2Ratio) %>% na.omit() %>% mutate(z.score.log2ratio = (log2ratio - wt.log2ratio)/sd.wt.log2ratio, z.score.log2ratio.inv = (log2ratioinv - wt.log2ratio.inv)/sd.wt.log2ratio.inv)

# Filter z-scor
z.scores.ratio.IPR.filtered <- filter(z.scores.ratio.IPR, abs(z.score.log2ratio) > 1.65) %>% dplyr::group_by(t.rep, gene,library, sample) %>% dplyr::summarise(IPR_signif_count = n()) %>% filter(IPR_signif_count  > 1) %>% ungroup() %>% dplyr::group_by(gene,library,sample) %>% dplyr::mutate(rep_signif = n()) %>% filter(rep_signif > 1) %>% select(t.rep,gene,library,sample,rep_signif)
```
...
log2.distance.ratio.all <- log2.ratio.screen.detail.all %>% left_join(z.scores.ratio.IPR.filtered) %>% na.omit() %>% filter(sample == "KO") %>% left_join(wt.set.ratio) %>% mutate(log2.ratio = log2ratio - wt.log2ratio, log2.ratio.inv = log2ratioinv - wt.log2ratio.inv)

# Each replicate has a different value (compute differences by replicate)
log2.distance.ratio.WT <- log2.ratio.screen.detail.all %>% filter(sample == "WT") %>% left_join(wt.set.ratio) %>% mutate(log2.ratio = log2ratio - wt.log2ratio, log2.ratio.inv = log2ratioinv - wt.log2ratio.inv)

# Test reproducibility across replicates
ratio.perturbations.WT <- log2.distance.ratio.WT %>% reshape2::dcast(gene + library + barcode + sample + well + plate ~ t.rep, value.var = "log2.ratio")

# Test reproducibility across replicates
ratio.perturbations <- log2.distance.ratio.all %>% reshape2::dcast(gene + library + barcode + sample + well + plate ~ t.rep, value.var = "log2.ratio") %>% filter(gene %in% unique(IPR.significance$gene)) %>% bind_rows(ratio.perturbations.WT)
...
# Step 3: Do means over 19IPRs correlate in each replicate?
```{r}
...
# Plot reproducibility DNA_repair library
ggplot(ratio.perturbations.overall %>% filter(library == "DNA_repair" & gene %in% unique(IPR.significance$gene)), aes(R1,R4)) + geom_point() + xlab("R1 (log2 Fold change chrom.ind))") + ylab("R4 (log2 Fold change chrom. ind)") + geom_abline() + theme_bw() + stat_cor()
ggplot(ratio.perturbations.overall %>% filter(library == "DNA_repair" & gene %in% unique(IPR.significance$gene)), aes(R4,R5))  + geom_point() + xlab("R4 (log2 Fold change chrom.ind))") + ylab("R5 (log2 Fold change chrom. ind)") + geom_abline() + theme_bw() + stat_cor()
ggplot(ratio.perturbations.overall %>% filter(library == "DNA_repair" & gene %in% unique(IPR.significance$gene)),aes(R1,R5)) + geom_point()+ xlab("R1 (log2 Fold change chrom.ind))") + ylab("R5 (log2 Fold change chrom. ind)") + geom_abline() + theme_bw() + stat_cor()
...
# Calculate everything per replicate and check reproducibility of the data
```{r}
# WT samples per replicate
WT.distribution.per.replicate <- log2.distance.ratio.WT %>% left_join(clone5_z.score_chrom_tib, by = c("barcode" = "ID")) %>% mutate(coordinate = paste(plate, well, sep = "_"))

# Compute slopes for all experiments
WT.slope.protein.features.all.ratio <- tibble(library = NA, coordinate = NA,t.rep = NA, feature = NA, slope.log2 = NA, term = NA,p.value = NA)

for (k in unique(WT.distribution.per.replicate$library)) {
for (h in unique(WT.distribution.per.replicate$coordinate)) {
for (i in unique(WT.distribution.per.replicate$t.rep)) {
  for (j in colnames(WT.distribution.per.replicate)[18:42]) {
    model.dt <- WT.distribution.per.replicate %>% filter(t.rep == i & library == k & coordinate == h)
   if (nrow(model.dt) == 0) {
next
}
    model.epistasis.log2 <- lm(formula = log2.ratio ~ unlist(model.dt[j]), data = model.dt) %>% tidy()
   WT.slope.protein.features.all.ratio <- WT.slope.protein.features.all.ratio %>% add_row(library = k,coordinate = h, t.rep = i, feature = j, slope.log2 = model.epistasis.log2 %>% pull(estimate), term = model.epistasis.log2 %>% pull(term), p.value = model.epistasis.log2 %>% pull(p.value))
  }
}
}
}
...
WT.slope.epistasis <- WT.slope.protein.features.all.ratio %>% reshape2::dcast(library + coordinate + feature + t.rep ~ term, value.var = "slope.log2") %>% dplyr::select(library, coordinate ,feature, t.rep,intercept = '(Intercept)', slope = 'unlist(model.dt[j])') %>% mutate(cross.x = -intercept/slope, epistasis.final = case_when(cross.x < 1 ~ slope, cross.x > 1 ~ 0))

#Dcast example
dcast.slope.epistasis.WT <- WT.slope.epistasis %>% reshape2::dcast(library + coordinate + feature ~ t.rep, value.var = "epistasis.final")

#Plot the chromatin specific interactions
ggplot(dcast.slope.epistasis.WT %>% filter(library == "DNA_repair"), aes(R1,R4)) + geom_point() + xlab("R1 (chromatin specific effects))") + ylab("R4 (chromatin specific effects)") + geom_abline() + theme_bw()
ggplot(dcast.slope.epistasis.WT %>% filter(library == "DNA_repair"), aes(R4,R5)) + geom_point() + xlab("R4 (chromatin specific effects))") + ylab("R5 (chromatin specific effects)") + geom_abline() + theme_bw()
ggplot(dcast.slope.epistasis.WT %>% filter(library == "DNA_repair"),aes(R1,R5))  + geom_point() + xlab("R1 (chromatin specific effects))") + ylab("R5 (chromatin specific effects)") + geom_abline() + theme_bw()

#Plot the chromatin specific interactions
ggplot(dcast.slope.epistasis.WT %>% filter(library == "Chromatin"), aes(R1,R2)) + geom_point() + xlab("R1 (chromatin specific effects))") + ylab("R4 (chromatin specific effects)") + geom_abline() + theme_bw()
ggplot(dcast.slope.epistasis.WT %>% filter(library == "Chromatin"), aes(R2,R3)) + geom_point() + xlab("R4 (chromatin specific effects))") + ylab("R5 (chromatin specific effects)") + geom_abline() + theme_bw()
ggplot(dcast.slope.epistasis.WT %>% filter(library == "Chromatin"),aes(R1,R3))  + geom_point() + xlab("R1 (chromatin specific effects))") + ylab("R5 (chromatin specific effects)") + geom_abline() + theme_bw()
...
ggplot(dcast.slope.epistasis.mean.WT %>% filter(library == "DNA_repair"), aes(mean.epistasis.after,epistasis.final)) + geom_point() + xlab("CCD (mean after conversion)") + ylab("CCD (mean before conversion)") + geom_abline() + theme_bw() + stat_cor()
