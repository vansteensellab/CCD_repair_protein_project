...
for (i in unique(WT.distribution.per.replicate$t.rep)) {
  for (j in colnames(WT.distribution.per.replicate)[14:38]) {
...
KO.distribution.per.replicate <- log2.distance.ratio.KO %>% left_join(clone5_z.score_chrom_tib, by = c("barcode" = "ID"))
...
for (i in unique(KO.distribution.per.replicate$t.rep)) {
  for (j in colnames(KO.distribution.per.replicate)[14:38]) {
...
KO.slope.epistasis <- KO.slope.protein.features.all.ratio %>% reshape2::dcast(library + gene + feature + t.rep ~ term, value.var = "slope.log2") %>% dplyr::select(library, gene ,feature, t.rep,intercept = '(Intercept)', slope = 'unlist(model.dt[j])') %>% mutate(cross.x = -intercept/slope, epistasis.final = case_when(cross.x < 1 ~ slope, cross.x > 1 ~ 0))
```
...
genes.non.random.CCD <- filter(ks_test_genes_adjust, p.adj < 0.001)
...
# Filter slopes of WT and KO
KO.slopes.trep <-slope.epistasis %>% select(library, gene,feature,t.rep,epistasis.final)
WT.slopes <- WT.slope.epistasis %>% select(library, coordinate,feature,t.rep,epistasis.final)
...
```{r}
# Sum all the replicates before conversion
mean.slope.epistasis <- KO.slope.protein.features.all.ratio %>% left_join(z.scores.ratio.IPR.filtered) %>% na.omit() %>% reshape2::dcast(library + gene + feature + t.rep ~ term, value.var = "slope.log2") %>% dplyr::select(library, gene ,feature, t.rep,intercept = '(Intercept)', slope = 'unlist(model.dt[j])') %>% dplyr::group_by(library,gene,feature) %>% dplyr::summarise(mean.slope = mean(slope, na.rm = T), sd.slope = sd(slope, na.rm = T), mean.intercept = mean(intercept, na.rm = T)) %>% mutate(cross.x = -mean.intercept/mean.slope, epistasis.final = case_when(cross.x < 1 ~ mean.slope, cross.x > 1 ~ 0))

# Add mean value to dcast
dcast.slope.epistasis.mean <- dcast.slope.epistasis %>% left_join(mean.slope.epistasis) %>% left_join(mean.slope.epistasis.after) 
dcast.slope.epistasis.filtered <-dcast.slope.epistasis.mean %>% left_join(perc.non.targ.CCD) %>% filter(epistasis.final > perc95 | epistasis.final < perc05)

# Correlation with slopes for interaction that are out of the 95CI
ggplot(dcast.slope.epistasis.filtered %>% filter(library == "DNA_repair"), aes(R1,R4)) + geom_point() + xlab("R1 (Chromatin Context Dependency)") + ylab("R4 (Chromatin Context Dependency)") + geom_abline() + theme_bw()
ggplot(dcast.slope.epistasis.filtered %>% filter(library == "DNA_repair"), aes(R1,R5)) + geom_point() + xlab("R1 (Chromatin Context Dependency)") + ylab("R5 (Chromatin Context Dependency)") + geom_abline() + theme_bw()
ggplot(dcast.slope.epistasis.filtered %>% filter(library == "DNA_repair"), aes(R4,R5)) + geom_point() + xlab("R4 (Chromatin Context Dependency)") + ylab("R5 (Chromatin Context Dependency)") + geom_abline() + theme_bw()


# Correlation with slopes for interaction that are out of the 95CI
ggplot(dcast.slope.epistasis.filtered %>% filter(library == "Chromatin"), aes(R1,R2)) + geom_point() + xlab("R1 (Chromatin Context Dependency)") + ylab("R2 (Chromatin Context Dependency)") + geom_abline() + theme_bw()
ggplot(dcast.slope.epistasis.filtered %>% filter(library == "Chromatin"), aes(R1,R3)) + geom_point() + xlab("R1 (Chromatin Context Dependency)") + ylab("R3 (Chromatin Context Dependency)") + geom_abline() + theme_bw()
ggplot(dcast.slope.epistasis.filtered %>% filter(library == "Chromatin"), aes(R2,R3)) + geom_point() + xlab("R2 (Chromatin Context Dependency)") + ylab("R3 (Chromatin Context Dependency)") + geom_abline() + theme_bw()
...
```{r}
#
null.distribution <- log2.ratio.screen.detail.all %>% filter(sample == "WT") %>% left_join(wt.set.ratio) %>% mutate(log2.ratio = log2ratio - wt.log2ratio)
