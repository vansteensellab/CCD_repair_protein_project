...
```{r}
#DDR library
DDR.gene.library <- gene.KO.CCD.dt %>% filter(library == "DNA_repair")
chromatin.gene.library <- gene.KO.CCD.dt %>% filter(library == "Chromatin")
# Run a model for every gene in the screening
set.seed(1)
#fit PCR model
PCR_model_DDR <- pcr(mean.log2foldchange~CTCF+EZH2+H2AFZ+H3K27ac+H3K27me3+H3K36me3+H3K4me1+H3K4me2+H3K4me3+H3K79me2+H3K9me2+H3K9me3+H4K5acK8ac+HDAC1+HDAC2+HDAC3+POL2+POL2AS2+SMC3+LMNB1+Dam+DNAse+m5C+late_replicating+TTseq, data=DDR.gene.library , validation="CV")

PCR_model_chromatin <- pcr(mean.log2foldchange~CTCF+EZH2+H2AFZ+H3K27ac+H3K27me3+H3K36me3+H3K4me1+H3K4me2+H3K4me3+H3K79me2+H3K9me2+H3K9me3+H4K5acK8ac+HDAC1+HDAC2+HDAC3+POL2+POL2AS2+SMC3+LMNB1+Dam+DNAse+m5C+late_replicating+TTseq, data=chromatin.gene.library, validation="CV")

# Compute slopes for all experiments
p.value.PCR.model.DDR <- tibble(library = NA, gene = NA, num_comp = NA, slope.log2 = NA, term = NA,p.value = NA)
...
next
   ...
   p.value.PCR.model.DDR <- p.value.PCR.model.DDR %>% add_row(library = k,gene = h, num_comp = i, slope.log2 = pred_vs_estim %>% pull(estimate), term = pred_vs_estim %>% pull(term), p.value = pred_vs_estim %>% pull(p.value))
  }
}
}

# Compute slopes for chromatin proteins
# Compute slopes for all experiments
p.value.PCR.model.chromatin <- tibble(library = NA, gene = NA, num_comp = NA, slope.log2 = NA, term = NA,p.value = NA)

for (k in unique(chromatin.gene.library$library)){
for (h in unique(chromatin.gene.library$gene)){
  for (i in 1:20){
    gene.dt <- chromatin.gene.library %>% filter(library == k & gene == h)
    pcr_pred <- predict(PCR_model_DDR, gene.dt, ncomp = i)
    combined.dt <- tibble(measured = gene.dt$mean.log2foldchange, predicted = as.numeric(pcr_pred))
  if (nrow(gene.dt) == 0) {
next
   ...
   pred_vs_estim <- lm(formula = measured ~ predicted, data = combined.dt) %>% tidy()
   p.value.PCR.model.chromatin <- p.value.PCR.model.chromatin %>% add_row(library = k,gene = h, num_comp = i, slope.log2 = pred_vs_estim %>% pull(estimate), term = pred_vs_estim %>% pull(term), p.value = pred_vs_estim %>% pull(p.value))
  }
}
}

#Adjust p-value for this
DDR.p.value.adj <- p.value.PCR.model.DDR %>% filter(term == "predicted" & num_comp == 12) %>% mutate(fdr = p.adjust(p.value, method = "fdr")) %>% filter(fdr < 0.1)

#Adjust p-value for this
chromatin.p.value.adj <- p.value.PCR.model.chromatin %>% filter(term == "predicted" & num_comp == 20) %>% mutate(fdr = p.adjust(p.value, method = "fdr")) %>% filter(fdr < 0.1)
