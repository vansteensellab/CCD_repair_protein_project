...
#Take log2 MMEJ:NHEJ fold change and try to run a linear model on it
```{r}
#DDR library
DDR.gene.library.m <- gene.KO.CCD.dt %>% filter(library == "DNA_repair")
non.targ.library.m <- non.targetting.CCD %>% filter(library == "DNA_repair")

# Run a model for every gene in the screening
set.seed(1)
#fit PCR model (Question? Should we do that with the whole dataset)
PCR_model_DDR_m <- pcr(mean.log2foldchange~CTCF+EZH2+H2AFZ+H3K27ac+H3K27me3+H3K36me3+H3K4me1+H3K4me2+H3K4me3+H3K79me2+H3K9me2+H3K9me3+H4K5acK8ac+HDAC1+HDAC2+HDAC3+POL2+POL2AS2+SMC3+LMNB1+Dam+DNAse+m5C+late_replicating+TTseq, data=DDR.gene.library.m, validation="CV")

#Using the non.targ.library
PCR_model_non_t_m <- pcr(log2.foldchange~H3K27me3+CTCF+EZH2+H2AFZ+H3K27ac+H3K36me3+H3K4me1+H3K4me2+H3K4me3+H3K79me2+H3K9me2+H3K9me3+H4K5acK8ac+HDAC1+HDAC2+HDAC3+POL2+POL2AS2+SMC3+LMNB1+Dam+DNAse+m5C+late_replicating+TTseq, data=non.targ.library.m , validation="CV")


# Compute slopes for all experiments
p.value.PCR.model.DDR.m <- tibble(library = NA, gene = NA, num_comp = NA, slope.log2 = NA, term = NA,p.value = NA, rown = NA)

for (k in unique(DDR.gene.library.m$library)){
for (h in unique(DDR.gene.library.m$gene)){
  for (i in 1:25){
    gene.dt <- DDR.gene.library.m %>% filter(library == k & gene == h)
    pcr_pred <- predict(PCR_model_DDR_m, gene.dt, ncomp = i)
    combined.dt <- tibble(measured = gene.dt$mean.log2foldchange, predicted = as.numeric(pcr_pred))
  if (nrow(gene.dt) == 0) {
next
}
   pred_vs_estim <- lm(formula = measured ~ predicted, data = combined.dt) %>% tidy()
   p.value.PCR.model.DDR.m <- p.value.PCR.model.DDR.m %>% add_row(library = k,gene = h, num_comp = i, slope.log2 = pred_vs_estim %>% pull(estimate), term = pred_vs_estim %>% pull(term), p.value = pred_vs_estim %>% pull(p.value), rown = nrow(combined.dt))
  }
}
}

#Adjust p-value for this
p.value.PCR.model.selected.m <- p.value.PCR.model.DDR.m %>% filter(term == "predicted") %>% dplyr::group_by(gene) %>% slice(which.min(p.value)) %>% mutate(sign = p.value < 0.05)

```

#Plot the comparison between mean and all replicates
```{r}
# With selection
mean.vs.reps.corr.selected <- p.value.PCR.model.selected %>% dplyr::select(library,gene,p.value, sign, p.value) %>% left_join(p.value.PCR.model.selected.m %>% dplyr::select(library,gene, p.value, mean.sign = sign, mean.p = p.value))

# They almost correlate: This is not affected by it
ggplot(mean.vs.reps.corr.selected,aes(-log10(p.value),-log10(mean.p))) + geom_point() + geom_abline() + stat_cor(method = "spearman") + theme_bw()
# With selection
mean.vs.reps.corr <- p.value.PCR.model.DDR %>% filter(term == "predicted") %>% dplyr::select(library,gene,num_comp, p.value) %>% left_join(p.value.PCR.model.DDR.m %>% filter(term == "predicted") %>% dplyr::select(library,gene,num_comp, mean.p = p.value))

# They almost correlate: This is not affected by it
ggplot(mean.vs.reps.corr,aes(-log10(p.value),-log10(mean.p))) + geom_point() + geom_abline() + facet_wrap(~ num_comp) + stat_cor(method = "spearman")
```


#Run the same for means Take log2 MMEJ:NHEJ fold change and try to run a linear model on it
```{r}
#DDR library
DDR.gene.library <- log2.distance.ratio.KO %>% filter(library == "DNA_repair") %>% left_join(clone5_z.score_chrom_tib, by = c("barcode" = "ID"))
non.targ.library <- log2.distance.ratio.WT %>% filter(library == "DNA_repair") %>% left_join(clone5_z.score_chrom_tib, by = c("barcode" = "ID"))

# Run a model for every gene in the screening
set.seed(1)
#fit PCR model (Question? Should we do that with the whole dataset)
PCR_model_DDR <- pcr(log2.foldchange~CTCF+EZH2+H2AFZ+H3K27ac+H3K27me3+H3K36me3+H3K4me1+H3K4me2+H3K4me3+H3K79me2+H3K9me2+H3K9me3+H4K5acK8ac+HDAC1+HDAC2+HDAC3+POL2+POL2AS2+SMC3+LMNB1+Dam+DNAse+m5C+late_replicating+TTseq, data=DDR.gene.library, validation="CV")

#Using the non.targ.library
PCR_model_non_t <- pcr(log2.foldchange~H3K27me3+CTCF+EZH2+H2AFZ+H3K27ac+H3K36me3+H3K4me1+H3K4me2+H3K4me3+H3K79me2+H3K9me2+H3K9me3+H4K5acK8ac+HDAC1+HDAC2+HDAC3+POL2+POL2AS2+SMC3+LMNB1+Dam+DNAse+m5C+late_replicating+TTseq, data=non.targ.library , validation="CV")


# Compute slopes for all experiments
p.value.PCR.model.DDR <- tibble(library = NA, gene = NA, num_comp = NA, slope.log2 = NA, term = NA,p.value = NA, rown = NA)

for (k in unique(DDR.gene.library$library)){
for (h in unique(DDR.gene.library$gene)){
  for (i in 1:25){
    gene.dt <- DDR.gene.library %>% filter(library == k & gene == h)
    pcr_pred <- predict(PCR_model_DDR, gene.dt, ncomp = i)
    combined.dt <- tibble(measured = gene.dt$log2.foldchange, predicted = as.numeric(pcr_pred))
  if (nrow(gene.dt) == 0) {
next
}
   pred_vs_estim <- lm(formula = measured ~ predicted, data = combined.dt) %>% tidy()
   p.value.PCR.model.DDR <- p.value.PCR.model.DDR %>% add_row(library = k,gene = h, num_comp = i, slope.log2 = pred_vs_estim %>% pull(estimate), term = pred_vs_estim %>% pull(term), p.value = pred_vs_estim %>% pull(p.value), rown = nrow(combined.dt))
  }
}
}

#Adjust p-value for this
p.value.PCR.model.selected <- p.value.PCR.model.DDR %>% filter(term == "predicted") %>% dplyr::group_by(gene) %>% slice(which.min(p.value)) %>% mutate(sign = p.value < 0.05)

```

# Re-try the PCR and run a linear model per gene
```{r}
#DDR library
DDR.gene.library.test.m <- gene.KO.CCD.dt %>% filter(library == "DNA_repair")


tmp.value <- tibble(library = NA, gene = NA, num_comp = NA, slope.log2 = NA, term = NA,p.value = NA)

for (i in unique(DDR.gene.library.test.m$gene)){
  for (j in 1:10){
gene.library.dt <- filter(DDR.gene.library.test.m, gene == i)
...
pred_vs_estim <- lm(formula = measured ~ predicted, data = combined.dt) %>% tidy()
tmp.value <- tmp.value %>% add_row(gene = i, num_comp = j, slope.log2 = pred_vs_estim %>% pull(estimate), term = pred_vs_estim %>% pull(term), p.value = pred_vs_estim %>% pull(p.value))
  }
}

#p-value per gene
ggplot(tmp.value %>% filter(term == "predicted")) + geom_density(aes(p.value)) + facet_wrap(~num_comp, scales = "free") + theme_bw()

#Adjust p-value for this
p.value.PCR.model.per.gene.correct <- tmp.value %>% filter(term == "predicted" & num_comp == 4) %>% mutate(fdr = p.adjust(p.value, method = "fdr"))

p.value.PCR.model.per.gene.correct.pc2 <- tmp.value %>% filter(term == "predicted" & num_comp == 2) %>% mutate(fdr = p.adjust(p.value, method = "fdr"))

# Compare single gene model vs. all data model
ggplot(tmp.value %>% filter(term == "predicted")) + geom_density(aes(p.value)) + facet_wrap(~num_comp, scales = "free") + theme_bw()

# With selection
mean.vs.reps.corr.selected <-
...
single.model.vs.mean <- mean.vs.reps.corr %>% select(-library) %>% right_join(tmp.value, by = c("gene","num_comp")) %>% filter(term == "predicted")

# They almost correlate: This is not affected by it
ggplot(single.model.vs.mean,aes(-log10(mean.p),-log10(p.value.y))) + geom_point() + geom_abline() + facet_wrap(~ gene, scales = "free") + stat_cor(method = "spearman")
...
DDR.CCD.values.mean <- gene.KO.CCD.mean %>% filter(library == "DNA_repair") %>% left_join(perc.non.targ.CCD) %>% mutate(CI_99_sign = case_when((CCD_value < perc01 | CCD_value > perc99) ~ T, T ~ F)) %>% select(-perc99, -perc01)

chromatin.CCD.values.mean <- gene.KO.CCD.mean %>% filter(library == "Chromatin") %>% left_join(perc.non.targ.CCD) %>% mutate(CI_99_sign = case_when((CCD_value < perc01 | CCD_value > perc99) ~ T, T ~ F)) %>% select(-perc99, -perc01)
```
...
#Add a column assessing NHEJ, MMEJ or mixed
DDR.genes.pathway.CCD <- DDR.CCD.values.pr %>% filter(ks_test_sign == T & CI_99_sign == T) %>% dplyr::group_by(gene) %>% dplyr::summarise(NHEJ = sum(m_CCD_value > 0), MMEJ = sum(m_CCD_value < 0)) %>% mutate(pathway = case_when(NHEJ > 0 & MMEJ == 0 ~ "NHEJ", MMEJ > 0 & NHEJ == 0 ~ "MMEJ", NHEJ > 0 & MMEJ > 0 ~ "both"))
...
chromatin.genes.pathway.CCD <- chromatin.CCD.values.pr %>% filter(ks_test_sign == T & CI_99_sign == T) %>% dplyr::group_by(gene) %>% dplyr::summarise(NHEJ = sum(m_CCD_value > 0), MMEJ = sum(m_CCD_value < 0)) %>% mutate(pathway = case_when(NHEJ > 0 & MMEJ == 0 ~ "NHEJ", MMEJ > 0 & NHEJ == 0 ~ "MMEJ", NHEJ > 0 & MMEJ > 0 ~ "both"))

#Bind data to final
DDR.CCD.values.pr.path <- DDR.CCD.values.pr %>% left_join(DDR.genes.pathway.CCD)
chromatin.CCD.values.pr.path <- chromatin.CCD.values.pr %>% left_join(chromatin.genes.pathway.CCD)
