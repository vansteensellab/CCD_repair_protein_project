...
null.distribtution.log2Ratio <- filter(log2.ratio.screen.detail.all , sample == "WT") %>% dplyr::group_by(barcode,library) %>% dplyr::summarise(wt.log2ratio = mean(log2ratio, na.rm = T), sd.wt.log2ratio = sd(log2ratio, na.rm = T))
...
tmp.value.scrambled <- tibble(library = NA, gene = NA, num_comp = NA, slope.log2 = NA, term = NA,p.value = NA, seed = NA)

for (i in unique(DDR.gene.library.test.m$gene)){
  for (j in 1:100){
    for (k in 1:10) {
set.seed(j)
gene.library.dt <- filter(DDR.gene.library.test.m, gene == i) %>% mutate(scramble.log2fc = sample(mean.log2foldchange))
# Run a model per gene
PCR_model_DDR_test <- pcr(scramble.log2fc~CTCF+EZH2+H2AFZ+H3K27ac+H3K27me3+H3K36me3+H3K4me1+H3K4me2+H3K4me3+H3K79me2+H3K9me2+H3K9me3+H4K5acK8ac+HDAC1+HDAC2+HDAC3+POL2+POL2AS2+SMC3+LMNB1+Dam+DNAse+m5C+late_replicating+TTseq, data=gene.library.dt , validation="CV")

pcr_pred <- predict(PCR_model_DDR_test, gene.library.dt, ncomp = k)
combined.dt <- tibble(measured = gene.library.dt$scramble.log2fc, predicted = as.numeric(pcr_pred))
pred_vs_estim <- lm(formula = measured ~ predicted, data = combined.dt) %>% tidy()
tmp.value.scrambled <- tmp.value.scrambled %>% add_row(gene = i, num_comp = k, slope.log2 = pred_vs_estim %>% pull(estimate), term = pred_vs_estim %>% pull(term), p.value = pred_vs_estim %>% pull(p.value), seed = j)
  }
  }
}

# Calculates mean over scrambled
m.value.scrambled <- tmp.value.scrambled %>% filter(term == "predicted") %>% dplyr::group_by(num_comp, gene) %>% dplyr::summarise(m_pvalue = mean(p.value))

# scrambled data and nt data
ggplot(nt.model.value %>% filter(term == "predicted")) + geom_density(aes(p.value), color = "grey") + geom_density(data = tmp.value.scrambled %>% filter(term == "predicted"), aes(p.value, group = seed), color = "black") +  facet_wrap(~num_comp, scales = "free_y") + theme_bw() + geom_density(data = tmp.value %>% filter(term == "predicted"), aes(p.value), color = "red") + geom_vline(xintercept = 0.05, linetype = 2)


#We selected three components as the most meaningful data
significant.genes_p.value <- tmp.value %>% filter(term == "predicted") %>% filter(num_comp == 3) %>% mutate(p.adj = p.adjust(p.value, method = "BH")) %>% select(gene,p.value,p.adj)

#false discovery rates in all these samples
nt_real_data_p <- nt.model.value %>% filter(term == "predicted") %>% dplyr::select(num_comp, p.value, gene = coordinate) %>% mutate(sample = "nt") %>% bind_rows(tmp.value %>% dplyr::select(num_comp, p.value, gene) %>% mutate(sample = "ko"))

#adjust p-value
nt_real_data_p.adjust <- nt.model.value %>% filter(term == "predicted") %>% dplyr::select(num_comp, p.value, gene = coordinate) %>% mutate(sample = "nt") %>% bind_rows(tmp.value %>% filter(term == "predicted") %>% dplyr::select(num_comp, p.value, gene) %>% mutate(sample = "ko"))


tmp.value %>% dplyr::select(num_comp, p.value, gene) %>% mutate(sample = "ko") %>% filter()

#Put all three pc together
all.pc.together <- pc2.values %>% bind_rows(pc3.values)
#Summarise three pc
summarise.pc.tog <- all.pc.together %>% dplyr::group_by(num_comp, sample) %>% dplyr::summarise(counts = n())

# Run the same with non-targeting controls
#DDR library non-targetting controls (n = 33)
non.targetting.test.m <- non.targetting.CCD %>% filter(library == "DNA_repair")


nt.model.value <- tibble(library = NA, coordinate = NA, num_comp = NA, slope.log2 = NA, term = NA,p.value = NA)

for (i in unique(non.targetting.test.m$coordinate)){
  for (j in 1:10){
gene.library.dt <- filter(non.targetting.test.m, coordinate == i)
# Run a model per gene
set.seed(1)
PCR_model_DDR_test <- pcr(mean.log2foldchange~CTCF+EZH2+H2AFZ+H3K27ac+H3K27me3+H3K36me3+H3K4me1+H3K4me2+H3K4me3+H3K79me2+H3K9me2+H3K9me3+H4K5acK8ac+HDAC1+HDAC2+HDAC3+POL2+POL2AS2+SMC3+LMNB1+Dam+DNAse+m5C+late_replicating+TTseq, data=gene.library.dt , validation="CV")
...
combined.dt <- tibble(measured = gene.library.dt$mean.log2foldchange, predicted = as.numeric(pcr_pred))
pred_vs_estim <- lm(formula = measured ~ predicted, data = combined.dt) %>% tidy()
nt.model.value <- nt.model.value %>% add_row(coordinate = i, num_comp = j, slope.log2 = pred_vs_estim %>% pull(estimate), term = pred_vs_estim %>% pull(term), p.value = pred_vs_estim %>% pull(p.value))
  }
}


#p-value per gene
ggplot(nt.model.value %>% filter(term == "predicted")) + geom_density(aes(p.value)) + facet_wrap(~num_comp, scales = "free") + theme_bw() + geom_density(data = )

#Adjusted p-value for this fdr < 0.05 (100)
p.value.PCR.model.per.gene.correct.pc3 <- tmp.value %>% filter(term == "predicted" & num_comp == 3) %>% mutate(fdr = p.adjust(p.value, method = "fdr"))
...
#saveRDS(DDR.CCD.values.mean %>% left_join(significant.genes_p.value), file = "~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/xv20220316_DDR_CCD_mean_significance.rds")
