...
# Calculate z-scores based on this values
z.scores.ratio.IPR <- filter(log2.ratio.screen.detail.all, sample != "WT") %>%  left_join(null.distribtution.log2Ratio) %>% na.omit() %>% mutate(z.score.log2ratio = (log2ratio - wt.log2ratio)/sd.wt.log2ratio) %>% select(t.rep, library, gene, barcode,z.score.log2ratio,sample)

#Combine replicates
z.scores.ratio.mean <- z.scores.ratio.IPR %>% dplyr::group_by(library,gene,barcode,sample) %>% dplyr::summarise(mean.z.score.IPR = sum(z.score.log2ratio, na.rm = T)/sqrt(n()), count = n())

# Filter z-scores with two significant IPRs in at least 2 replicates
z.scores.ratio.IPR.filtered <- filter(z.scores.ratio.mean, abs(mean.z.score.IPR) > 1.95 & count > 1) %>% dplyr::group_by(gene,library, sample) %>% dplyr::summarise(MMEJ_signif = sum(mean.z.score.IPR < -1.95), NHEJ_signif = sum(mean.z.score.IPR > 1.95)) %>% filter(MMEJ_signif >= 1 | NHEJ_signif >= 1) %>% mutate(pathway_signif = case_when(MMEJ_signif == 0 ~ "NHEJ", NHEJ_signif == 0 ~ "MMEJ", T ~ "both"))
...
# Run the model: For this I need average values ()mock in all data
```{r}
#DDR library
...
KO.pvalue_b <- tibble(library = NA, gene = NA, num_comp = NA, slope.log2 = NA, term = NA,p.value = NA)
...
KO.pvalue_b <- KO.pvalue_b %>% add_row(gene = i, num_comp = j, slope.log2 = pred_vs_estim %>% pull(estimate), term = pred_vs_estim %>% pull(term), p.value = pred_vs_estim %>% pull(p.value))
  }
}
