...
# Calculate z-scores based on this values
z.scores.ratio.IPR <- log2.ratio.screen.detail.all %>%  left_join(null.distribtution.log2Ratio) %>% na.omit() %>% mutate(z.score.log2ratio = (log2ratio - wt.log2ratio)/sd.wt.log2ratio) %>% select(t.rep, library, gene, barcode,z.score.log2ratio,sample, well,plate)
...
log2.distance.ratio.KO <- log2.ratio.screen.detail.all %>% na.omit() %>% filter(sample == "KO") %>% left_join(wt.set.ratio, by = c("t.rep", "library", "barcode")) %>% mutate(log2.foldchange = log2ratio - wt.log2ratio) %>% left_join(z.scores.ratio.IPR.filtered) %>% na.omit()
...
gene.KO.CCD.dt_tmp <- tmp_KO.dt %>% dplyr::group_by(library,gene,well,plate,barcode) %>% dplyr::summarise(mean.log2foldchange = mean(log2.foldchange,na.rm = T)) %>% left_join(clone5_z.score_chrom_tib, by = c("barcode" = "ID"))

DDR.gene.library.test.m_tmp <- gene.KO.CCD.dt_tmp %>% filter(library == "DNA_repair")

KO.pvalue_c <- tibble(library = NA, gene = NA, num_comp = NA, r.squared = NA, adj.r.squared = NA,p.value = NA)

for (i in unique(DDR.gene.library.test.m_tmp$gene)){
  for (j in 1:10){
gene.library.dt <- filter(DDR.gene.library.test.m_tmp, gene == i)
...
KO.pvalue_c <- KO.pvalue_c %>% add_row(gene = i, num_comp = j, r.squared = pred_vs_estim %>% pull(r.squared), adj.r.squared = pred_vs_estim %>% pull(adj.r.squared), p.value = pred_vs_estim %>% pull(p.value))
  }
}
...
nt.model.value_b <- tibble(library = NA, gene = NA, num_comp = NA, r.squared = NA, adj.r.squared = NA,p.value = NA)
...
combined.dt <- tibble(measured = gene.library.dt$mean.log2foldchange, predicted = as.numeric(pcr_pred))
pred_vs_estim <- lm(formula = measured ~ predicted, data = combined.dt) %>% glance()
nt.model.value_b <- nt.model.value_b %>% add_row(gene = i, num_comp = j, r.squared = pred_vs_estim %>% pull(r.squared), adj.r.squared = pred_vs_estim %>% pull(adj.r.squared), p.value = pred_vs_estim %>% pull(p.value))
  }
}

#non-targetings
significant.nt <- nt.model.value%>% filter(term == "predicted") %>% dplyr::select(num_comp, p.value, gene = coordinate) %>% mutate(sample = "nt") 

#significant genes
significant.genes_p.value <- KO.pvalue_b %>% filter(term == "predicted") %>% dplyr::select(num_comp, p.value, gene) %>% mutate(sample = "ko") %>% bind_rows(significant.nt) %>% filter(num_comp == 3) %>% mutate(p.adj = p.adjust(p.value, method = "BH")) %>% select(gene,p.value,p.adj, sample)
...
CCD.non.targeting.mean <- non.t.CCD.mean %>% 
  reshape2::dcast(library + coordinate + feature ~ term, value.var = "slope.log2") %>% 
  dplyr::select(library, coordinate ,feature,intercept = '(Intercept)', slope = 'unlist(model.dt[j])') %>%
  left_join(significant.genes_p.value %>% 
              select(coordinate = gene, model_p.value = p.value, model_p.adj = p.adj)) %>%
  left_join(z.scores.ratio.IPR.filtered %>% 
              mutate(coordinate = paste(plate,well, sep = "_"))) %>% 
  filter(library == "DNA_repair") %>% mutate(cross.x = -intercept/slope, slope_cor = case_when(cross.x < 1 ~ slope, cross.x > 1 ~ 0)) %>% mutate(CCD_value = case_when(pathway_signif == "MMEJ" & slope_cor < 0 ~ slope_cor, pathway_signif == "MMEJ" & slope_cor > 0 ~ 0, pathway_signif == "NHEJ" & slope > 0 ~ slope_cor, pathway_signif == "NHEJ" & slope < 0 ~ 0, T ~ slope_cor))
...
for (k in unique(DDR.gene.library.test.m_tmp$library)) {
for (h in unique(DDR.gene.library.test.m_tmp$gene)) {
  for (j in colnames(DDR.gene.library.test.m_tmp)[9:33]) {
    model.dt <- DDR.gene.library.test.m_tmp %>% filter(library == k & gene == h)
   if (nrow(model.dt) == 0) {
next
}
    model.epistasis.log2 <- lm(formula = mean.log2foldchange ~ unlist(model.dt[j]), data = model.dt) %>% tidy()
...
gene.KO.CCD.mean.DDR_tmp <- gene.KO.CCD %>% reshape2::dcast(library + gene + feature ~ term, value.var = "slope.log2") %>% dplyr::select(library, gene ,feature,intercept = '(Intercept)', slope = 'unlist(model.dt[j])') %>% left_join(significant.genes_p.value %>% select(gene, model_p.value = p.value, model_p.adj = p.adj)) %>% left_join(z.scores.ratio.IPR.filtered %>% filter(library == "DNA_repair")) %>% mutate(cross.x = -intercept/slope, slope_cor = case_when(cross.x < 1 ~ slope, cross.x > 1 ~ 0)) %>% mutate(CCD_value = case_when(pathway_signif == "MMEJ" & slope_cor < 0 ~ slope_cor, pathway_signif == "MMEJ" & slope_cor > 0 ~ 0, pathway_signif == "NHEJ" & slope > 0 ~ slope_cor, pathway_signif == "NHEJ" & slope < 0 ~ 0, T ~ slope_cor))
...
saveRDS(gene.KO.CCD.mean.DDR, file = "~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/xv20220317_DDR_CCD_mean_significance.rds")
saveRDS(CCD.non.targeting.mean, file = "~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/xv20220317_non_targetting_DDR_CCD_mean_significance.rds")
