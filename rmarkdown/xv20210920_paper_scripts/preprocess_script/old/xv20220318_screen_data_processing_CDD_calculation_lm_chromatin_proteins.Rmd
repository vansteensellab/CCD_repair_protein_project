...
chromatin.gene.library.test.m_tmp <- gene.KO.CCD.dt_tmp %>% filter(library == "Chromatin")

KO.pvalue_chromatin <- tibble(library = NA, gene = NA, num_comp = NA, r.squared = NA, adj.r.squared = NA,p.value = NA)

for (i in unique(chromatin.gene.library.test.m_tmp$gene)){
  for (j in 1:10){
gene.library.dt <- filter(chromatin.gene.library.test.m_tmp, gene == i)
...
KO.pvalue_chromatin <- KO.pvalue_chromatin %>% add_row(gene = i, num_comp = j, r.squared = pred_vs_estim %>% pull(r.squared), adj.r.squared = pred_vs_estim %>% pull(adj.r.squared), p.value = pred_vs_estim %>% pull(p.value))
  }
}

#DDR library non-targetting controls (n = 33)
non.targetting.test.chromatin <- non.targetting.CCD %>% filter(library == "Chromatin")

nt.model.value_chromatin <- tibble(library = NA, gene = NA, num_comp = NA, r.squared = NA, adj.r.squared = NA,p.value = NA)

for (i in unique(non.targetting.test.chromatin$coordinate)){
  for (j in 1:10){
gene.library.dt <- filter(non.targetting.test.chromatin, coordinate == i)
# Run a model per gene
set.seed(1)
PCR_model_DDR_test <- pcr(mean.log2foldchange~CTCF+EZH2+H2AFZ+H3K27ac+H3K27me3+H3K36me3+H3K4me1+H3K4me2+H3K4me3+H3K79me2+H3K9me2+H3K9me3+H4K5acK8ac+HDAC1+HDAC2+HDAC3+POL2+POL2AS2+SMC3+LMNB1+Dam+DNAse+m5C+late_replicating+TTseq, data=gene.library.dt , validation="CV")
...
combined.dt <- tibble(measured = gene.library.dt$mean.log2foldchange, predicted = as.numeric(pcr_pred))
pred_vs_estim <- lm(formula = measured ~ predicted, data = combined.dt) %>% glance()
nt.model.value_chromatin <- nt.model.value_chromatin %>% add_row(gene = i, num_comp = j, r.squared = pred_vs_estim %>% pull(r.squared), adj.r.squared = pred_vs_estim %>% pull(adj.r.squared), p.value = pred_vs_estim %>% pull(p.value))
  }
}

#Plot R-squared
ggplot() + geom_density(data = nt.model.value_chromatin %>% filter(as.integer(num_comp) < 6), aes(r.squared), color = "red") + geom_density(data = KO.pvalue_chromatin %>% filter(as.integer(num_comp) < 6), aes(r.squared), color = "black") + facet_wrap(~num_comp, scales = "free") + theme_bw()

#Plot p-value
ggplot() + geom_density(data = nt.model.value_chromatin %>% filter(as.integer(num_comp) < 6), aes(p.value), color = "red") + geom_density(data = KO.pvalue_chromatin %>% filter(as.integer(num_comp) < 6), aes(p.value), color = "black") + facet_wrap(~num_comp, scales = "free") + theme_bw()

#Plot R-squared
ggplot() + geom_density(data = nt.model.value_chromatin %>% filter(as.integer(num_comp) < 6), aes(r.squared), color = "red") + geom_density(data = KO.pvalue_chromatin %>% filter(as.integer(num_comp) < 6), aes(r.squared), color = "black") + facet_wrap(~num_comp, scales = "free") + theme_bw()

#non-targetings
significant.nt_chromatin <- nt.model.value_chromatin %>% dplyr::select(num_comp, p.value, gene) %>% mutate(sample = "nt") 

#significant genes
significant.genes_p.value_chromatin <- KO.pvalue_chromatin %>% dplyr::select(num_comp, p.value, gene) %>% mutate(sample = "ko") %>% bind_rows(significant.nt_chromatin) %>% filter(num_comp == 3) %>% mutate(p.adj = p.adjust(p.value, method = "BH")) %>% select(gene,p.value,p.adj, sample)

#How maany nt are picked
sgnf.values <- significant.genes_p.value_chromatin %>% filter(p.adj < 0.05) %>% dplyr::group_by(sample) %>% dplyr::summarise(count = n())
#Total numbers
...
CCD.non.targeting.mean.chromatin <- non.t.CCD.mean %>%
  ...
  left_join(significant.genes_p.value_chromatin %>% 
  ...
  filter(library == "Chromatin") %>% mutate(cross.x = -intercept/slope, slope_cor = case_when(cross.x < 1 ~ slope, cross.x > 1 ~ 0)) %>% mutate(CCD_value = case_when(pathway_signif == "MMEJ" & slope_cor < 0 ~ slope_cor, pathway_signif == "MMEJ" & slope_cor > 0 ~ 0, pathway_signif == "NHEJ" & slope > 0 ~ slope_cor, pathway_signif == "NHEJ" & slope < 0 ~ 0, T ~ slope_cor)) %>% na.omit()
...
```{r}
# Compute slopes for all experiments
gene.KO.CCD_chromatin <- tibble(library = NA, gene = NA, feature = NA, slope.log2 = NA, term = NA,p.value = NA)

for (k in unique(chromatin.gene.library.test.m_tmp$library)) {
for (h in unique(chromatin.gene.library.test.m_tmp$gene)) {
  for (j in colnames(chromatin.gene.library.test.m_tmp)[9:33]) {
    model.dt <- chromatin.gene.library.test.m_tmp %>% filter(library == k & gene == h)
   if (nrow(model.dt) == 0) {
next
}
    model.epistasis.log2 <- lm(formula = mean.log2foldchange ~ unlist(model.dt[j]), data = model.dt) %>% tidy()
   gene.KO.CCD_chromatin <- gene.KO.CCD_chromatin %>% add_row(library = k,gene = h, feature = j, slope.log2 = model.epistasis.log2 %>% pull(estimate), term = model.epistasis.log2 %>% pull(term), p.value = model.epistasis.log2 %>% pull(p.value))
  }
}
}

#Calculate KO proteins (terms)
gene.KO.CCD.mean.chromatin <- gene.KO.CCD_chromatin %>% reshape2::dcast(library + gene + feature ~ term, value.var = "slope.log2") %>% dplyr::select(library, gene ,feature,intercept = '(Intercept)', slope = 'unlist(model.dt[j])') %>% left_join(significant.genes_p.value_chromatin %>% select(gene, model_p.value = p.value, model_p.adj = p.adj)) %>% left_join(z.scores.ratio.IPR.filtered %>% filter(library == "Chromatin")) %>% mutate(cross.x = -intercept/slope, slope_cor = case_when(cross.x < 1 ~ slope, cross.x > 1 ~ 0)) %>% mutate(CCD_value = case_when(pathway_signif == "MMEJ" & slope_cor < 0 ~ slope_cor, pathway_signif == "MMEJ" & slope_cor > 0 ~ 0, pathway_signif == "NHEJ" & slope > 0 ~ slope_cor, pathway_signif == "NHEJ" & slope < 0 ~ 0, T ~ slope_cor))

#Filter significant hits
chromatin_proteins_selected <- gene.KO.CCD.mean.chromatin %>% na.omit() %>% filter(model_p.adj < 0.05)
...
saveRDS(gene.KO.CCD.mean.chromatin, file = "~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/xv20220412_chromatin_CCD_mean_significance_all.rds")
