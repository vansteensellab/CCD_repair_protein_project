...
# Summary values
K27_domain_mutations_summary <- K27_domain_mutations %>% purrr::reduce(left_join, by = c("chr","start","end","strand","mut_id")) %>% select(-DET562_GSE149670.txt)%>% mutate(domain_counts = rowSums(. == "Domain"), idomain_counts = rowSums(. == "iDomain"), H3K27me3_status = case_when(domain_counts %in% c(0,1) ~ "ciH3K27me3", domain_counts %in% c(4,5) ~ "cH3K27me3", domain_counts %in% c(2) ~ "fiH3K27me3", domain_counts %in% c(3) ~ "fH3K27me3")) 
...
#Plot H3K27me3
ggplot(per_sample_muts_K27_calc)  + geom_boxplot(aes(type, K27_iK27_ratio)) + geom_quasirandom(aes(type, K27_iK27_ratio))
...
stat_K27 <- map_dfr(mut_type, .f = function(x) {
  ...
  wilcox.test(mut_mut,spo_mut) %>% tidy()
})
...
#Plot LAD vs iLAD
ggplot(per_sample_muts_LAD_calc)  + geom_boxplot(aes(fct_relevel(type, c("spo","BRCA2mut","BRCA1mut","FA"), ), LAD_iLAD_ratio, fill = type), alpha = 0.5,outlier.shape = NA) + geom_quasirandom(aes(type, LAD_iLAD_ratio)) + theme_bw()
