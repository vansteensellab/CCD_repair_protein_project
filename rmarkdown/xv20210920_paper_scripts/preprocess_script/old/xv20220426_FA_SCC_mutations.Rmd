...
```{r}
# Prepare data frame
mut_per_tumour <- mutation_location_export  %>%mutate(type = case_when(grepl("BRCA2", mut_id) ~ "BRCA2mut", grepl("BRCA1", mut_id) ~ "BRCA1mut",grepl("HNSCC", mut_id) ~ "spo", grepl("F",mut_id) ~ "FA"))  %>% separate(mut_id, sep = "_(?=[1-9])", into = c("sample","id")) %>% rowwise() %>% mutate(HPV = grepl("HPVPos", sample)) %>% select(sample, type,HPV) %>% distinct() %>% dplyr::group_by(type,HPV) %>% dplyr::summarise(s_count = n())

#Total amount of mutations
ggplot(sim.chrom.status.cd) + geom_bar(aes(fct_relevel(type, c("spo")), fill = del_class)) + theme_bw()

#
tmp.mut <- sim.chrom.status.cd %>% dplyr::group_by(sample,type,del_class) %>% dplyr::summarise(count = n()) %>% ungroup() %>% dplyr::group_by(type,del_class) %>% dplyr::summarise(mean_c = mean(count))

ggplot(tmp.mut) + geom_col(aes(fct_relevel(type, c("spo")),mean_c, fill = del_class)) + theme_bw() + ylab("Mutations per tumour") + xlab("Tumor class")

#calculate per tumour
sim.ch.tumor.hpv <- sim.chrom.status.cd.hpv %>% dplyr::group_by(type,del_class,HPV) %>% dplyr::summarise(count = n()) %>% left_join(mut_per_tumour) %>% rowwise() %>% mutate(mean_c = count/s_count) %>% ungroup()


ggplot(sim.ch.tumor.hpv %>% filter(del_class == "MH")) + geom_col(aes(fct_relevel(paste(type,HPV, sep = "_"), c("spo_TRUE","spo_FALSE","BRCA2mut_FALSE")),mean_c,)) + theme_bw() + ylab("Average MH deletions per tumour") + xlab("Tumor class")

#Only focus on MH
ggplot(sim.ch.tumor.hpv) + geom_col(aes(fct_relevel(paste(type,HPV, sep = "_"), c("spo_TRUE","spo_FALSE")),mean_c, fill = del_class)) + theme_bw() + ylab("Average  per tumour") + xlab("Tumor class")


#Plot split by LAD and iLAD
LAD.data.mut <- sim.chrom.status.cd.hpv %>% dplyr::group_by(type,del_class,HPV, LAD_status) %>% dplyr::summarise(count = n()) %>% left_join(mut_per_tumour) %>% rowwise() %>% mutate(mean_c_LAD = count/s_count) %>% ungroup()
LAD.data.mut.filtered <- LAD.data.mut %>% filter(LAD_status %in% c("ciLAD","cLAD") & del_class == "MH")

ggplot(LAD.data.mut.filtered) + geom_col(aes(LAD_status, mean_c_LAD, fill = LAD_status)) + theme_bw() + ylab("Mutations per tumour") + xlab("Tumor class") + facet_wrap(~ fct_relevel(paste(type,HPV, sep = "_"), c("spo_TRUE","spo_FALSE", "BRCA2mut_FALSE")))
