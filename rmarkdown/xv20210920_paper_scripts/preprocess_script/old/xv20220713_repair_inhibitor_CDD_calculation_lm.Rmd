...
for( i in unique(log2.distance.ratio.inhibitors.noGFP$drug)) {
  for(j in unique(log2.distance.ratio.inhibitors.noGFP$barcode)) {
    IPR.drug <- log2.distance.ratio.inhibitors.noGFP %>% filter(drug == i & barcode == j) %>% pull(log2ratio)
   if (length(IPR.drug) < 3) {
next
}
    IPR.DMSO <- log2.distance.ratio.inhibitors.noGFP %>% filter(drug == "DMSO" & barcode == j) %>% pull(log2ratio)
...
significant.inh.wilc.test <- inh.wilc.test %>% filter(drug %in% c("ATMi","DNAPKi")) %>% mutate(p.adj = p.adjust(p.value, method = "BH")) %>% dplyr::select(drug,barcode,p.value,p.adj, mean)
