...
mean.diff.balance <- sel.inhibitor.data %>% 
  filter(drug != "DMSO") %>%
  group_by(drug) %>% 
...
#Create table S3: Data to export all major features
```{r}
export_screen_result_drug <- estimated_FC_CCD %>% 
  left_join(mean.diff.balance.export, by = "drug") %>%
  dplyr::select(drug, CCD_model_p_adj = "model_p.adj",chrom_feature = "feature", CCD_synergy_score = "CCD_value",CCD_estim_diff = "estim_fc_CCD", global_diff = "mean.balance.diff", global_p_adj = "p.adj", drug_effect_type = "class")
...
saveRDS_proof(sel.inhibitor.data, file = "data/processed_data/CCD_inhibitors/xv20220816_inhibitors_repair_metrics.rds")

#Step 2: dcast data table for 
saveRDS_proof(log2.distance.ratio, file = "data/processed_data/CCD_inhibitors/xv20220816_drug_differentials_log2_MMEJ_NHEJ_balance.rds")

#Step 3: Student's t-test p-value
saveRDS_proof(significant.inh.t.test, file = "data/processed_data/CCD_inhibitors/xv20220816_drug_t_test_final_values.rds") #T-test values

#Step 4: diff log2 MMEJ:NHEJ values for DR proteins with chromatin scores
saveRDS_proof(drug.CCD.dt, file = "data/processed_data/CCD_inhibitors/xv20220816_drug_log2_MMEJNHEJ_differentials_chromatin_KO.rds")

#Step 5: save global changes for all proteins
saveRDS_proof(mean.diff.balance.export, file = "data/processed_data/CCD_inhibitors/xv20220816_drug_global_diff_MMEJNHEJ.rds")

#Final data table: Table S3
saveRDS_proof(export_screen_result_drug, file = "data/processed_data/xv20220816_drug_CCD_results.rds")
write.csv(export_screen_result_drug, file = "data/processed_data/xv20220816_Table_drug_CCD_results.csv")
