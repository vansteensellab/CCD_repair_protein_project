...
#Extract non-targeting samples from this dataset
...
```
#Alternative Step2: fit normal distribution through wt values
```{r}
...
fit.z.scores.ratio.IPR.filtered <- filter(fit.z.scores.ratio.mean, abs(mean.z.score.IPR) > 1.95 & count > 1) %>% dplyr::group_by(gene,library, sample,well,plate) %>% dplyr::summarise(MMEJ_signif = sum(mean.z.score.IPR < -1.95), NHEJ_signif = sum(mean.z.score.IPR > 1.95)) %>% filter(MMEJ_signif >= 2 | NHEJ_signif >= 2) %>% mutate(pathway_signif = case_when(MMEJ_signif == 0 ~ "NHEJ", NHEJ_signif == 0 ~ "MMEJ", T ~ "both"))
...
adj_p.value_KO_model <- DDR_KO_CCDs_dt %>% dplyr::select(num_comp, p.value, gene) %>% mutate(sample = "ko") %>% filter(num_comp == 3) %>% mutate(p.adj = p.adjust(p.value, method = "BH")) %>% dplyr::select(gene,p.value,p.adj, sample)
```
...
adj_p.value_NT_model <- DDR_NT_CCDs_model %>% dplyr::select(num_comp, p.value, gene) %>% mutate(sample = "nt") %>% filter(num_comp == 3) %>% mutate(p.adj = p.adjust(p.value, method = "BH")) %>% dplyr::select(gene,p.value,p.adj, sample)
```
