...
summary_read_number <- indels.dt %>% mutate(proccessed_read = rowSums(indels.dt[,c(-1,-2)]), indel_reads = rowSums(indels.dt[,c("ins_1","del_7")])) %>% select(exp, barcode, proccessed_read, indel_reads, del_7, ins_1)
...
final_distribution_table <- tibble(quantile = names(distribution_del_7), 
                                   processed_reads = distribution_reads,
                                   indel_reads = distribution_indels,
...
DDR.KO.CCD.dt_tmp <- log2.distance.ratio %>%
  filter(sample == "KO") %>%
...
DDR.NT.CCD.dt_tmp <- log2.distance.ratio %>% 
  filter(sample == "WT") %>%
...
samples_significant_CCDs <- adj_p.value_model %>% 
  ...
  dplyr::summarise(counts = n())#1 mock values have significant CCDs (3% of total proteins) & 90 KO proteins
```
...
saveRDS_proof(screen.data.names.tib, file = "data/processed_data/CCD_analysis/xv20220802_screening_raw_data_repair_metrics.rds")

#Step 2: dcast data table for 
saveRDS_proof(log2.distance.ratio, file = "data/processed_data/CCD_analysis/xv20220802_differentials_log2_MMEJ_NHEJ_balance.rds")

#Step 3: z-scores every step
saveRDS_proof(fit.null.distribtution.log2Ratio, file = "data/processed_data/CCD_analysis/xv20220802_z_test_null_distribution_parameters.rds") #Null distribution parameters
saveRDS_proof(fit.z.scores.ratio.IPR, file = "data/processed_data/CCD_analysis/xv20220802_z_test_per_replicate_transformation.rds") #Z-test without any averaging
saveRDS_proof(fit.z.scores.ratio.mean, file = "data/processed_data/CCD_analysis/xv20220802_z_test_final_values.rds") #Z-test values

#Step 4: diff log2 MMEJ:NHEJ values for DR proteins with chromatin scores
saveRDS_proof(DDR.KO.CCD.dt, file = "data/processed_data/CCD_analysis/xv20220802_log2_MMEJNHEJ_differentials_chromatin_KO.rds")

#Step 5: save global changes for all proteins
saveRDS_proof(mean.diff.balance.export, file = "data/processed_data/CCD_analysis/xv20220802_global_diff_MMEJNHEJ.rds")

#Final data table: Table S3
saveRDS_proof(export_screen_result_TS3, file = "data/processed_data/xv20220802_Table_S3_DR_screen_results.rds")
write.csv(export_screen_result_TS3, file = "data/processed_data/xv20220802_Table_S3_DR_screen_results.csv")
