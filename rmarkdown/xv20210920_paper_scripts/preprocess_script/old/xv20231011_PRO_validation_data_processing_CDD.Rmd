...
RPE1_data_chrom_tib <- readRDS_proof("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen","chromatin_data_RPE1") %>%
  filter(cell_line == "PRO" & binsize == 2000) #Chromatin
...
ggplot() + 
  geom_point(data = summary_table_control_plots,aes(IPR_frequency, log2((del_7 + 1) / (ins_1 + 1)), color = IPR_frequency > 0.008)) +
  theme_bw() +
  theme(axis.title.x = element_blank()) + 
  ylab("log2MMEJ:NHEJ") +
  theme(legend.position = "none") +
  scale_color_manual(values = c("grey20","#95c482"))
```
...
CCDs_dt_RPE1 <- tibble(cell_line = NA, gene = NA, r.squared = NA, adj.r.squared = NA,p.value = NA)
  ...
  for (i in unique(cell_line_df$gene)){
    gene.library.dt <- filter(DDR.KO.CCD.dt, gene == i)
    set.seed(1)
       if (nrow(gene.library.dt) < 9) {
next
}
    PCR_model_DDR_test <- pls::pcr(mean.log2foldchange~ chip_H3K4me1+chip_H3K36me3+dam_LMNB1+chip_H3K4me2+chip_H3K4me3+chip_H3K27ac+dam_H3K9me2+dam_H3K9me3+late_replicating+dam_H3K27me3, data=gene.library.dt, validation = "CV") #Run principal component regression
...
CCDs_dt_RPE1_adjs <- CCDs_dt_RPE1%>% mutate(fdr = p.adjust(p.value, method = "BH")) %>% left_join(test.mean.diff.balance %>% select(gene,n))
...
export_screen_result_TS7 <- estimated_FC_CCD %>% 
  ...
  left_join(pathway_summary) %>%
  dplyr::select(gene, cell_line, CCD_model_p_adj = "model_p.adj",chrom_feature, CCD_synergy_score = "CCD_value",CCD_estim_diff = "estim_fc_CCD", global_diff = "mean.balance.diff", global_p_adj = "p.adj", n, pathway_signif)
...
saveRDS_proof(screen.data.names.tib, file = "data/processed_data/CCD_validation_RPE1/xv20231011_PRO_screening_raw_data_repair_metrics.rds")

#Step 2: dcast data table for 
saveRDS_proof(log2.distance.ratio, file = "data/processed_data/CCD_validation_RPE1/xv20231011_PRO_differentials_log2_MMEJ_NHEJ_balance.rds")

#Step 4: diff log2 MMEJ:NHEJ values for DR proteins with chromatin scores
saveRDS_proof(DDR.KO.CCD.dt, file = "data/processed_data/CCD_validation_RPE1/xv20231011_PRO_log2_MMEJNHEJ_differentials_chromatin_KO.rds")

#Step 5: save global changes for all proteins
saveRDS_proof(mean.diff.balance.export, file = "data/processed_data/CCD_validation_RPE1/xv20231011_PRO_global_diff_MMEJNHEJ.rds")

#Save final results as rds
saveRDS_proof(export_screen_result_TS7, file = "data/processed_data/xv20231011_PRO_CCD_summary.rds")
