...
toxic_genes <-dna_quantification %>%
  ...
  select(gRNA) %>%
  distinct()
#there's no gRNA that is toxic compared to NTC
...
log2.distance.ratio <- screen.data.names.tib %>% 
  ...
  dplyr::group_by(gene,barcode,gRNA) %>% #Group by sample
...
DDR.KO.CCD.dt <- log2.distance.ratio %>%
  ...
  left_join(RPE1_data_chrom_tib, by = c("barcode"))#Join chromatin info
...
next
    ...
    CCDs_dt_RPE1 <- CCDs_dt_RPE1 %>% add_row(cell_line = "RPE1", gene = i, r.squared = pred_vs_estim %>% pull(r.squared), adj.r.squared = pred_vs_estim %>% pull(adj.r.squared), p.value = pred_vs_estim %>% pull(p.value)) #Extract valuable metrics
  }
...
export_screen_result_TS7 <- estimated_FC_CCD %>% 
  ...
  left_join(pathway_summary) %>%
  dplyr::select(gene, cell_line, CCD_model_p_adj = "model_p.adj",chrom_feature, CCD_synergy_score = "CCD_value",CCD_estim_diff = "estim_fc_CCD", global_diff = "mean.balance.diff", global_p_adj = "p.adj", pathway_signif, n)
...
saveRDS_proof(screen.data.names.tib, file = "data/processed_data/CCD_validation_RPE1/xv20231011_RPE1_screening_raw_data_repair_metrics.rds")

#Step 2: dcast data table for 
saveRDS_proof(log2.distance.ratio, file = "data/processed_data/CCD_validation_RPE1/xv20231011_RPE1_differentials_log2_MMEJ_NHEJ_balance.rds")

#Step 4: diff log2 MMEJ:NHEJ values for DR proteins with chromatin scores
saveRDS_proof(DDR.KO.CCD.dt, file = "data/processed_data/CCD_validation_RPE1/xv20231011_RPE1_log2_MMEJNHEJ_differentials_chromatin_KO.rds")

#Step 5: save global changes for all proteins
saveRDS_proof(mean.diff.balance.export, file = "data/processed_data/CCD_validation_RPE1/xv20231011_RPE1_global_diff_MMEJNHEJ.rds")

#Save final results as rds
saveRDS_proof(export_screen_result_TS7, file = "data/processed_data/xv20231011_RPE1_CCD_summary.rds")
