...
filtered_screen_data <- grouped_screen.tib %>%
  ...
  filter(replicate != "R4" & ins_1 > 50 & del_7 > 50) %>%
...
mean.diff.balance.export <- test.mean.diff.balance %>%
  dplyr::select(inhibitor,p,p.adj,mean.balance.diff = mean.fc, n)
...
synergy_summaries <- DR_KO_CCD_all_values %>% filter(model_p.adj < 0.25) %>% dplyr::group_by(inhibitor) %>% dplyr::summarise(M_synergy = sum(CCD_value < 0), N_synergy = sum(CCD_value > 0)) %>% distinct() %>% mutate(synergy_class = case_when(N_synergy == 0 ~ "M_synergy", M_synergy == 0 ~ "N_synergy", T ~ "both")) %>% dplyr::group_by(synergy_class) %>% dplyr::summarise(c = n())
```
...
saveRDS_proof(export_screen_result_TS7, file = "data/processed_data/xv20231013_DEF_CCD_summary.rds")
