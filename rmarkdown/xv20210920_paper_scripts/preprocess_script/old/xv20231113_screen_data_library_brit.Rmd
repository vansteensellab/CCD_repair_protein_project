...
#Load Brit Adamson library specification
hussman_library <- read_xlsx("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/import/xv20231113_brit_adamson_library.xlsx") %>%
  mutate(plot_annotation = gsub(" ","_", `Plot Annotation`),
         all_annotation = gsub(" ","_", `All Annotations`)) %>%
  select(Gene, plot_annotation, all_annotation) %>%
  separate(all_annotation, into = c("A","B","C","D","E","F","G","H","I","J","K","L","M","N","O","P"), sep = ",_")

#Merge with DR_library_Infor
DR_brit_adamson <- DR_library_info %>%
  left_join(hussman_library, c("gene" = "Gene"))

#Filter significant
significant_proteins <- table_S3_CCDs %>% filter(CCD_model_p_adj < 0.05) %>% na.omit() #Melt

#filter significant genes
tmp <- DR_brit_adamson %>% filter(gene %in% significant_proteins$gene)
