...
mean.diff.balance.export <- test.mean.diff.balance %>%
  ...
  left_join(log2.distance.ratio %>% dplyr::select(gene,well,plate)) %>%
  distinct()
...
saveRDS_proof(export_screen_result_TS7, file = "data/processed_data/xv20231130_Table_S7_DR_screen_results.rds")
...
openxlsx::write.xlsx(export_screen_result_TS7, file = "data/supplementary_tables/xv20231130_Table_S7a_global_CCD_MMEJ_NHEJ_results.xlsx", keepNA = T)
