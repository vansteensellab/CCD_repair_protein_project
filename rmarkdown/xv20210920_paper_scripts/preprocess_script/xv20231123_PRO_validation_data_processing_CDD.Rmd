...
synergy_per_gene <- DR_KO_CCD_all_values %>% filter(model_p.adj < 0.25) %>% dplyr::group_by(gene) %>% dplyr::summarise(M_synergy = sum(CCD_value < 0), N_synergy = sum(CCD_value > 0)) %>% distinct() %>% mutate(CCD_effect = case_when(N_synergy == 0 ~ "M_synergy", M_synergy == 0 ~ "N_synergy", T ~ "both"))
...
export_screen_result_TS4 <- log2.distance.ratio %>% 
  mutate(cell_line = "RPE1_p53_KO") %>%
  dplyr::select(cell_line, gene,"gRNA_pool"=gRNA, IPR_barcode = "barcode", delta_log2_MMEJ_NHEJ = "mean.log2foldchange", number_replicates = "n_rep")
```
...
IPR_locations <- read_delim("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20230307_revision_figures/DSB_TRIP_mapping/xv20230519_RPE_pools_mapping.tsv") %>%
  filter(grepl("RPE1Proff_Low_250", name)) %>% separate(name, into =c("barcode", NA, NA, NA))

# Data table to export
PRO_bc_used <- DDR.KO.CCD.dt %>%
  filter(binsize == 2000) %>%
  select(-gene, -gRNA, -n_rep, -dam_LMNB2, -mean.log2foldchange,-pathway_signif, -binsize) %>%
  distinct() %>%
  mutate(cell_line = "RPE1_p53_KO")

export_screen_result_TS6 <- IPR_locations %>% right_join(PRO_bc_used, by = c("barcode"))
```
...
export_screen_result_TS7 <- pathway_summary %>%
  ...
  mutate(toxicity = case_when(gene %in% toxic_genes$gene ~ "low_recovery",
         T ~ "normal_recovery")) %>%
  ...
  mutate(global_effect = case_when(global_p_adj < 0.05 & global_diff < 0 ~ "favorMMEJ",
                                   ...
                                   T ~ "none"),
         cell_line = "RPE1_p53_KO") %>%
  left_join(synergy_per_gene %>% 
              select(gene, CCD_effect))
...
saveRDS_proof(export_screen_result_TS7, file = "data/processed_data/xv20231117_PRO_CCD_summary.rds")
...
write.xlsx(export_screen_result_TS4, file = "data/supplementary_tables/xv20231123_Table_S4c_RPE1_p53KO_delta_log2_MMEJ_NHEJ.xlsx") #Check!

#Table S6: Genomic coordinates, chromatin feature scores and barcodes of 19 IPRs in K562 clone 5
write.xlsx(export_screen_result_TS6, file = "data/supplementary_tables/xv20231117_Table_S6_RPE1_p53KO_chromatin_features.xlsx") #Check!

#Table S7: Chromatin context dependent effects of proteins in the screen.
write.xlsx(export_screen_result_TS7, file = "data/supplementary_tables/xv20231117_Table_S7c_p53KO_global_CCD_MMEJ_NHEJ_results.xlsx",keepNA = T)
