...
big.rearrangement.2 <- slope.protein.features.all %>% 
...
slope.prot.features.dcast.hits <- slope.protein.features.all %>% filter(gene %in% big.rearrangement.2) %>% reshape2::dcast(gene ~ feature, value.var = "epistasis.final") %>% column_to_rownames(var = "gene")
...
interaction.summary <- slope.protein.features.all %>% dplyr::group_by(gene) %>% dplyr::summarise(mmej.ep = sum(epistasis.final > 0), nhej.ep = sum(epistasis.final < 0)) %>% mutate(gene.class = case_when(mmej.ep == 0 & nhej.ep > 0 ~ "all.nhej",nhej.ep == 0 & mmej.ep > 0 ~ "all.mmej", nhej.ep > 0 & mmej.ep > 0 ~ "both.ep", T ~ "no.ep"))

#slope.with.gene.feature
gene.f.slope.protein <- slope.protein.features.all %>% left_join(interaction.summary)

epistasis.summary <- slope.protein.features.all %>% 
...
# Both sides
ggplot(slope.protein.features.all %>% left_join(interaction.summary, by = "gene") %>% filter(gene %in% mmej.genes.highlight)) + 
  geom_tile(aes(fct_relevel(gene,heatmap.gene.order.slope.diff.mmej),fct_relevel(feature, heatmap.chromatin.order.slope.diff.hits), fill = epistasis.final)) +
...
# Both sides
ggplot(slope.protein.features.all %>% left_join(interaction.summary, by = "gene") %>% filter(gene %in% nhej.genes.highlight)) + 
...
# Both sides
ggplot(slope.protein.features.all %>% left_join(interaction.summary, by = "gene") %>% filter(gene %in% mix.genes.highlight)) + 
...
```{r}
# Filter for umap
umap.data.table <- slope.protein.features.all %>% mutate(epistasis.sign = case_when(epistasis.final > 0 ~ 1, epistasis.final < 0 ~ -1, epistasis.final == 0 ~ 0)) %>% dcast(gene ~ feature, value.var = "epistasis.sign") %>% column_to_rownames(var = "gene")

umap.epistasis <- umap(umap.data.table)
tmp.umap.epistasis <- umap.epistasis$layout
colnames(tmp.umap.epistasis) <- c("A","B")


ggplot(as.data.frame(tmp.umap.epistasis)) + geom_point(aes(A,B))
```
