...
slope.prot.features.scale.dcast.FA <- slope.protein.features.all %>% filter(gene %in% c("FANCL","FAAP100","FAAP24","FANCA","FANCB","FANCC","FANCE","FANCF","FANCG","FANCM","FANCD2","FANCI")) %>% reshape2::dcast(gene ~ feature, value.var = "epistasis.final") %>% column_to_rownames(var = "gene")

#Heatmap for slope differences
heatmap.epistasis.FANC <- pheatmap(slope.prot.features.scale.dcast.FA )
gene.order.FA <- rownames(slope.prot.features.scale.dcast.FA[heatmap.epistasis.FANC$tree_row[["order"]],])
feature.order.FA <- colnames(slope.prot.features.scale.dcast.FA[heatmap.epistasis.FANC$tree_col[["order"]]])
...
ggplot(slope.protein.features.all %>% filter(gene %in% c("FAAP24","FANCM","FANCA","FANCG","FANCF","FANCC","FAAP100","FANCE","FANCL","FANCB","FANCI","FANCD2"))) + 
  geom_tile(aes(fct_relevel(feature,rev(feature.order.FA)),fct_relevel(gene,c("FAAP24","FANCM","FANCA","FANCG","FANCF","FANCC","FAAP100","FANCE","FANCL","FANCB","FANCI","FANCD2")), fill = epistasis.final)) +
  scale_fill_gradient2( low = "#8c510a" ,mid = "#f5f5f5", high = "#01665e")  + 
...
ggplot(gene.f.slope.protein %>% filter(gene %in% c("FAAP24","FANCM","FANCA","FANCG","FANCF","FANCC","FAAP100","FANCE","FANCL","FANCB","FANCI","FANCD2"))) +
  geom_tile(aes(fct_relevel(gene,c("FAAP24","FANCM","FANCA","FANCG","FANCF","FANCC","FAAP100","FANCE","FANCL","FANCB","FANCI","FANCD2")),fct_relevel(feature, rev(feature.order.FA)),fill = epistasis.final.b)) +
  scale_fill_gradient2( low = "#8c510a" ,mid = "#f5f5f5", high = "#01665e")  + 
  coord_fixed(expand = F, ratio = 1) + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), axis.title = element_blank(), legend.position = "top")
