...
#Epistasis analysis
slope.protein.features.all <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/xv20220128_ratio_epistasis_all_genes.rds")
...
mmej_module_I_euchr_features <- tibble(feature = c("H3K4me1","H3K27ac","H3K4me2","H3K4me3","H4K5acK8ac","TTseq","POL2","POL2AS2"), module = "mmej_I_euchromatin", gene.class = "all.mmej")
...
ggplot(module.summary  %>% filter(grepl("nhej", module) & mean < -0.02) %>% na.omit() %>% ungroup() %>% select(librar,gene,module)  %>% distinct() %>% group_by(librar,module) %>% summarise(counts = n())) + geom_col(aes(fct_relevel(module,c("nhej_I_euchromatin")), counts, fill = fct_relevel(librar,c("DNA_repair","both","Chromatin"))), position = "fill") + theme_bw() + coord_cartesian(expand = F) + scale_fill_manual(values = c("#56B3E6","grey40","#E69F03")) + theme(axis.title.x = element_blank(), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
# 
...
# Signalling factors
# MLL4 complex
ggplot(module_III_nhej_gost$result %>% dplyr::group_by(term_name, term_id) %>% dplyr::summarise(p_val = -log10(mean(p_value)), CORUM_name = paste0(term_name, term_id, sep = "_"))) + geom_col(aes(p_val,reorder(CORUM_name,p_val)), fill = "#EB2030") + coord_cartesian(expand = F) + theme_minimal() + xlab("-log10(p-adj)") + ylab("")
...
```{r}
epistasis.drug <- slope.protein.features.inhibitors.filt %>% bind_rows(proteins.PIKK) %>% filter(epistasis.final != 0) %>% dplyr::group_by(drug,mean.fc) %>% dplyr::summarise(mean.epistasis = mean(epistasis.final, na.rm = T), max.epistasis = sign(mean.fc)*(max(abs(epistasis.final), na.rm = T))) %>% mutate(class = case_when(grepl("i",drug) ~ "inhibitor", T ~ "KO"))
