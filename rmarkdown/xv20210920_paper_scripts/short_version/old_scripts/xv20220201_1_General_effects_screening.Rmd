...
```
# Mock example: presentation
```{r}
mock.slopes <- tibble(z_score = seq(-3,3,0.25), slope_0 = z_score*0 + 0.6, slope_0_15 = z_score*0.15 +0.6,slope_0_2 = z_score*0.2 + 0.6,slope_0_1 = z_score*0.1 +0.6,slope_0.05= z_score*0.05 + 0.6, slope_m_0_1 = z_score*(-0.1)+0.6) %>% reshape2::melt(id.vars = "z_score")

ggplot(mock.slopes %>% filter(variable %in% c("slope_0","slope_m_0_1", "slope_0_1", "slope_0_2")), aes(z_score,value,color = variable)) + geom_smooth() + theme_bw() + geom_vline(xintercept = 0, linetype = 2) + geom_hline(yintercept = 0, linetype = 2) + ylab("log2(fold change NHEJ:MMEJ)") + xlab("Feature signal (z-score)") + theme(legend.position = "none") + coord_cartesian(ylim= c(-0.5,1.5)) + scale_color_manual(values = c("black","red","orange","blue"))
bar.mock <- tibble(name = c("slope_0","slope_m_0_1", "slope_0_1", "slope_0_2"), slope = c(0,-0.1,0.1,0.2))

ggplot(bar.mock) + geom_col(aes(name,slope)) + scale_fill_brewer(palette = "Spectral")

ggplot(bar.mock) + geom_col(aes(name,slope, fill = as.character(slope)))  + scale_fill_manual(values = c("blue","black","orange","red"))+ theme_bw()
```
...
controls.by.well <- log2.ratio.screen.detail.all %>% dplyr::group_by(well,plate,library,barcode,sample) %>% dplyr::summarise(log2_ratio = mean(log2ratio, na.rm = T)) %>% left_join(wt.set.ratio) %>% mutate(log2.ratio = log2_ratio - wt.log2ratio)

ggplot(controls.by.well) + geom_quasirandom(aes(fct_relevel(barcode, clone5_bc_cluster),log2_ratio, color = fct_relevel(sample, c("WT","POLQ","KO"))), dodge.width = 0.5) + theme_bw() + theme(axis.text.x = element_blank(), legend.position = "top")
...
# Other way for plotting controls
all.values <- log2.ratio.screen.detail.all %>% left_join(wt.set.ratio) %>% mutate(log2.ratio = log2ratio - wt.log2ratio) %>% dplyr::group_by(well,plate,library,barcode,sample) %>% dplyr::summarise(log2_ratio = mean(log2.ratio, na.rm = T), log2_balance = mean(log2ratio, na.rm = T))

all.values.mean <- all.values %>% dplyr::group_by(well,plate,library,sample) %>% dplyr::summarise(log2_b_mean = mean(log2_ratio, na.rm = T))

ggplot(all.values) + geom_quasirandom(aes(fct_relevel(barcode, clone5_bc_cluster),log2_ratio, color = fct_relevel(sample, c("WT","POLQ","KO"))), dodge.width = 0.5) + theme_bw() + theme(axis.text.x = element_blank(), legend.position = "top") + coord_cartesian(ylim = c(-2.3,2.3))

ggplot(all.values.mean) + geom_quasirandom(aes("mean",log2_b_mean, color = fct_relevel(sample, c("WT","POLQ","KO"))), dodge.width = 1) + theme_bw() + theme(axis.text.x = element_blank(), legend.position = "top") + coord_cartesian(ylim = c(-2.3,2.3))
