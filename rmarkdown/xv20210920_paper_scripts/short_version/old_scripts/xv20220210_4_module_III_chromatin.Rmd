...
```
# Figure supp 4B: Complexes enriched in module I
```{r}
...
PA1.combined <- bind_rows(screen.PA1,PA1.followup) %>% mutate(gene_gRNA = paste(gene,gRNA, sep = "_")) 
...
ggplot(PA1.combined) +
  geom_tile(aes(fct_relevel(gene_gRNA,pa1.gene.order), fct_relevel(feature,heatmap.chromatin.order), fill = epistasis.final)) + scale_fill_gradient2( low = "#8c510a" ,mid = "#f5f5f5", high = "#01665e", limits = c(-0.05,0.1), oob = squish)  + 
  coord_fixed(expand = F, ratio = 1) + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5), axis.title = element_blank(), legend.position = "top")
...
KMT.combined <- bind_rows(screen.KMT,KMT.followup) %>% mutate(gene_gRNA = paste(gene,gRNA, sep = "_"))
