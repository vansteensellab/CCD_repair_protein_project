...
# genes.all.NHEJ.MMEJ
all.same.summary <- bind_rows(all.mmej.matrix,all.nhej.matrix,mixed.matrix) %>% filter(abs(highest) > 0.03) %>% select(gene,gene.class) %>% left_join(gene_library) %>% ungroup() %>% dplyr::group_by(librar,gene.class) %>% dplyr::summarise(counts = n())
