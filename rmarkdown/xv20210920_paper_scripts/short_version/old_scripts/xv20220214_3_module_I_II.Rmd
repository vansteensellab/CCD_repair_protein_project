...
I <- module.summary %>% filter(gene %in% c(module_I_euchr)) %>% na.omit() %>% ungroup() %>% select(librar,gene)  %>% distinct() %>% dplyr::group_by(librar) %>% dplyr::summarise(counts = n())

#Number of proteins in module II (n =20 of 80, 25%)
II <- module.summary %>% filter(gene %in% c(module_II_polycomb)) %>% na.omit() %>% ungroup() %>% select(librar,gene)  %>% distinct() %>% dplyr::group_by(librar) %>% dplyr::summarise(counts = n())

#Number of proteins in module III (n =81 of 177, 46%)
III <- module.summary %>% filter(gene %in% c(module_III_trip_het)) %>% na.omit() %>% ungroup() %>% select(librar,gene)  %>% distinct() %>% dplyr::group_by(librar) %>% dplyr::summarise(counts = n())

```

# Figure 3B: gost analysis with module I
```{r}
...
```{r}
# For this we select
mmej_module_III_tripl_het_features <- tibble(feature = c("late_replicating","H3K9me3","H3K9me2","LMNB1"), module = "mmej_III_triple_het", gene.class = "all.mmej")
mmej_module_II_polycomb_features <- tibble(feature = c("EZH2","H3K27me3"), module = "mmej_II_polycomb", gene.class = "all.mmej")
mmej_module_I_euchr_features <- tibble(feature = c("H3K4me1","H3K27ac","H3K4me2","H3K4me3","H4K5acK8ac"), module = "mmej_I_euchromatin", gene.class = "all.mmej")
...
nhej_module_III <- tibble(feature =c("late_replicating","H3K9me3","H3K9me2","LMNB1"), module = "nhej_III_triple_het", gene.class = "all.nhej")
# Module summary
module_summary <- bind_rows(mmej_module_III_tripl_het_features,mmej_module_II_polycomb_features, mmej_module_I_euchr_features, nhej_module_I,nhej_module_III)
...
#Left_join data
module.summary <- selected.slopes %>% left_join(interaction.summary, by = "gene") %>% left_join(module_summary, by = c("feature")) %>% dplyr::group_by(module,gene) %>% dplyr::summarise(mean = mean(epistasis.final, na.rm = T)) %>% left_join(gene_library, by = "gene")
module_I_genes <- module.summary %>% filter(module == "mmej_I_euchromatin" & mean > 0.02)
module_II_genes <- module.summary %>% filter(module == "mmej_II_polycomb" & mean > 0.02)
module_III_genes <- module.summary %>% filter(module == "mmej_III_triple_het" & mean > 0.02)
module_nhej <- module.summary %>% filter(module == "nhej_I_euchromatin" & mean < -0.02)
module_nhej_III <- module.summary %>% filter(module == "nhej_III_triple_het" & mean < -0.02)
#
ggplot(module.summary %>% filter(grepl("nhej", module) & mean < -0.02) %>% na.omit() %>% ungroup() %>% select(librar,gene,module, gene.class)  %>% distinct() %>% group_by(module,gene.class) %>% summarise(counts = n())) + geom_col(aes(fct_relevel(module,"nhej_I_euchromatin"), counts,fill = gene.class)) + theme_bw() + coord_cartesian()

ggplot(module.summary %>% filter(grepl("mmej", module) & mean > 0.02) %>% na.omit() %>% ungroup() %>% select(librar,gene,module, gene.class)  %>% distinct() %>% group_by(module, gene.class) %>% summarise(counts = n())) + geom_col(aes(fct_relevel(module,"nhej_I_euchromatin"), counts, fill = gene.class)) + theme_bw() + coord_cartesian()



ggplot(module.summary  %>% filter(grepl("mmej", module) & mean > 0.02) %>% na.omit() %>% ungroup() %>% select(librar,gene,module)  %>% distinct() %>% dplyr::group_by(librar,module) %>% dplyr::summarise(counts = n())) + geom_col(aes(fct_relevel(module,c("nhej_I_euchromatin")), counts, fill = fct_relevel(librar,c("DNA_repair","both","Chromatin")))) + theme_bw() + coord_cartesian(expand = T) + scale_fill_manual(values = c("#56B3E6","grey40","#E69F03")) + theme(axis.title.x = element_blank(), axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + theme(legend.position = "top")
...
proteins.PIKK <- slope.protein.features.all %>% filter(gene %in% c("PARP3","PARP1","PARG")) %>% select(drug = gene, feature,intercept,slope,mean.fc,high,sign.presence,epistasis.sign,epistasis.final)

PAR.prots <- slope.protein.features.inhibitors.filt %>% bind_rows(proteins.PIKK) %>% filter(drug %in% c("PARP1","PARP3","PARG","olaparib","talazoparib","veliparib"))


# dcaast with selected
slope.prot.features.dcast.PAR <- PAR.prots %>% filter(feature %in% mmej_module_I_euchr_features$feature) %>% reshape2::dcast(drug ~ feature, value.var = "epistasis.final") %>% column_to_rownames(var = "drug")

#Heatmap
heatmap.slope.diff.PAR <- pheatmap(slope.prot.features.dcast.PAR, silent = F)
...
# Both sides
ggplot(PAR.prots) + 
  geom_tile(aes(fct_relevel(drug,c("PARP1","PARP3","PARG","olaparib","talazoparib","veliparib")),fct_relevel(feature, heatmap.chromatin.order), fill = epistasis.final)) +
...
slope.prot.features.scale.dcast.FA <- slope.protein.features.all %>% filter(gene %in% c("FANCL","FAAP100","FAAP24","FANCA","FANCB","FANCC","FANCE","FANCF","FANCG","FANCM","FANCD2","FANCI") & feature %in% module_II_polycomb_features$feature) %>% reshape2::dcast(gene ~ feature, value.var = "epistasis.final") %>% column_to_rownames(var = "gene")


ggplot(slope.protein.features.all %>% filter(gene %in% c("FAAP24","FANCM","FANCA","FANCG","FANCF","FANCC","FAAP100","FANCE","FANCL","FANCB","FANCI","FANCD2"))) + 
  geom_tile(aes(fct_relevel(gene,c("FAAP24","FANCM","FANCA","FANCG","FANCF","FANCC","FAAP100","FANCE","FANCL","FANCB","FANCI","FANCD2")), fct_relevel(feature,heatmap.chromatin.order), fill = epistasis.final)) +
