...
C- Volcano plot with all perturbations
...
log2.distance.ratio.all <- log2.ratio.screen.detail.all %>% filter(sample == "KO") %>% left_join(wt.set.ratio) %>% mutate(log2.ratio = log2ratio - wt.log2ratio)
...
ggplot(test.log2.dist %>% left_join(highlight.complex)) +
  ...
  theme_bw() + geom_text_repel(data = subset(test.log2.dist,mean.fc < -0.2 | gene %in% c("SUPT5H","SAP130")), aes(mean.fc,-log10(p.adj), label = gene), max.overlaps = Inf, box.padding = 0.5) + geom_vline(xintercept = 0, linetype = 2) + geom_hline(yintercept = -log10(0.001), linetype = 2) + ylab("-log10(FDR)") + xlab("log2(Average fold change NHEJ:MMEJ balance)") + theme(legend.position = "top") + scale_x_continuous(breaks = c(-2,-1,0,1)) + scale_color_manual( values = c("#ffa700","#ff4000","#f8f9ac"))
```
