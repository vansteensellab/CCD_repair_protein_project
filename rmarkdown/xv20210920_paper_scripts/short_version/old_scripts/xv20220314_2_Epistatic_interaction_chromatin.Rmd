...
#CCD analysis data: Take all different datasets
step2.epistasis <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/xv20220110_ratio_epistasis_2_step.rds")
#mean first
DDR.CCD.values.mean <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/xv20220314_DDR_CCD_mean.rds")
chromatin.CCD.values.mean <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/xv20220314_chromatin_CCD_mean.rds")
#Per replicate
DDR.CCD.values.pr <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/xv20220314_DDR_CCD_per_rep.rds")
chromatin.CCD.values.pr <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/xv20220314_Chromatin_CCD_per_rep.rds")
...
# Figure 4B: Matrix for hits (Only DDR proteins and only significant)
#Plot NHEJ proteins in one
```{r}
# Filter CCD table for significant rearrangements
DDR.CCD.values.pr.signif <- DDR.CCD.values.pr %>% filter(ks_test_sign == T)

#Make heatmap and dcast
DDR.CCD.values.dcast <- DDR.CCD.values.pr.signif %>% reshape2::dcast(gene ~ feature, value.var = "m_CCD_value") %>% column_to_rownames(var = "gene")
heatmap.DDR.CCD.values <- pheatmap(t(DDR.CCD.values.dcast), clustering_method = "ward.D2", cluster_rows = F)

DDR.CCD.gene.order <- rownames(DDR.CCD.values.dcast[heatmap.DDR.CCD.values$tree_col[["order"]],])

# heatmap
ggplot(DDR.CCD.values.pr.signif) + 
  geom_tile(aes(fct_relevel(gene,DDR.CCD.gene.order),fct_relevel(feature, heatmap.chromatin.order.slope.diff.hits), fill = m_CCD_value)) +
