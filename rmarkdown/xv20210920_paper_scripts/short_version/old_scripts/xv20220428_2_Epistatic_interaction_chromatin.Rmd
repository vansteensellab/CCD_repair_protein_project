...
ggplot(step2.epistasis %>% 
       ...
       aes(H3K36me3,mean.log2foldchange, color = 0.07)) + 
  geom_point(color = "black") +
  geom_smooth(method = "lm") +
  stat_cor(color = "black") +
  scale_color_gradient2(low = "#8c510a",mid = "#f5f5f5", high = "#01665e", limits = c(-0.1,0.15)) +
  theme_bw() + ylab("log2(FC MMEJ:NHEJ balance)") + theme(legend.position = "none")
```
...
```{r}
#Data frame for p_value
cluster.feature.test.median <- tibble(cluster_name = NA, feature = NA, p.value = NA, median = NA)
...
for(i in unique(significant_proteins_clust$cluster_name)) {
  ...
  control_data <- non.targetting.CCD_data %>% filter(feature == j) %>% pull(CCD_value)
  wil_t <- wilcox.test(cluster_data,control_data) %>% tidy()
  cluster.feature.test.median <- cluster.feature.test.median %>% add_row(cluster_name = i, feature = j, p.value = wil_t %>% pull(p.value), median = median(cluster_data, na.rm = T))
  }
}

#Apply fdr correction
cluster.feature.test.adj.median <- cluster.feature.test.median %>% mutate(p.adj = p.adjust(p.value))

# Features that drive clustering
ggplot(cluster.feature.test.adj.median %>% na.omit()) + geom_col(aes(fct_relevel(feature, heatmap.chromatin.order),median, fill = p.adj < 0.01)) + facet_wrap(~ fct_relevel(cluster_name, c("N-synergy","M-synergy_1","M-synergy_2","M-synergy_3")), nrow = 1) + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + scale_fill_manual(values = c("grey60","orange")) + 
  geom_quasirandom(data = significant_proteins_clust, aes(fct_relevel(feature, heatmap.chromatin.order), CCD_value), size = 0.5, alpha = 0.5)

```

```{r}
#Test wilcox test if
significant_proteins_clust <- significant_proteins %>% left_join(cluster_tibble) %>% mutate(pathway = case_when(cluster_name == "NHEJ" ~ "N-synergy", T ~ "M-synergy"))

#Data frame for p_value
cluster.feature.test_p <- tibble(pathway = NA, feature = NA, p.value = NA, mean = NA, sd = NA)

for(i in unique(significant_proteins_clust$pathway)) {
  for(j in unique(significant_proteins_clust$feature)) {
  cluster_data <- significant_proteins_clust %>% filter(pathway == i, feature == j) %>% pull(CCD_value)
  control_data <- non.targetting.CCD_data %>% filter(feature == j) %>% pull(CCD_value)
  wil_t <- wilcox.test(cluster_data,control_data) %>% tidy()
  cluster.feature.test_p <- cluster.feature.test_p %>% add_row(pathway = i, feature = j, p.value = wil_t %>% pull(p.value), mean = mean(cluster_data, na.rm = T), sd = sd(cluster_data, na.rm = T))
  }
}

#Apply fdr correction
cluster.feature.test.adj_p <- cluster.feature.test_p %>% mutate(p.adj = p.adjust(p.value))

# Features that drive clustering
ggplot(cluster.feature.test.adj_p %>% na.omit()) + geom_col(aes(mean, fct_relevel(feature, heatmap.chromatin.order), fill = pathway, alpha = p.adj < 0.05), position = "dodge2") + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) + scale_fill_manual(values = c( "#8c510a","#01665e")) + scale_alpha_manual(values = c(0.6,1))
...
ggplot(FA.proteins.CCD) + 
  ...
  scale_fill_gradient2(low = "#8c510a" ,mid = "#f5f5f5", high = "#01665e", breaks = c(-0.1,-0.05,0), limits = c(-0.1,0))  + 
...
#Correlation plots
PARP_inh_dcast <- PARP.proteins.dcast %>% bind_rows(PARP.inhibitors.dcast) %>% t()

ggplot(as_data_frame(PARP_inh_dcast), aes(PARP1, olaparib)) + geom_point() + stat_cor() + geom_smooth(, method = "lm") + theme_bw() + geom_abline(linetype = 2)

ggplot(as_data_frame(PARP_inh_dcast), aes(PARP1, talazoparib)) + geom_point() + stat_cor() + geom_smooth(, method = "lm") + theme_bw() + geom_abline(linetype = 2)
...
ggplot(DNAPK.perturbations.dcast,aes(PRKDC,DNAPKi)) + geom_point() + stat_cor() + geom_smooth( method = "lm", color = "black") + theme_bw() + geom_text_repel(aes(label = feature))
