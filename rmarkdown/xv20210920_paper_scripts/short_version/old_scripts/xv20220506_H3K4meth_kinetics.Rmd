...
```{r}
#Antibodies for paper
antibodies.hist.mod <- c("H3K4me1","H3K4me2","H3K4me3","H3","Input")

#Load data set from E1057
E1057_chip_kinetics <- readRDS("/DATA/usr/x.vergara/XV_ChIPIndels/xv20200702_HistMod_R2/data/xv20200827_E1057_histmod_chip.rds") %>% filter(spikein == FALSE)
E1057_K4_selection <- E1057_chip_kinetics %>% dplyr::select(n.exp,antibody,timepoint,barcode,norm.pull.down,clone,MMEJscore,indelreads,freqWT, freqMMEJ, freqNHEJ) %>% filter(antibody %in% antibodies.hist.mod)

#Load data set from E900
load("/DATA/projects/DSBrepair/data/R/XV20200122_E900_HistmodTest.RData")
E900_chip_kinetics <- E900.tib %>% filter(spikein == FALSE) %>% mutate(n.exp = case_when(n.exp == "E862" ~ "1.1", n.exp == "E893" ~ "1.2"))
E900_K4_selection <- E900_chip_kinetics %>% dplyr::select(n.exp,antibody,timepoint,barcode,norm.pull.down,clone,MMEJscore,indelreads,freqWT = pct_wt, freqMMEJ = pct_del_7, freqNHEJ = pct_ins_1) %>% filter(antibody %in% antibodies.hist.mod)

# bind both
K4_kinetics_chip_data <- bind_rows(E1057_K4_selection,E900_K4_selection)
#Input data 

# n per barcode 
n_per_barcode <- K4_kinetics_chip_data %>% na.omit() %>% distinct() %>% group_by(antibody, barcode, timepoint) %>% dplyr::summarise(n = n())
clones_per_replicate <- K4_kinetics_chip_data %>% na.omit() %>% select(n.exp, clone) %>% distinct() %>% mutate(n = 1) %>% dcast(clone ~ n.exp)
antibodies_per_replicate <- K4_kinetics_chip_data %>% na.omit() %>% select(n.exp, antibody) %>% distinct() %>% mutate(n = 1) %>% dcast(antibody ~ n.exp)

#Clone 5 chromatin
clone5_z.score_chrom_tib <- readRDS('/DATA/projects/DSBrepair/data/R/cl20201026_ChIP_zscore_selection.RDS')

```
# Check for 1.1 and 1.2
```{r}
correlation_same_replicate <- K4_kinetics_chip_data %>% na.omit() %>% select(antibody, barcode,timepoint, clone, n.exp, norm.pull.down) %>% distinct() %>% dcast(antibody + barcode + timepoint + clone ~ n.exp, value.var = "norm.pull.down", , fun.aggregate = mean)

example.barcode <- K4_kinetics_chip_data %>% filter(antibody == "H3" & barcode == "AAAAGCTCACTAGCAG" & timepoint == "120h" & clone == "RSTP2_1")

```

# Calculate averages per n.exp first
```{r}
K4_kinetics_per_n_exp <- K4_kinetics_chip_data %>% select(-indelreads) %>% group_by(n.exp, antibody,timepoint, barcode,clone) %>% summarise_all(.funs = "mean")

#Replicates per barcode
replicates_barcode <- K4_kinetics_per_n_exp %>% group_by(antibody, timepoint, barcode, clone) %>% dplyr::summarise(count = n())
