...
ggplot() + 
  geom_segment(data = linerange_plot.dt, aes(x = gene1.x, y = gene1.y.y, xend = gene2.x, yend = gene2.y.y), alpha = 0.25) +
  geom_point(data = umap.hits.plot, aes(A,B, color = cluster_name, size = gene %in% FA_prots)) + 
  geom_text_repel(data = umap.hits.plot %>% subset(gene %in% int_prots_names), aes(A,B, label = gene)) +
  theme_minimal() + 
  coord_fixed()
