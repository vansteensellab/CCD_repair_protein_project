...
cluster_umap <- cluster_tibble %>% separate(cluster_name, into = c("pathway", "cluster"), sep = "_")
umap.hits.plot <- umap.hits$layout %>% as.data.frame() %>% rownames_to_column(var = "gene") %>% left_join(cluster_umap)
umap.hits.plot[,umap.hits.plot$gene == "CHAF1A"]$cluster <- 2
...
interacting.prots <- filter(dist.int.partners, interaction == T)
...
linerange_plot.dt <- interacting.prots %>% left_join(gene1.coord) %>% left_join(gene2.coord) %>% mutate(cluster.int = case_when(gene1.cluster == gene2.cluster ~ "intracluster", T ~ "intercluster"), similarity = case_when(cos.distance < -0.5 ~ "opposed", cos.distance > -0.5 & cos.distance < 0.5  ~ "complementary", cos.distance > 0.5 ~ "similar")) %>% mutate(al_val = 1^cos.distance)
hl_interactions <- filter(linerange_plot.dt, gene1 == "BLM" & gene2 == "RMI2" |
                            gene1 == "NSMCE1" & gene2 == "SMC5" |
                            grepl("FA",gene1) & grepl("FA", gene2) |
                            gene1 == "BRCC3" & gene2 == "UIMC1")

#Plot with linerange (latest)
ggplot() + 
...
geom_segment(data = hl_interactions, aes(x = gene1.x, y = gene1.y.y, xend = gene2.x, yend = gene2.y.y, alpha = exp(cos.distance), size = exp(cos.distance)), color = "black") +
theme_bw() + scale_alpha(range = c(0.3,1)) + scale_size(range = c(0.1,1))  + geom_point(data = umap.hits.plot, aes(A,B, color = pathway, shape = cluster), size = 2) + 
...
ggplot() + 
  geom_segment(data = linerange_plot.dt, aes(x = gene1.x, y = gene1.y.y, xend = gene2.x, yend = gene2.y.y, alpha = exp(cos.distance))) +
  geom_segment(data = linerange_plot.dt, aes(x = gene1.x, y = gene1.y.y, xend = gene2.x, yend = gene2.y.y, alpha = exp(cos.distance))) +
  geom_point(data = umap.hits.plot, aes(A,B, color = pathway, shape = cluster), size = 4) + 
  geom_text_repel(data = subset(umap.hits.plot, gene %in% highlighted_prots), aes(A,B,label = gene)) +
  theme_bw() + scale_shape_manual(values = c(16,17,18))



ggplot() + 
+     geom_segment(data = linerange_plot.dt, aes(x = gene1.x, y = gene1.y.y, xend = gene2.x, yend = gene2.y.y, alpha = exp(cos.distance), size = exp(cos.distance)), color = "grey80") +
+     theme_bw() + scale_alpha(range = c(0.3,1)) + scale_size(range = c(0.1,1))  + geom_point(data = umap.hits.plot, aes(A,B, color = cluster_name)) + 
+     geom_text_repel(data = subset(umap.hits.plot, gene %in% highlighted_prots), aes(A,B,label = gene)) +
