...
ggplot() +
     stat_density2d(data = umap.hits.plot, geom = "polygon", aes(A,B, fill = cluster, alpha = ..level..), bins = 8) +
     geom_segment(data = all_cliques, aes(x = gene1.x, y = gene1.y.y, xend = gene2.x, yend = gene2.y.y, colour = as.character(clique)), size = 0.5) +
     geom_segment(data = hl_interactions, aes(x = gene1.x, y = gene1.y.y, xend = gene2.x, yend = gene2.y.y), size = 0.5, linetype = 2, colour = "black") +
     theme_bw() + geom_point(data = umap.hits.plot, aes(A,B, color = pathway, shape = cluster), size = 4) + 
     theme(panel.grid = element_blank()) + 
     scale_shape_manual(values = c(16,17,18)) + scale_color_manual(values = c("grey10","grey90","grey50","grey80","grey20","#8c510a", "#01665e")) + 
     scale_fill_manual(values = c("#FAC075","#9E75B2","#F088B6")) + scale_alpha(range = c(0.15,0.3)) +
     geom_text_repel(data = subset(umap.hits.plot, gene %in% genes.cliques), aes(A,B,label = gene), force = 40) +
     ylab("UMAP2") + xlab("UMAP1") + ylim(c(-5,5)) + xlim(-3,3) 

#Clique 1 
ggplot() +
     stat_density2d(data = umap.hits.plot, geom = "polygon", aes(A,B, fill = cluster, alpha = ..level..), bins = 8) +
     geom_segment(data = all_cliques %>% filter(clique == 1), aes(x = gene1.x, y = gene1.y.y, xend = gene2.x, yend = gene2.y.y), colour = "black", size = 1) +
     theme_bw() + theme(panel.grid = element_blank()) + 
     scale_fill_manual(values = c("#FAC075","#9E75B2","#F088B6")) + scale_alpha(range = c(0.3,0.6)) +
     ylab("UMAP2") + xlab("UMAP1") + ylim(c(-5,5)) + xlim(-3,3) 


 ggplot() +
     stat_density2d(data = umap.hits.plot, geom = "polygon", aes(A,B, fill = cluster, alpha = ..level..), bins = 8) +
     geom_segment(data = all_cliques, aes(x = gene1.x, y = gene1.y.y, xend = gene2.x, yend = gene2.y.y, fill = as.character(clique))) +
     theme_bw() + geom_point(data = umap.hits.plot, aes(A,B, color = pathway, shape = cluster), size = 4) + 
     geom_text_repel(data = subset(umap.hits.plot, gene %in% highlighted_prots), aes(A,B,label = gene), force = 40) +
     theme(panel.grid = element_blank()) + 
     scale_shape_manual(values = c(16,17,18)) + scale_color_manual(values = c("#8c510a", "#01665e")) + 
     scale_fill_manual(values = c("#FAC075","#9E75B2","#F088B6")) + scale_alpha(range = c(0.15,0.3)) +
     ylab("UMAP2") + xlab("UMAP1") + ylim(c(-5,5)) + xlim(-3,3) 
...
ggplot() + 
stat_density2d(data = umap.hits.plot, geom = "polygon", aes(A,B, fill = cluster, alpha = ..level..), bins = 8) +
geom_segment(data = all_cliques, aes(x = gene1.x, y = gene1.y.y, xend = gene2.x, yend = gene2.y.y), color = "#e5000c") +
theme_bw() + geom_point(data = umap.hits.plot, aes(A,B, color = pathway, shape = cluster), size = 4) + 
geom_text_repel(data = subset(umap.hits.plot, gene %in% highlighted_prots), aes(A,B,label = gene)) +
theme(panel.grid = element_blank()) + scale_shape_manual(values = c(16,17,18)) + scale_color_manual(values = c("#8c510a", "#01665e")) +
ylab("UMAP2") + xlab("UMAP1") + ylim(-10,10)


#Pot maximum cliques only
ggplot() + 
geom_segment(data = linerange_plot.dt, aes(x = gene1.x, y = gene1.y.y, xend = gene2.x, yend = gene2.y.y, alpha = exp(cos.distance), size = exp(cos.distance)), color = "grey80") +
geom_segment(data = hl_interactions, aes(x = gene1.x, y = gene1.y.y, xend = gene2.x, yend = gene2.y.y, alpha = exp(cos.distance), size = exp(cos.distance)), color = "#e5000c") +
theme_bw() + scale_alpha(range = c(0.3,1)) + scale_size(range = c(0.1,1))  + geom_point(data = umap.hits.plot, aes(A,B, color = pathway, shape = cluster), size = 4) + 
geom_text_repel(data = subset(umap.hits.plot, gene %in% highlighted_prots), aes(A,B,label = gene)) +
theme_bw() + theme(panel.grid = element_blank()) + scale_shape_manual(values = c(16,17,18)) + scale_color_manual(values = c("#8c510a", "#01665e")) +
ylab("UMAP2") + xlab("UMAP1")

##### TMP: plot all string links
# Create data frame to plot interactions between hits
gene1.coord <- umap.hits.plot %>% 
  select(gene1 = gene, gene1.x = A, gene1.y = B, gene1.cluster = cluster) #Select umap coordinates for gene1
gene2.coord <- umap.hits.plot %>% 
  select(gene2 = gene, gene2.x = A, gene2.y = B, gene2.cluster = cluster) #Select umap coordinates for gene2
linerange_plot_string.dt <- string_interaction_pairs %>% 
  left_join(gene1.coord) %>% 
  left_join(gene2.coord) %>% mutate(distan = sqrt((gene1.x - gene2.x)^2 + (gene1.y.y - gene2.y.y)^2)) #Merge gene1 and gene2 coordinates
hl_interactions <- filter(linerange_plot.dt, gene1 == "BLM" & gene2 == "RMI2" |
                            gene1 == "NSMCE1" & gene2 == "SMC5" |
                            grepl("FA",gene1) & grepl("FA", gene2) |
                            gene1 == "BRCC3" & gene2 == "UIMC1") #Highlight some interactions that are mentioned in the text

#Find cliques
#Find cliques
igraph_interaction_string <- graph_from_data_frame(string_interaction_pairs)
maximum_cliques_data_string <- max_cliques(igraph_interaction_string, min = 10) 
maximum_cliques_string_tib <- do.call("rbind",maximum_cliques_data_string)

#Plot with umap linerange (latest)
ggplot() + 
geom_segment(data = linerange_plot_string.dt %>% filter(distan < 1.5), aes(x = gene1.x, y = gene1.y.y, xend = gene2.x, yend = gene2.y.y)) +
geom_segment(data = hl_interactions, aes(x = gene1.x, y = gene1.y.y, xend = gene2.x, yend = gene2.y.y, alpha = exp(cos.distance), size = exp(cos.distance)), color = "#e5000c") +
theme_bw() + scale_alpha(range = c(0.3,1)) + scale_size(range = c(0.1,1))  + geom_point(data = umap.hits.plot, aes(A,B, color = pathway, shape = cluster), size = 4) + 
geom_text_repel(data = subset(umap.hits.plot, gene %in% highlighted_prots), aes(A,B,label = gene)) +
theme_bw() + theme(panel.grid = element_blank()) + scale_shape_manual(values = c(16,17,18)) + scale_color_manual(values = c("#8c510a", "#01665e")) +
