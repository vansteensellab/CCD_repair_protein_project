...
```{r}
cosine.highlights.genes <- slope.protein.features.all %>% filter(gene %in% c("BRCC3","UIMC1","BRCA1","SMC5","NSMCE1","NSMCE3","NSMCE2","NSMCE4","SMC6","ATM", "CHEK2","PRKDC")) %>% na.omit()
...
#highlight order
highlight.order <- c("ATM","ATMi","CHEK2","SMC6","NSMCE3","NSMCE1","SMC5","PRKDC","DNAPKi","UIMC1","BRCC3","BRCA1")
...
ggplot(cosine.d.highlingts %>% filter(Var1 %in% c("ATM","ATMi","CHEK2") & Var2 %in% c("ATM","ATMi","CHEK2"))) + 
  geom_tile(aes(fct_relevel(Var1,highlight.order), fct_relevel(Var2,highlight.order), fill = value)) + theme_bw() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), legend.position = "top") + coord_fixed(expand = F,ratio = 1) + scale_fill_gradient2(low = "#af8dc3", mid = "#f7f7f7", high = "#7fbf7b", limits = c(-1,1))  + geom_text(aes(fct_relevel(Var1,highlight.order), fct_relevel(Var2,highlight.order),label = round(value, digits = 2))) + theme(axis.title = element_blank())
...
#D
ggplot(cosine.d.highlingts %>% filter(Var1 %in% c("BRCC3","UIMC1","BRCA1") & Var2 %in% c("BRCC3","UIMC1","BRCA1"))) + 
  geom_tile(aes(fct_relevel(Var1,highlight.order), fct_relevel(Var2,highlight.order), fill = value)) + theme_bw() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), legend.position = "top") + coord_fixed(expand = F,ratio = 1) + scale_fill_gradient2(low = "#af8dc3", mid = "#f7f7f7", high = "#7fbf7b", limits = c(-1,1)) + geom_text(aes(fct_relevel(Var1,highlight.order), fct_relevel(Var2,highlight.order),label = round(value, digits = 2))) + theme(axis.title = element_blank())

ggplot(cosine.d.highlingts %>% filter(Var1 %in% c("BRCC3","UIMC1") & Var2 %in% c("UIMC1","BRCA1"))) + 
  geom_tile(aes(fct_relevel(Var1,highlight.order), fct_relevel(Var2,highlight.order), fill = value)) + theme_bw() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1), legend.position = "top") + coord_fixed(expand = F,ratio = 1) + scale_fill_gradient2(low = "#af8dc3", mid = "#f7f7f7", high = "#7fbf7b", limits = c(-1,1)) + geom_text(aes(fct_relevel(Var1,highlight.order), fct_relevel(Var2,highlight.order),label = round(value, digits = 2))) + theme(axis.title = element_blank())
