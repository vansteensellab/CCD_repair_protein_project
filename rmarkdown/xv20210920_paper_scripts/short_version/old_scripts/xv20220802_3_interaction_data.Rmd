...
#Unique interactions
interacting_pairs <- ddr.ccd.physical.biogrid.db %>% select(gene1,gene2) %>% distinct() %>% dim()
```
...
all_cliques <- tibble(gene = c("ATM","FANCD2","MDC1","TOPBP1","FANCM","FANCF","FANCG","FANCD2","RMI2","BLM","FANCD2","FANCM"), clique = c(rep(1,4), rep(2,4),rep(3,4)), type = c(rep("ATM",4), rep("FA",4), rep("mixed",4))) %>% left_join(umap.hits.plot)
#Plot with umap with 4-node cliques
ggplot() +
     geom_polygon(data = all_cliques, aes(x = A, y = B, group = clique, fill = type), alpha = 0.4) +
     stat_density2d(data = umap.hits.plot, geom = "polygon", aes(A,B, fill = cluster_b, alpha = ..level..), bins = 8) + xlim(-2.5,4) + ylim(-5.5,4) +
