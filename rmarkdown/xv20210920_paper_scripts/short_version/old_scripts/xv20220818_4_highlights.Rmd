...
distribution_tumor <- SV_MH_del_data_tumors %>% 
  ...
  left_join(total_mut_tumor, by = c("tum_id" = "sample")) %>%
  reshape2::dcast(type + tum_id + mut_total ~ LAD_status, value.var = "mut_count", fill = 0) %>%
  mutate(freq_ciLAD = ciLAD/(ciLAD + cLAD),
         t_freq_ciLAD = ciLAD/mut_total,
         t_freq_cLAD = cLAD/mut_total)
