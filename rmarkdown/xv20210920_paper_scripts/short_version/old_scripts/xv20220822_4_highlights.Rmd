...
#contingenty tables
contingency_tables <- distribution_tumor_indel %>% dplyr::group_by(tumor_type, CLASS) %>% dplyr::summarise(tot_ciLAD = sum(ciLAD, na.rm = T), tot_cLAD = sum(cLAD,na.rm = T))
larde_del_ct <- LAD.data.mut %>% reshape2::dcast(type ~ LAD_status, value.var = "mutation_count") %>% column_to_rownames(var = "type")
...
```{r}
#Plot A in figure 4
ggplot(distribution_tumor_indel) + geom_col(aes(CLASS,log2_ratio, fill = CLASS)) + facet_wrap(~ fct_relevel(tumor_type, c("HHNSC_indel","BRCA2mut_indel"))) + theme_bw() + scale_fill_manual(values = c("#8c510a","#01665e"))

#Plot B in figure B
ggplot(distribution_tumor) + geom_col(aes("large_deletion",log2_ratio, fill = "large_deletion")) + facet_wrap(~ fct_relevel(type, c("sporadic_HPVneg","BRCA2mut"))) + theme_bw() + scale_fill_manual(values ="#01665e") + geom_hline(yintercept = 0, linetype = 2)


#Plot A in figure 4
ggplot(distribution_tumor_indel) + geom_col(aes(fct_relevel(tumor_type, c("HHNSC_indel","BRCA2mut_indel")),log2_ratio, fill = tumor_type)) + facet_wrap(~ CLASS) + theme_bw() + scale_fill_manual(values = c("#74A1CE","grey80")) + theme(legend.position = "top") + xlab("Tumor genetic background") + ylab("log2 Heterochromatin/Euchromatin") + coord_cartesian(ylim = c(-0.25,1.25)) + geom_hline(yintercept = 0, linetype = 2)

#Plot B in figure B
ggplot(distribution_tumor) + geom_col(aes(fct_relevel(type, c("sporadic_HPVneg","BRCA2mut")),log2_ratio, fill = type)) + theme_bw()  + scale_fill_manual(values = c("#74A1CE","grey80")) + geom_hline(yintercept = 0, linetype = 2) + facet_wrap(~ c("Large_deletions")) + theme(legend.position = "top") + xlab("Tumor genetic background") + ylab("log2 Heterochromatin/Euchromatin") + coord_cartesian(ylim = c(-0.25,1.25)) 

#chi squared test for all these cases
#cNHEJ
cNHEJ_fisher <- contingency_tables %>% filter(CLASS == "cNHEJ") %>% select(-CLASS) %>% column_to_rownames(var = "tumor_type") %>% fisher.test(alternative = "less") %>% tidy() %>% mutate(CLASS = "NHEJ")
#MMEJ
MMEJ_fisher <- contingency_tables %>% filter(CLASS == "MMEJ") %>% select(-CLASS) %>% column_to_rownames(var = "tumor_type") %>% fisher.test( alternative = "greater") %>% tidy() %>% mutate(CLASS = "MMEJ")
#large_del
large_del_fisher <- larde_del_ct  %>% fisher.test(alternative = "greater") %>% tidy() %>% mutate(CLASS = "large_del_MH") %>% mutate(p.adj = p.adjust(p.value))

#test_correction
corr_fisher <- bind_rows(cNHEJ_fisher, MMEJ_fisher, large_del_fisher)  %>% mutate(p.adj = p.adjust(p.value))
