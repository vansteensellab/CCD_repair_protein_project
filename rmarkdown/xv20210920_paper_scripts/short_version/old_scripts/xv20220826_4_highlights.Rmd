...
SV_data_LAD <- SV_MH_del_data_tumors %>% 
  ...
  left_join(sample_n, by = "type") %>% rowwise() %>% mutate(mean_mutation_count = mutation_count) %>% #Mean numbers
  ungroup() #Ungroup
...
dplyr::group_by(tumor_type, LAD_status) %>%
  dplyr::summarise(freq_MMEJ = sum(MMEJ,na.rm = T)/sum(total_mutation, na.rm = T),
                   freq_cNHEJ = sum(cNHEJ,na.rm = T)/sum(total_mutation, na.rm = T), 
                   log2_ratio = log2(sum(MMEJ, na.rm = T)/sum(cNHEJ, na.rm = T)))

distribution_tumor_ind_indel_pathway <- indel_data_tumors %>% 
  filter(ciLAD == 1 | cLAD == 1) %>%
  mutate(LAD_status = case_when(ciLAD == 1 ~ "ciLAD",
                                cLAD == 1 ~ "cLAD")) %>%
  dplyr::group_by(tumor_type, tumor_id, LAD_status, CLASS) %>% dplyr::summarise(mut_count = n(), .groups = "drop") %>% 
  reshape2::dcast(tumor_type + tumor_id + LAD_status ~ CLASS, value.var = "mut_count", fill = 0) %>%
  dplyr::mutate(log2_ratio = log2((MMEJ)/(cNHEJ)))

#Check the deltas
deltas_tumor_indels <- distribution_tumor_ind_indel_pathway %>% reshape2::dcast(tumor_id + tumor_type ~ LAD_status) %>% mutate(diff_ratio = cLAD - ciLAD) 
...
#cNHEJ
BRCA2_fisher <- contingency_tables %>% ungroup() %>% filter(tumor_type == "BRCA2mut_indel") %>% select(-tumor_type) %>% column_to_rownames(var = "CLASS") %>% fisher.test() %>% tidy()
#MMEJ
HNSC_fisher <- contingency_tables %>% ungroup() %>% filter(tumor_type == "HHNSC_indel") %>% select(-tumor_type) %>% column_to_rownames(var = "CLASS") %>% fisher.test() %>% tidy()

#test_correction
corr_fisher <- bind_rows(BRCA2_fisher, HNSC_fisher)  %>% mutate(p.adj = p.adjust(p.value))

```

#Figure S8: to be copied
```{r}
# Data for plotting
