...
DEF_CCD_old<- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230915_CCD_analysis_summary_DEF_all_viable_3reps.rds") %>% 
  left_join(proteins_gRNA) %>% 
  mutate(chrom_feature = str_extract(pattern = "(H3|l|L).*", feature)) %>%
  select(gene, fdr, chrom_feature, CCD_score,global_diff, global_p_adj,gRNA, cell_line, pathway)
...
CI_95 <-  quantile(mean_per_iteration$mean_cosdist, 0.95)
...
```{r}
#Select random pairs from population
DEF_K562_CCDs <- filtered_CCDs %>% select(-PRO, -RPE1) %>% na.omit()
