...
pdf("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/figure_3_CCD_other_cell_lines/xv20231013_CCD_cosine_similarity_DEF.pdf", width = 5, height = 3)
...
pdf("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/figure_3_CCD_other_cell_lines/xv20231013_CCD_cosine_similarity_PRO.pdf", width = 5, height = 3)
...
pdf("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/figure_3_CCD_other_cell_lines/xv20231013_CCD_cosine_similarity_RPE1.pdf", width = 5, height = 3)
...
#Sampling loop to select random samples of equal size as interacting pairs
  ...
  random_sample_cosine_values <- map_dfr(c(1:1000), function(i){
   set.seed(i)
    K562_genes <- K562_CCD_all %>% pull(gene) %>% unique() %>% sample(18)
    DEF_genes <- DEF_K562_dcast %>% filter(!gene %in% proteins_gRNA$gene) %>% pull(gene) %>% unique()
...
#Sampling loop to select random samples of equal size as interacting pairs
  ...
  random_sample_cosine_values <- map_dfr(c(1:1000), function(i){
   set.seed(i)
    ...
    K562_CCDs_random <- K562_CCD_all %>% filter(chrom_feature %in% RPE1_chrom_features & gene %in% K562_genes) %>% select(chrom_feature, K562 = CCD_synergy_score, K562_gene = gene)
    K562_CCDs_random_cos <- K562_CCDs_random %>% left_join(K562_PRO_table, by = "K562_gene") %>% left_join(PRO_data, by = c("PRO_gene","chrom_feature")) %>% ungroup() %>% dplyr::group_by(PRO_gene,K562_gene) %>% dplyr::summarise(cos_dist = cosine(K562, PRO)) %>% mutate(seed = i)
  })
...
```{r,fig.width=2.75,fig.height=2.5}
#Plot this figure
pdf("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/figure_3_CCD_other_cell_lines/xv20231013_PRO_CCD_cosine_distribution.pdf", width = 2.75, height = 2.5)
ggplot(summarise.mean.sd_sample) + 
  geom_ribbon(aes(round_x, ymax = avg_y + sd_y, ymin = avg_y - sd_y), alpha = 0.2, fill = "grey60") +
  geom_line(aes(round_x,avg_y), color = "grey60", linetype = 2) + 
...
#Calculate empirical test
#Real mean
real_median <- PRO_K562_distances_all %>% na.omit() %>% pull(cosine_dist) %>% mean()
mean_cosine_similarity <- random_sample_cosine_values %>% 
  dplyr::group_by(seed) %>%
  dplyr::summarise(mean_cos = mean(cos_dist))
...
```
#Compute distribution of cosine similarities
#Cosine similarity distribution in RPE1 cells
```{r, message = F, warning = F}
#Sampling loop to select random samples of equal size as interacting pairs

  random_sample_cosine_values <- map_dfr(c(1:10), function(i){
   set.seed(i)
    K562_genes <- K562_CCD_all %>% filter(!gene %in% proteins_gRNA$gene) %>% pull(gene) %>% unique() %>% sample(8)
    RPE1_genes <- RPE1_K562_dcast %>% pull(gene) %>% unique()
    K562_RPE1_table <- tibble(K562_gene = K562_genes, RPE1_gene = RPE1_genes)
    RPE1_data <- RPE1_K562_dcast %>% select(RPE1, chrom_feature, RPE1_gene = gene)
    K562_CCDs_random <- K562_CCD_all %>% filter(chrom_feature %in% RPE1_chrom_features & gene %in% K562_genes) %>% select(chrom_feature, K562 = CCD_synergy_score, K562_gene = gene)
    K562_CCDs_random_cos <- K562_CCDs_random %>% left_join(K562_RPE1_table, by = "K562_gene") %>% left_join(RPE1_data, by = c("RPE1_gene","chrom_feature")) %>% ungroup() %>% dplyr::group_by(RPE1_gene,K562_gene) %>% dplyr::summarise(cos_dist = cosine(K562, RPE1)) %>% mutate(seed = i)
  })
...
#Calculate real interaction density data
real_density_plot <- density(RPE1_K562_distances_all %>% na.omit() %>% pull(cosine_dist), from = -1, to = 1) %>% tidy()
```
...
```{r,fig.width=2.75,fig.height=2.5}
#Plot this figure
pdf("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/figure_3_CCD_other_cell_lines/xv20231013_RPE1_CCD_cosine_distribution.pdf", width = 2.75, height = 2.5)
ggplot(summarise.mean.sd_sample) + 
  geom_ribbon(aes(round_x, ymax = avg_y + sd_y, ymin = avg_y - sd_y), alpha = 0.2, fill = "grey60") +
  geom_line(aes(round_x,avg_y), color = "grey60", linetype = 2) + 
  geom_line(data = real_density_plot, aes(x,y), color = "#95c482", size = 1.25) +
