...
```
#Figure 3A: Volcano plot with global effects
```{r, fig.height=2.25,fig.width=3}
volcano_plot_dt <- cell_lines_CCD %>%
  filter(!cell_line %in%  c("RPE1","K562")) %>%
  select(gene,cell_line,global_diff,global_p_adj) %>%
  distinct()
#Pro data
pdf("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/figure_3_CCD_other_cell_lines/xv20231020_Fig3A_PRO_global.pdf", height = 2.25, width = 3)
ggplot(volcano_plot_dt %>% filter(cell_line == "PRO")) +
  geom_point(aes(global_diff,-log10(global_p_adj),
                 color = global_p_adj < 0.01)) +
  geom_text_repel(data = volcano_plot_dt %>%
                    filter(cell_line == "PRO" & gene %in% c("RBBP8","ATM","RAD50","BRCA2","POLL","FANCM")),
                  aes(global_diff,-log10(global_p_adj),
                      label = gene)) +
  theme_bw() +
  ...
  ylab("-log10FDR") + xlab("Global ∆log2MMEJ:NHEJ") +
  scale_color_manual(values = c("grey70", "#95c482"))
dev.off()
```
...
```{r}
#Pro data
pdf("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/figure_3_CCD_other_cell_lines/xv20231020_Fig3A_DEF_global.pdf", height = 2.25, width = 3)
ggplot(volcano_plot_dt %>% filter(cell_line == "DEF")) +
  geom_point(aes(global_diff,-log10(global_p_adj),
                 color = global_p_adj < 0.01)) +
  geom_text_repel(data = volcano_plot_dt %>%
                    filter(cell_line == "DEF" & gene %in% c("RBBP8","ATM","RAD50","BRCA2","POLL","FANCM")),
                  aes(global_diff,-log10(global_p_adj),
                      label = gene)) +
  theme_bw() +
  theme(panel.grid = element_blank(),
  ...
  geom_hline(yintercept = -log10(0.01), linetype = 2) +
  ylab("-log10FDR") + xlab("Global ∆log2MMEJ:NHEJ") +
  scale_color_manual(values = c("grey70", "#9f86bf"))
dev.off()
