...
write.table(fig4a, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig4a.txt", row.names = F)
```
...
```{r Fig_3_B, warning=FALSE, message=FALSE, fig.width = 3, fig.height=2}
...
### Data processing
```{r Fig_3_C, warning=FALSE, message=FALSE}
...
### Plotting script (Figure 4C)

```{r Fig_3D, warning=FALSE, message=FALSE}
...
### Data ploting
#### Figure 3G: ATM clique I
```{r Fig_3_D, warning=FALSE, message=FALSE, fig.width=2, fig.height=3}
#Plot diferent
...
#Calculate means (for bar)
mean_value_cellline <- clique1_data %>% 
  select(cell_line, value) %>% 
  distinct() %>%
  dplyr::group_by(cell_line) %>%
  dplyr::summarise(mean_value = mean(value)) %>%
  mutate(symbol_meaning = "bar_height")

# Source data
fig4g <- clique1_data %>% 
  select(cell_line, value) %>% 
  distinct() %>%
  mutate(symbol_meaning = "datapoint") %>%
  bind_rows(mean_value_cellline) %>%
  select(x_celltype = cell_line, y_similarity = value, symbol_meaning) %>%
  mutate(figure = "Figure_4G")

write.table(fig4g, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig4g.txt", row.names = F)

```


#### Figure 3H: FA clique I
...
#Calculate means (for bar)
mean_cell_clique3 <- clique3_data %>% 
  select(cell_line, value) %>% 
  distinct() %>%
  dplyr::group_by(cell_line) %>%
  dplyr::summarise(mean_value = mean(value)) %>%
...
fig4h <- clique3_data %>% 
  ...
  distinct() %>%
  mutate(symbol_meaning = "datapoint") %>%
  bind_rows(mean_cell_clique3) %>%
  select(x_celltype = cell_line, y_similarity = value, symbol_meaning) %>%
  mutate(figure = "Figure_4H")

write.table(fig4h, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig4h.txt", row.names = F)
...
#Calculate means (for bar)
mean_cell_clique4 <- clique4_data %>% 
  select(cell_line, value) %>% 
  distinct() %>%
  dplyr::group_by(cell_line) %>%
  dplyr::summarise(mean_value = mean(value)) %>%
...
fig4i <- clique4_data %>% 
  ...
  distinct() %>%
  mutate(symbol_meaning = "datapoint") %>%
  bind_rows(mean_cell_clique4) %>%
  select(x_celltype = cell_line, y_similarity = value, symbol_meaning) %>%
  mutate(figure = "Figure_4H")

write.table(fig4i, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig4i.txt", row.names = F)
...
J-N) M- and N-synergies discussed in the text. Column labels are names of proteins or the inhibitor used (‘i’ suffix). Proteins or inhibitors with significant CCDs (FDRCCD < 0.05) are marked with an asterisk. Chromatin features are colored as in Fig. 1C. (J) ATM signaling. (K) Fanconi anemia complex. (L) SMC5/6 complex. (M) DNAPKcs KO and inhibition. (N) BRCA1-A complex.
...
# Source data
fig4j <- ATM.combined %>% 
  select(x_perturbation = gene, y_chromatin = chrom_feature, fill_CCDscore = CCD_synergy_score) %>%
  mutate(figure = "Figure_4J")

write.table(fig4j, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig4j.txt", row.names = F)
```
...
# Source data
fig4k <- FA.proteins.CCD %>% 
  select(x_perturbation = gene, y_chromatin = chrom_feature, fill_CCDscore = CCD_synergy_score) %>%
  mutate(figure = "Figure_4K")

write.table(fig4k, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig4k.txt", row.names = F)
...
fig4l <- SMC5_compl %>% 
  select(x_perturbation = gene, y_chromatin = chrom_feature, fill_CCDscore = CCD_synergy_score) %>%
  mutate(figure = "Figure_4L")

write.table(fig4l, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig4l.txt", row.names = F)
```
...
# Source data
fig4m <- DNAPK.perturbations %>% 
  select(x_perturbation = gene, y_chromatin = chrom_feature, fill_CCDscore = CCD_synergy_score) %>%
  mutate(figure = "Figure_4M")

write.table(fig4m, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig4m.txt", row.names = F)
...
# Source data
fig4n <- DNAPK.perturbations %>% 
  select(x_perturbation = gene, y_chromatin = chrom_feature, fill_CCDscore = CCD_synergy_score) %>%
  mutate(figure = "Figure_4N")

write.table(fig4n, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig4n.txt", row.names = F)
