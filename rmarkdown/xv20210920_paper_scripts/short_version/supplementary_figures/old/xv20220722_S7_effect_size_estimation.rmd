...
slope.protein.features.all <- readRDS("data/processed_data/xv20220624_DR_KO_CCD_summary.rds")
```

#Supplementary Figures

#Figure S6A: Simulate example of distribution
```{r}
set.seed(1)
mock_ditsr <- tibble(p = rep("mock",1000), values = rnorm(1000))
sample_19 <- tibble(p = rep("mock", 19), v = sample(mock_ditsr$values, 19))

#Calculate CI99
CI99_mock <- mock_ditsr %>% dplyr::group_by(p) %>% dplyr::summarise(CI0.5 = quantile(values, 0.005), CI99.5 = quantile(values, 0.995), range = CI99.5 - CI0.5, mean = mean(values))

#Plot 
ggplot(mock_ditsr) + 
  geom_violin(aes(p, values), fill = "grey70", color = "black") + theme_bw() + 
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))+ 
  geom_quasirandom(data = sample_19, aes(p, v), color = "red", size = 2)  + 
  geom_errorbar(data = CI99_mock, aes(x = p,ymin = CI0.5, ymax = CI99.5), color = "black", width = 0.5) 

```

#Figure S6B: Dynamic ranges
```{r}
...
mean.log2.fc <- log2_fold_change_IPR_KO %>% group_by(gene) %>% dplyr::summarise(mean.fc = mean(mean.log2foldchange, na.rm = T))

#Run t.test with all the data
test.log2.dist <- log2_fold_change_IPR_KO %>% dplyr::group_by(gene) %>% t_test(mean.log2foldchange ~ 0) %>% mutate(p.adj = p.adjust(p, method = "fdr")) %>% left_join(mean.log2.fc, by = "gene")
...
# Classes summary
classes_summary <- tibble(gene = c(both_CCD_global_proteins,only_CCD), class = c(rep("both", length(both_CCD_global_proteins)), rep("CCD", length(only_CCD))))

##Compute estimated log2FC
estimated_FC_CCD <- slope.protein.features.all %>% left_join(CI99_chromatin_features, by = c("feature" ="variable")) %>% mutate(estim_fc_CCD = range*CCD_value) %>% right_join(classes_summary)
...
#Calculate CI95 from max
CI95_max_CCD_effects <- max_estimated_CCD_FC  %>% pull(estim_fc_CCD) %>% abs() %>% quantile(c(0.025,0.975))
CI95_max_global_effects <- mean.log2.fc %>% filter(gene %in% global_effect_proteins)  %>% pull(mean.fc) %>% abs() %>% quantile(c(0.025,0.975))
...
ggplot(estimated_CCD_global %>% na.omit()) + 
  geom_point(aes(abs(estim_fc_CCD), abs(mean.fc), color = class)) +
  scale_color_manual(values = c("#CBCE91","#EA738D")) + theme(legend.position = "top") + geom_abline() + labs(color = "Type of DR protein") + theme_bw() + coord_cartesian(xlim = c(0,2), ylim = c(0,2))
