...
ggplot(drug_summary_test %>% filter(drug %in% c("ATMi","DNAPKi"))) + geom_quasirandom(aes(drug, mean)) + geom_hline(aes(yintercept = 0), linetype = 2) + theme_bw()
```
...
ggplot(significant.inh.t.test_levels) + geom_tile(aes(fct_relevel(barcode, clone5_bc_cluster), drug, fill = -log10(p.adj))) + coord_fixed(ratio = 1) + theme(axis.text.x = element_blank()) + scale_fill_gradient2() + geom_text(aes(fct_relevel(barcode, clone5_bc_cluster), drug, label = signif_level))
```
