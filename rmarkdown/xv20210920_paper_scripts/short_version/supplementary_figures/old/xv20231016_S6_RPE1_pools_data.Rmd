...
log2_data_RPE1 <- readRDS_proof("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/processed_data/CCD_validation_RPE1/","RPE1_diff")
log2_data_PRO <- readRDS_proof("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/processed_data/CCD_validation_RPE1/","PRO_diff")
log2_data_DEF <- readRDS_proof("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/processed_data/CCD_validation_RPE1/","DEF_diff")

all_RPE1_log2 <- bind_rows(log2_data_RPE1,log2_data_PRO,log2_data_DEF)
...
#Import raw metrics of all clones
revision_data_raw_RPE1 <- readRDS_proof("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/processed_data/CCD_validation_RPE1/","RPE1_screening")
revision_data_raw_PRO <- readRDS_proof("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/processed_data/CCD_validation_RPE1/","PRO_screening")
revision_data_raw_DEF <- readRDS_proof("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/processed_data/CCD_validation_RPE1/","DEF_screening")

all_RPE1_raw <- bind_rows(revision_data_raw_RPE1,revision_data_raw_PRO,revision_data_raw_DEF)


#log2 chromatin
PRO_log2_chrom <- readRDS_proof("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/processed_data/CCD_validation_RPE1/","PRO_log2") %>% select(-binsize, -dam_LMNB2)
K562_log2_chrom <- readRDS_proof("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/processed_data/CCD_analysis/","log2") %>% select(-binsize)
DEF_log2_chrom <- readRDS_proof("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/processed_data/CCD_validation_RPE1/","DEF_log2") %>% select(-binsize, -dam_LMNB2)
RPE1_log2_chrom <- readRDS_proof("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/processed_data/CCD_validation_RPE1/","RPE1_log2") %>% select(-binsize, -dam_LMNB2)
#Bind_rows 
chromatin_concordance <- bind_rows(PRO_log2_chrom, DEF_log2_chrom,RPE1_log2_chrom) %>% select(-gene,-gRNA,-mean.log2foldchange, -n_rep, -pathway_signif) %>% distinct() %>% melt() %>% mutate(chrom_feature = str_extract(variable, "(H3|l|L).*"))
...
B) Number of IPRs on average per cell line
```{r, fig.width=6, fig.height=4}
#Calculate mean per position
IPR_n_plot <- all_RPE1_log2 %>%
  dplyr::group_by(gene, cell_line) %>%
  dplyr::summarise(IPR_n = n())

#Summary table average per cell line
mean_IPR_n <- IPR_n_plot %>%
...
ggplot(IPR_n_plot, aes(rep, TIF)) +
  geom_col(aes(gene,IPR_n)) +
  ylab("Total IPR number") +
  theme_bw()  +
...
#IPRs on average per experiment
overall_freq <- all_RPE1_raw %>% 
  dplyr::group_by(cell_line, barcode) %>%
  dplyr::summarise(overall_freq_IPR = mean(mean_IPR_freq))

ggplot(overall_freq) +
  geom_point(aes(cell_line, overall_freq_IPR))
...
#Chromatin concordance in the pools
```{r}
ggplot(chromatin_concordance) + geom_quasirandom(aes(chrom_feature, value, color = value > 0.5)) + facet_wrap(~ cell_line) + theme(axis.text.x = element_text(angle = 90)) + geom_hline(yintercept = 0.5, linetype = 2)

filter_out_chromatin <- chromatin_concordance %>% filter(value > 0.5) %>% dplyr::group_by(cell_line, chrom_feature) %>% dplyr::summarise(IPR_n = n()) %>% filter(IPR_n < 2)
