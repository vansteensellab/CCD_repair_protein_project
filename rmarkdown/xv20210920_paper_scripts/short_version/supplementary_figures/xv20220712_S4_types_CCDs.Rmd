...
#Figure S4: Types of different CCDs (How to interpret CCDs)
```{r}

#Hits
proteins_CCDs <- adj_p.value_KO_model %>% filter(p.adj < 0.05) %>% pull(gene)
proteins_global_order <- test.log2.dist %>% arrange(mean.fc) %>%  pull(gene)
proteins_global <- test.log2.dist %>% filter(p.adj < 0.05) %>% pull(gene)

#No global effect
no_global_effect <- setdiff(proteins_CCDs,proteins_global)
global_effect <- intersect(proteins_CCDs, proteins_global)

#Select
barcode_significance <- fit.z.scores.ratio.mean %>% ungroup() %>% mutate(IPR_signif = case_when(mean.z.score.IPR < -1.95 ~ "MMEJ", mean.z.score.IPR > 1.95 ~ "NHEJ", T ~ "none")) %>% dplyr::select(gene,barcode,mean.z.score.IPR, IPR_signif)

#Arrange data
CCD_types <- DDR.KO.CCD.dt %>% left_join(fit.z.scores.ratio.IPR.filtered) %>% dplyr::select(gene,barcode,mean.log2foldchange, pathway_signif) %>% left_join(barcode_significance) %>% filter(gene %in% proteins_CCDs) %>% mutate(CCD_class = case_when(gene %in% global_effect ~ "global_CCD", T ~ "restricted_CCD"))

ggplot(CCD_types %>% filter(pathway_signif == "MMEJ")) + geom_quasirandom(aes(fct_relevel(gene, proteins_global),mean.log2foldchange, color = IPR_signif)) +
  facet_wrap(~ pathway_signif, scales = "free_x") + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))

ggplot(CCD_types %>% filter(pathway_signif == "NHEJ")) + geom_quasirandom(aes(fct_relevel(gene, proteins_global),mean.log2foldchange, color = IPR_signif)) +
  facet_wrap(~ pathway_signif, scales = "free_x") + theme_bw() + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
