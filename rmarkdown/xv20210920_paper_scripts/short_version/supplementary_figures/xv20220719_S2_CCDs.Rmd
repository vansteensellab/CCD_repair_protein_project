...
RAD50KO_example <- log2.distance.ratio.KO %>% filter(gene == "RAD50") %>% filter(barcode == "CATTTCTGATCAATAA")
...
#Fitted curve (R1)
ggplot() + 
  geom_histogram(data = log2_fold_change_IPR_WT %>% filter(barcode == "CATTTCTGATCAATAA" & t.rep == "R1"), aes(log2ratio), fill = "grey80", bins = 40) + 
  geom_line(data = distribution.R1 %>% filter(value > -2.5 & value < 0), aes(value,2*y), color = "black", size = 1) + 
  geom_segment(data = normal.parameters %>% filter(t.rep == "R1"), aes(x = wt.log2ratio + sd.wt.log2ratio, y = 0, xend = wt.log2ratio + sd.wt.log2ratio, yend = 6), color = "red")  + 
  geom_segment(data = normal.parameters %>% filter(t.rep == "R1"), aes(x = wt.log2ratio - sd.wt.log2ratio, y = 0, xend = wt.log2ratio - sd.wt.log2ratio, yend = 6), color = "red") +
  geom_segment(data = normal.parameters %>% filter(t.rep == "R1"), aes(x = wt.log2ratio, y = 0, xend = wt.log2ratio, yend = 7), color = "black", size = 1) +
  geom_segment(data = RAD50KO_example %>% filter(t.rep == "R1"), aes(x = log2ratio, y = 0, xend = log2ratio, yend = 7), color = "black", size = 1) +
  theme_bw() + coord_cartesian(xlim = c(-2.5,0))

#Step 1: Fitted curve (R4)
ggplot() + 
  geom_histogram(data = log2_fold_change_IPR_WT %>% filter(barcode == "CATTTCTGATCAATAA" & t.rep == "R4"), aes(log2ratio), fill = "grey80", bins = 40) + 
  geom_line(data = distribution.R2 %>% filter(value > -2.5 & value < 0), aes(value,2*y), color = "black", size = 1) + 
  geom_segment(data = normal.parameters %>% filter(t.rep == "R4"), aes(x = wt.log2ratio + sd.wt.log2ratio, y = 0, xend = wt.log2ratio + sd.wt.log2ratio, yend = 6), color = "red")  + 
  geom_segment(data = normal.parameters %>% filter(t.rep == "R4"), aes(x = wt.log2ratio - sd.wt.log2ratio, y = 0, xend = wt.log2ratio - sd.wt.log2ratio, yend = 6), color = "red") +
  geom_segment(data = normal.parameters %>% filter(t.rep == "R4"), aes(x = wt.log2ratio, y = 0, xend = wt.log2ratio, yend = 7), color = "black", size = 1) +
  geom_segment(data = RAD50KO_example %>% filter(t.rep == "R4"), aes(x = log2ratio, y = 0, xend = log2ratio, yend = 7), color = "black", size = 1) +
  theme_bw() + coord_cartesian(xlim = c(-2.5,0))

#Step 1: Fitted curve (R5)
ggplot() + 
  geom_histogram(data = log2_fold_change_IPR_WT %>% filter(barcode == "CATTTCTGATCAATAA" & t.rep == "R5"), aes(log2ratio), fill = "grey80", bins = 40) + 
  geom_line(data = distribution.R3 %>% filter(value > -2.5 & value < 0), aes(value,2*y), color = "black", size = 1) + 
  geom_segment(data = normal.parameters %>% filter(t.rep == "R5"), aes(x = wt.log2ratio + sd.wt.log2ratio, y = 0, xend = wt.log2ratio + sd.wt.log2ratio, yend = 6), color = "red")  + 
  geom_segment(data = normal.parameters %>% filter(t.rep == "R5"), aes(x = wt.log2ratio - sd.wt.log2ratio, y = 0, xend = wt.log2ratio - sd.wt.log2ratio, yend = 6), color = "red") +
  geom_segment(data = normal.parameters %>% filter(t.rep == "R5"), aes(x = wt.log2ratio, y = 0, xend = wt.log2ratio, yend = 7), color = "black", size = 1) +
  geom_segment(data = RAD50KO_example %>% filter(t.rep == "R5"), aes(x = log2ratio, y = 0, xend = log2ratio, yend = 7), color = "black", size = 1) +
  theme_bw()  + coord_cartesian(xlim = c(-2.5,0))
