...
Fig. S10: Impact of CCDs on human tumor genomes. 
...
driver_mutations_indels <- list.files(path = "import/xv20231017_indel_data_mathijs_new/BOOTSTRAP", full.names = T)[c(1:4,6,7)]
...
setwd(in.dir)
boot_matched_distribution <- readRDS_proof("data/processed_data/","bootstrap")

```

# Data processing of PCAWG data
...
#dev.off()
```
(A) Total number of human tumor genomes analyzed, classified by tumor types and driver genes in mutant cohorts (left) and controls (right). We analyzed genomes from 29 different tumor subtypes and 6 different driver genes.
...
ggplot(bias_per_tissue_type) +
  ...
  ylab("∆log2MMEJ:NHEJ het/eu")
#dev.off()
```
(B) log2MMEJ:NHEJ short deletion ratios per tumor subtype in control genomes. 
...
print(phenotype_conserved)
```


(C) Differential of log2MMEJ:NHEJ balance (∆log2MMEJ:NHEJHet/Eu) in heterochromatin (cLADs) and eu-chromatin (ciLADs) in controls and mutants, split by tumor type and driver gene. The null-distribution generated by bootstrapping of the control data is shown in grey for each tissue and driver cohort. Red dots indicate the mean ∆log2MMEJ:NHEJHet/Eu of the control distribution (with red error-bars showing 95% CI); blue dots indicate mean ∆log2MMEJ:NHEJHet/Eu of the indicated tumor type and driver gene. The fraction in the header of each panel summarizes the fraction of tumor types that follow the expect trend according to the K562 screen results.
...
## Plot Figure S10E
```{r, fig.width=2,fig.height=4, message = F, warning = F}
...
```
(D) in mutant cancer genomes compared to same tissue subtype controls by driver gene and (E) BRCA2-/- compared to genome-instable HPVneg HNSCC. 


# Analyse SV data from the manuscript in the same way as PCAWG data
## Data processing
```{r, message = F, warning = F}
...
# Figure S8F
```{r Fig_S8_D, warning=FALSE, message=FALSE, fig.width=2,fig.height=3, message = F, warning = F}
#Plot SV S8D
...
```
(F) Distribution of large deletions with MH at break sites counts over euchromatin (Eu.) and constitutive lamina-associated heterochromatin (Het.) in genome-instable HPV negative HNSCC and BRCA2-/- tumors. Two-sided p-value is shown in the figure.
