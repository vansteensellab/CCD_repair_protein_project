...
# Script to generate figure 6 in the paper.
...
# Data processing of PCAWG data (Figures 6A-E)
...
```{r, message = F, warning = F}
#Tumor type and figure
figure_tumor_type <- tibble(driver = c("ATM","MEN1","BRCA1","ATRX","BRCA2"),
                            figure = c("Figure_6A","Figure_6B","Figure_6C","Figure_6D","Figure_6E"))

figure_mean_bar <- balance_chromatin_plot_tumors %>%
  dplyr::group_by(driver, driver_s, variable) %>%
  dplyr::summarise(value = mean(value)) %>% mutate(symbol_meaning = "bar_height")

# Source data table
fig6ae <- balance_chromatin_plot_tumors %>% 
  mutate(symbol_meaning = "datapoint") %>%
  bind_rows(figure_mean_bar) %>%
  left_join(figure_tumor_type) %>%
  select(x_chromatin = variable, y_balance = value, symbol_meaning, panel_background = driver_s, figure)

# Split into different files
fig6a <- fig6ae %>% filter(figure == "Figure_6A")
fig6b <- fig6ae %>% filter(figure == "Figure_6B")
fig6c <- fig6ae %>% filter(figure == "Figure_6C")
fig6d <- fig6ae %>% filter(figure == "Figure_6D")
fig6e <- fig6ae %>% filter(figure == "Figure_6E")

# Save each one in a file
write.table(fig6a, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig6a.txt", row.names = F)
write.table(fig6b, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig6b.txt", row.names = F)
write.table(fig6c, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig6c.txt", row.names = F)
write.table(fig6d, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig6d.txt", row.names = F)
write.table(fig6e, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig6e.txt", row.names = F)
```
(A-E) Total MMEJ and NHEJ signature indel log2 ratio in euchromatin (Eu.) and constitutive lamina-associated heterochromatin (Het.) in genomes of tumors with indicated genotypes. Control tumors (+/+) were tissue-matched as much as possible to the respective mutant tumors (-/-) (Fig. S10A). Black dots represent values of individual tumor subtypes and the bars represent the median value across tissues and two-sided p-values are shown in the figure. 

# Data processing of HNSCC data (Figure 6F)
## Process data
```{r, message = F, warning = F}
...
# Figure 6F
```{r, fig.width= 2.5, fig.height=2, message = F, warning = F}
...
# Source data table
fig6f <- plot_BRCA2_balance %>% 
  select(x_chromatin = variable, y_balance = value, panel_background = tumor_type) %>%
  mutate(figure = "Figure_6F")

# Save each one in a file
write.table(fig6f, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/source_data/xv20240417_sourcedata_fig6f.txt", row.names = F)
```
(F) same as A-E, but for BRCA2-/- tumors compared to genome-instable HPVneg HNSCC, which have a more similar overall mutation rate than the set in panel E.

# Figure 6G
(G) Cartoon illustrating ATM M-synergy with triple heterochromatin. In ATM+/+ tumors (topleft panel), triple heterochromatin has a higher abundance of MMEJ mutations (brown speckles) compared to NHEJ mutations (green speckles), relative to euchromatin. In ATM-/- tumors (right panel), euchromatin remains relatively unchanged, while triple heterochromatin shows a stronger reduction of MMEJ mutations than of NHEJ mutations.
