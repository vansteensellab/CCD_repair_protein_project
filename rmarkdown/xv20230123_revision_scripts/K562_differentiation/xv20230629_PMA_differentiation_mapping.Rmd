...
slopes_calculation_pools <- map_dfr(c("LMNB2","H3K9me3","H3K27me3"), function(x) {
  single_ab <- chrom_features_RSTP2_pools_mean %>% filter(antibody == x)
  lm(single_ab$PMA~single_ab$DMSO) %>% tidy() %>% 
    filter(grepl("single_ab",term)) %>% 
    mutate(antibody = x) %>% 
    select(antibody,slope = estimate, std.error,statistic,p.value)
...
#Calculate slopes (Is the compresion different than genomewide)
slopes_calculation_clone5 <- map_dfr(c("LMNB2","H3K9me3","H3K27me3"), function(x) {
  single_ab <- chrom_values_mean %>% filter(antibody == x)
  lm(single_ab$PMA~single_ab$DMSO) %>% tidy() %>% 
    filter(grepl("single_ab",term)) %>% 
    mutate(antibody = x) %>% 
    select(antibody,slope = estimate, std.error,statistic,p.value)
