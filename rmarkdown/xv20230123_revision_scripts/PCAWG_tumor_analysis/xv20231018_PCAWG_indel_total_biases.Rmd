...
summary_tumor_n <- mutant_indel_data %>%
...
mutant_total_indels <- mutant_indel_data %>% 
  ...
  dplyr::summarise(sum_CILAD_NHEJ = mean(CILAD_NHEJ),
                   sum_CILAD_MMEJ = mean(CILAD_MMEJ),
                   sum_CLAD_NHEJ = mean(CLAD_NHEJ),
                   sum_CLAD_MMEJ = mean(CLAD_MMEJ),
                   sample_n = n()) %>%
  rowwise() %>%
...
mutant_total_indels_sample <- mutant_indel_data %>% 
  ...
  mutate(LAD_bal = log2((CLAD_MMEJ+1)/(CLAD_NHEJ+1)),
         ...
         sample = "cases") %>% right_join(tumor_types_cases)
...
control_total_indels_sample <- control_indel_data %>% 
  ...
  mutate(LAD_bal = log2((CLAD_MMEJ+1)/(CLAD_NHEJ+1)),
         ...
         total_indel = sum(CLAD_MMEJ,CLAD_NHEJ, CILAD_MMEJ,CILAD_NHEJ),
         min_indel = min(CLAD_MMEJ,CLAD_NHEJ, CILAD_MMEJ,CILAD_NHEJ),
         sample = "control") %>% right_join(tumor_types_cases)
#bind_rows
...
ggplot(total_changes_indels_sample) + 
  ...
  facet_wrap(~ driver+tumor, scales = "free_y") + theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))

ggplot(total_changes_indels_sample) + 
  geom_quasirandom(aes(sample,global_bal, color = sample)) +
  facet_wrap(~ driver+tumor) + theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
