...
# Tumor biases (per sample): Show distribution
```{r}
mutant_total_indels_sample <- mutant_indel_data %>% 
    filter(!TYPE %in% multiple_drivers_data$TYPE) %>%
    separate(project, into = c("tumor", "country"), remove = F) %>%
  rowwise() %>%
  mutate(LAD_bal = log2((CLAD_MMEJ+1)/(CLAD_NHEJ+1)),
         iLAD_bal = log2((CILAD_MMEJ+1)/(CILAD_NHEJ+1)),
         global_bal = log2((CLAD_MMEJ+CILAD_MMEJ+1)/(CLAD_NHEJ+CILAD_NHEJ)),
                 bias = LAD_bal - iLAD_bal,
         total_indel = sum(CLAD_MMEJ,CLAD_NHEJ, CILAD_MMEJ,CILAD_NHEJ),
         min_indel = min(CLAD_MMEJ,CLAD_NHEJ, CILAD_MMEJ,CILAD_NHEJ),
         sample = "cases") 
control_total_indels_sample <- control_indel_data %>% 
    separate(project, into = c("tumor", "country"), remove = F) %>%
  rowwise() %>%
  mutate(LAD_bal = log2((CLAD_MMEJ+1)/(CLAD_NHEJ+1)),
         iLAD_bal = log2((CILAD_MMEJ+1)/(CILAD_NHEJ+1)),
         ...
         sample = "control")
#bind_rows
total_changes_indels_sample <- mutant_total_indels_sample %>% bind_rows(control_total_indels_sample)
...
#Per indel count
ggplot(summary_tumor_n) +
  geom_col(aes(fct_relevel(driver,tumor_order), indel_total, fill = project)) +
  facet_wrap(~ sample) +
  theme_bw() +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
...
ggplot(total_changes_indels_sample) + 
  ...
  facet_wrap(~ driver, scales = "free_y") + theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))

ggplot(total_changes_indels_sample) + 
  geom_quasirandom(aes(sample,global_bal, color = sample)) +
  facet_wrap(~ driver) + theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))

```

#Calculate min_indel per permutation
```{r, warning=F, message = F}
#How many total mutations?
ggplot(total_changes_indels_sample) + 
  geom_quasirandom(aes(sample, min_indel, color = sample)) +
...
boot_strap_total_rough <- map_dfr(mutant_bias$driver, function(x) {
  driver_dt <- control_total_indels_sample %>% filter(driver == x)
...
#Calculate the maximum n to match distributions
```{r}
...
limiting_factor_driver <- total_n_per_tumor %>%
  ungroup() %>%
  dplyr::group_by(driver) %>%
  slice_min(ratio) %>%
...
#Matched control distributions per tumor type
...
mean_project_value <- mutant_total_indels_project %>%
  select(project, driver, bias) %>%
  left_join(null_distriution) %>%
  mutate(z_score = (bias - mean_null)/sd_null) %>%
  dplyr::group_by(driver) %>%
  dplyr::summarise(combined = mean(z_score, na.rm = T)/sqrt(n()),
