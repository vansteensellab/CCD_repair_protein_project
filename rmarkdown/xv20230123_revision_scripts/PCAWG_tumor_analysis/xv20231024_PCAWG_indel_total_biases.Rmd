...
#Plot amount of mutation per patient
```{r, fig.width=8, fig.height=4}

plot_indel <- filtered_indels_samples %>%
  filter(sample == "mutant") %>%
  melt()

ggplot(plot_indel) +
  geom_quasirandom(aes(variable, value)) +
  facet_wrap(~driver, scales = "free_y", ncol = 4) +
  theme_bw() +
...
chq_test_summary <- map2_dfr(combinations$driver, combinations$sample, function(x,y){
  ...
  column_to_rownames(var = "pathway") %>% chisq_test() %>% mutate(driver = x, sample = y)
...
```{r}
#FA proteins order
FA_prots <- c("BRCA2","ATRX","TRRAP","EGFR","SETD2","ATM","MEN1")
...
#Summary number of tumors (cases vs. controls)
```{r, fig.width=4, fig.height=3}
summary_tumor_n <- total_changes_indels_sample %>%
  dplyr::group_by(driver, project, sample) %>%
  dplyr::summarise(count = n(),
                   indel_total = sum(total_indel, na.rm = T))

#Tumor order
tumor_order <- mutant_indel_data %>%
  filter(sample == "mutant") %>%
  bind_rows(control_indel_data) %>%
  select(TYPE, driver,project, sample) %>%
  distinct() %>%
...
ggplot(summary_tumor_n %>% filter(sample == "cases")) +
  geom_col(aes(fct_relevel(driver,tumor_order), count, fill = project)) +
  facet_wrap(~ sample) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
...
#Trends of chromatin bias accumulation (same figure as in paper)
```{r, fig.width=3,fig.height=6, warning=F, message=F}
#Gene order
driver_order <- c("BRCA2","ATM","MEN1","ATRX","BRCA1","TRRAP","SETD2","EGFR")

balance_chromatin_plot <- total_changes_indels %>%
  select(driver, sample, LAD_bal, iLAD_bal) %>%
  reshape2::melt()

#Trend concordant
ggplot(balance_chromatin_plot %>% filter(driver %in% c("BRCA2","ATM","MEN1","ATRX"))) +
  geom_col(aes(fct_relevel(variable, c("iLAD_bal","LAD_bal")),value, fill = variable)) +
  facet_grid(fct_relevel(driver, driver_order) ~ sample) +
  theme_bw() +
  theme(legend.position = "none",
        panel.grid = element_blank(),
        axis.title.x = element_blank()) +
...
#Trend discordant
ggplot(balance_chromatin_plot %>% filter(driver %in% c("BRCA1","TRRAP","SETD2","EGFR"))) +
  geom_col(aes(fct_relevel(variable, c("iLAD_bal","LAD_bal")),value, fill = variable)) +
  facet_grid(fct_relevel(driver, driver_order) ~ sample) +
  theme_bw() +
  theme(legend.position = "none",
...
```{r, warning=F, message=F}
mutant_bias <- total_changes_indels %>% 
  filter(sample == "mutant") %>% 
  dplyr::group_by(driver, sample_n,bias) %>%
  dplyr::summarise(min_indel = min(sum_CILAD_MMEJ,sum_CILAD_NHEJ,sum_CLAD_NHEJ,sum_CLAD_MMEJ))

#bootstrap_general_tumors
boot_strap_total_rough <- map_dfr(mutant_bias$driver, function(x) {
  driver_dt <- filtered_indels_samples %>% filter(sample == "control" & driver == x)
...
ggplot(boot_strap_total_rough, aes(driver,bias)) + 
  ...
  theme_bw() + geom_point(data = total_changes_indels, aes(driver,bias, color = sample), size = 3)


#Calculate p-value: 
mean_sd_calculation_driver <- boot_strap_total_rough %>%
  dplyr::group_by(driver) %>%
  dplyr::summarise(mean_bias = mean(bias),
                   sd_bias = sd(bias)) %>%
  left_join(mutant_bias) %>%
  mutate(z_score = (bias - mean_bias)/sd_bias,
...
ggplot(mean_sd_calculation_driver) + 
  geom_col(aes(fct_reorder(driver, z_score), z_score, fill = fdr < 0.05)) +
  theme_bw() +
...
ggplot(balance_chromatin_plot %>% filter(driver %in% c("BRCA2","ATRX"))) +
  geom_col(aes(fct_relevel(variable, c("iLAD_bal","LAD_bal")),value, fill = variable)) +
  facet_grid(fct_relevel(driver, driver_order) ~ sample) +
  theme_bw() +
  theme(legend.position = "none",
...
boot_matched_distribution <- map2_dfr(simplified_table_filter$driver,simplified_table_filter$project, function(x,y) {
  driver_dt <- mutant_indel_data %>% bind_rows(control_indel_data) %>% filter(driver == x & project == y)
  n_samples <- nrow(driver_dt)
  iteration_tib <- map_dfr(c(1:1000), function(j) {
    set.seed(j)
      driver_dt %>% 
...
#Trend discordant: BRCA2 example
ggplot(balance_chromatin_plot_tumors %>% filter(driver %in% c("BRCA2","ATRX")),aes(fct_relevel(variable, c("iLAD_bal","LAD_bal")),value, fill = variable)) +
...
#Create matched distributions
```{r}
total_n_per_driver <- summary_tumor_n %>%
  ungroup() %>%
  dplyr::group_by(driver, sample) %>%
  dplyr::summarise(count = sum(count)) %>%
  reshape2::dcast(driver  ~ sample) %>%
  select(driver, n_cases = cases, n_control = control)


#Calculate all ratios
total_n_per_tumor <- summary_tumor_n %>%
  ungroup() %>%
  dplyr::group_by(driver, sample, project) %>%
  dplyr::summarise(count = sum(count)) %>%
  reshape2::dcast(driver + project ~ sample) %>%
  left_join(total_n_per_driver) %>%
  na.omit() %>%
  mutate(ratio = control/cases,
         freq_cases = cases/n_cases,
         freq_control = control/n_control,
         max_total = control/freq_cases,)

#Max total per limiting factor


#Calculate limiting ratio
limiting_factor_driver_tumor <- total_n_per_tumor %>%
  ungroup() %>%
  dplyr::group_by(driver) %>%
  slice_min(ratio) %>%
  select(driver, limiting_ratio = ratio)

#Calculate bootstrapping minimum
tumor_types <- total_n_per_tumor %>%
  left_join(limiting_factor_driver) %>%
  mutate(sample_n = round(limiting_ratio*cases, digit = 0))

#Simplified table for bootstraping
simplified_table <- tumor_types %>%
  select(driver, project, sample_n)

#How many controls are thrown away?
ggplot() +
  geom_col(data = summary_tumor_n %>% filter(sample == "control"), aes(driver, count), fill = "grey70") + 
 geom_col(data = simplified_table, aes(fct_relevel(driver,tumor_order),sample_n, fill = project)) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5),
        axis.title.x = element_blank(),
        panel.grid = element_blank())
...
#Sample controls to get equal distributions in cases and controls
```{r}
#Check if number add up
check_control_n <- control_indel_data %>% count(driver, project,name = "control_n")
check_driver_control_n <- control_indel_data %>% count(driver, name = "control_n")
# Tumor biases (per project)
matched_distribution_controls <- map2_dfr(simplified_table$driver, simplified_table$project, function(x,y){
  set.seed(1)
  n_sampling <- simplified_table %>% filter(driver == x & project == y) %>% pull(sample_n)
  control_indel_data %>% filter(driver == x & project == y) %>% sample_n(n_sampling)
})

# Calculate bias per driver (mean values)
matched_distribution_data_driver_mean <- matched_distribution_controls %>%
dplyr::group_by(driver,sample) %>%
  dplyr::summarise(sum_CILAD_NHEJ = sum(CILAD_NHEJ),
                   sum_CILAD_MMEJ = sum(CILAD_MMEJ),
                   sum_CLAD_NHEJ = sum(CLAD_NHEJ),
...
#Bootstrap with matched data
```{r, warning=F, message=F}
mutant_bias <- total_changes_indels %>% 
  filter(sample == "mutant") %>% 
  dplyr::group_by(driver, sample_n,bias, sample, LAD_bal, iLAD_bal) %>%
  dplyr::summarise(min_indel = min(sum_CILAD_MMEJ,sum_CILAD_NHEJ,sum_CLAD_NHEJ,sum_CLAD_MMEJ))

#Bootstrap n
bootstrap_n_summary <- matched_distribution_controls %>%
  count(driver) %>% left_join(check_driver_control_n)

#bootstrap_general_tumors
boot_strap_total_matched <- map_dfr(mutant_bias$driver, function(x) {
  driver_dt <- matched_distribution_controls %>% filter(driver == x)
  n_samples <- nrow(driver_dt)
  iteration_tib <- map_dfr(c(1:1000), function(j) {
    set.seed(j)
      driver_dt %>% 
...
#Control mutant match
control_mutant_bias <- mutant_bias %>%
  bind_rows(matched_distribution_data_driver_mean)


#ggplot difference densities
ggplot(boot_strap_total_matched, aes(driver,bias)) + 
  geom_quasirandom(color = "grey80") + 
  stat_summary(geom = "errorbar", fun.min = function(z) { quantile(z,0.05) },
               fun.max = function(z) { quantile(z,0.95) }, color = "red", width = 0.25) +
  theme_bw()  + geom_point(data = control_mutant_bias, aes(driver,bias, color = sample), size = 3) +
  theme(legend.position = "top")


#Calculate p-value: 
mean_sd_calculation <- boot_strap_total_matched %>%
#  filter(driver %in% c("ATM","ATRX","BRCA2","MEN1")) %>%
  dplyr::group_by(driver) %>%
  dplyr::summarise(mean_bias = mean(bias),
                   sd_bias = sd(bias)) %>%
  left_join(mutant_bias) %>%
...
#Plot balance as it was in manuscript
```{r, fig.width=3.5, fig.height=4}
balance_chromatin_plot_matched <- control_mutant_bias %>%
  ungroup() %>%
  select(driver, sample, LAD_bal, iLAD_bal) %>%
  reshape2::melt()

#Trend discordant: BRCA2 example
ggplot(balance_chromatin_plot_matched %>% filter(driver %in% c("BRCA2","ATRX"))) +
  geom_col(aes(fct_relevel(variable, c("iLAD_bal","LAD_bal")),value, fill = variable)) +
  facet_grid(fct_relevel(driver, driver_order) ~ sample) +
  theme_bw() +
  theme(legend.position = "none",
...
#Bootstrap per tumor-type
```{r, message=F, warning=F}
#bootstrap_general_tumors
boot_matched_distribution <- map2_dfr(simplified_table$driver,simplified_table$project, function(x,y) {
  driver_dt <- control_total_indels_sample %>% filter(driver == x & project == y)
  n_samples <- simplified_table %>% filter(driver == x & project == y) %>% pull(sample_n)
  iteration_tib <- map_dfr(c(1:100), function(j) {
    set.seed(j)
      driver_dt %>% 
        ungroup() %>%
        sample_n(n_samples,replace = TRUE) %>%
        mutate(iteration = j, driver = x, poject =y) %>% 
          dplyr::group_by(iteration, driver, project) %>%
          dplyr::summarise(tumLAD_MMEJ = sum(CLAD_MMEJ), 
                           tumLAD_NHEJ = sum(CLAD_NHEJ), 
                           tumiLAD_MMEJ = sum(CILAD_MMEJ), 
                           tumiLAD_NHEJ = sum(CILAD_NHEJ))
        })
      })

#Compute metrics per iteration

metrics_matched_distribution <- boot_matched_distribution %>%
  dplyr::group_by(iteration, driver,project) %>%
  dplyr::summarise(LAD_MMEJ = sum(tumLAD_MMEJ), 
                           LAD_NHEJ = sum(tumLAD_NHEJ), 
                           iLAD_MMEJ = sum(tumiLAD_MMEJ), 
                           iLAD_NHEJ = sum(tumiLAD_NHEJ),
                           global_bal = log2((LAD_MMEJ+iLAD_MMEJ)/(LAD_NHEJ+iLAD_NHEJ)),
                           LAD_bal = log2(LAD_MMEJ/LAD_NHEJ),
                           iLAD_bal = log2(iLAD_MMEJ/iLAD_NHEJ),
                           bias = LAD_bal - iLAD_bal)
