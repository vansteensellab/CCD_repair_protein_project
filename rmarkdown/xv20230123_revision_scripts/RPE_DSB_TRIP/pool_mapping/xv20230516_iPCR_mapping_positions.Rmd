...
```{r}
#function
levels <- c(paste0("chr",c(1:22)),"chrX")
...
#plot ideogram
ggplot() +
  geom_segment(data = chromsizes %>% filter(chr_r %in% levels),
               aes(x = chr_r, xend = chr_r, y = 0, yend = pos_r),
               lineend = "round", color = "lightgrey", size = 2) +
  geom_point(data = total_reads_map %>% filter(cell != "mESC"),
               aes(x = fct_relevel(seqname, levels),
                   y = start),
             size = .8) +
  theme_classic(base_size = 16) +
  xlab("Chromosome") + facet_wrap(~cell+ complexity + transfection) +
...
genomic_ranges_files <- list.files(path = "bins", pattern = "2kb", full.names = T)
...
heatmap_matrix <-  chromatin_features_pools %>% filter(nchar(barcode) == 16) %>% select(-cell, -transfection, - complexity, - barcode) %>% column_to_rownames(var = "ID") 
...
chromatin_cluster <-colnames(heatmap_matrix[,pools_heatmap$tree_col[["order"]]])
...
heatmaps_pools <- map(selected_pools, function(x) {
  ggplot(pools_IPR_melt %>% filter(grepl((paste0(x,"$")), barcode))) + 
