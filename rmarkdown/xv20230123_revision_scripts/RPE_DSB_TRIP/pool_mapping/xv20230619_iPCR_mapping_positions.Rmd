  ...
  read.delim(x, header = F) %>% 
    mutate(filename = str_extract(x, "(?<=NoIndex_).*(?=.starcode.count)"),
           ...
           transfection = case_when(grepl("High", filename) ~ "High",
                                    ...
                                    T ~ "High")) %>%
    dplyr::select(barcode = V1, bc_count = V2, filename, exp, cell, pool, complexity,transfection)})
...
#Summary plot
#number of reporters in IPR_high_confidence
summary_pools_bcPCR <- table_IPR_pools %>% 
  filter(bc_count > 10) %>%
  dplyr::group_by(cell, complexity, transfection) %>%
  dplyr::summarise(pool_IPR_match = n())

#Plot number if IPR
ggplot(summary_pools_bcPCR) + 
  geom_col(aes(fct_relevel(transfection, c("Low","High")),
               pool_IPR_match,
               fill = fct_relevel(complexity,c("100","250","1000"))),
           position = "dodge") + 
...
#Quality control: check if data looks comparable to genomewide data.
###Note: This is not easy with Christ script. First with Dam data
```{r}
setwd("/DATA/projects/DSBrepair/data/DSB_TRIP_cell_lines_mapping/xv20230519_all_RPE_pools_mapping/")
# Load chip data
dam_files <- list.files(path = "dam", pattern = "2000", full.names = T)
...
chromatin_features_pools_dam_control <- map_dfr(dam_files, function(x) {
  read.delim(x, header = T) %>%
    dplyr::select(name, "score" =mean) %>%
    na.omit() %>%
    mutate(file = paste0("dam_",str_extract(x, "(?<=2000_).*(?=.txt)")))
})
...
#Make a dataframe with dam values at the IPRs (mean values, check dynamic range)
chromatin_features_pools_chip_control <- map_dfr(chip_files, function(x) {
  read.delim(x, header = T) %>%
    dplyr::select("name"=ID, "score" =log2_mean) %>%
    na.omit() %>%
    mutate(file = paste0("chip_",str_extract(x, "H.*(?=_GS)")))
})

#Transform repliseq data to fit pattern
repliseq_dt <- repliseq_dataframe %>% dplyr::select("name" = ID, "score" = repliseq) %>% mutate(file = "repliseq")

chromatin_features_pools_IPR <- bind_rows(chromatin_features_pools_dam_control,chromatin_features_pools_chip_control)
```
#Note: I will use the mean value, as it has the most similar range than genomewide values. But, I am not 100% sure they are equivalent
...
chromatin_features_pools_genomewide <- bind_rows(chromatin_features_pools_dam_genomewide,chromatin_features_pools_chip_genomewide)
```


#Plot both correlation
```{r}
#Plot both data
ggplot() + 
  geom_density(data =chromatin_features_pools_IPR, aes(score)) +
  geom_density(data=chromatin_features_pools_genomewide, aes(score), color = "red", fill = NA) +
  theme_bw() +facet_wrap(~file)
```


#Plot coverage in the pools selected
```{r}
selected_pools <- c("RPE1_Low_1000","RPE1Deff_Low_1000","RPE1Proff_Low_250")

filtered_chromatin_pools_IPR <- map_dfr(selected_pools,function(x){
  chromatin_features_pools_IPR %>% filter(grepl(x, name)) %>% mutate(pool = x)
})

#Plot both data
ggplot() + 
  geom_quasirandom(data =filtered_chromatin_pools_IPR, aes(file,score)) +
  geom_violin(data=chromatin_features_pools_genomewide, aes(file,score), color = "red", fill = NA) +
  theme_bw() + facet_wrap(~ pool)
```
...
number_IPRs <- plot_densities %>%
  dplyr::select(barcode,cell,transfection,complexity) %>% 
  distinct() %>%
...
ggplot() +
  geom_segment(data = chromsizes %>% filter(chr_r %in% levels) %>% mutate(chr = fct_reorder(chr_r,levels)),
              aes(x = fct_relevel(chr,levels), xend = fct_relevel(chr, levels), y = 0, yend = pos_r),
               lineend = "round", color = "lightgrey", size = 2) +
  geom_point(data = selected_locations %>% filter(chr != "chrY"),
...
#With a single heatmap
 #Make dataframe for heatmap
heatmap_matrix <- map_dfr(selected_pools, function(x) {
  
    chromatin_features_pools %>% 
    filter(nchar(barcode) == 16 & grepl(x,ID)) %>% 
    dplyr::select(-cell, -transfection, - complexity, - barcode) %>% 
    column_to_rownames(var = "ID") 
...
#dt for plotting
pools_IPR_melt <- heatmap_matrix %>% rownames_to_column(var = "barcode") %>% melt(value.name = "z_score", var.name = "feature") %>% separate(barcode, into = c("bc", "cell","DNA","complexity"), remove = F)
