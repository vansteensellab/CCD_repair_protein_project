...
control_balance_bio <- indel_data %>% filter(condition %in% c("LBR2","tracr") & outcome %in% c("ins_1","del_-7","wt_0") & bio_rep != "R4")
...
filtered_balance <- read_counts_filter_reprod_summary %>%
  filter(read_filter == 20 & reps > 1) %>%
  select(barcode, cell_line, mean_bal, sd_bal, reps)

filtered_TIF <- read_counts_filter_reprod_summary_TIF %>%
  filter(read_filter == 20 & reps > 1) %>%
  select(barcode, cell_line, mean_TIF = mean_bal, sd_TIF = sd_bal, reps)
