...
spearman_coefficients <- map2_dfr(chromatin_analysis_pair$cell_line, chromatin_analysis_pair$read_filter, function(x,y){
  ...
  rho_table <- map_dfr(colnames(values_chromatin[9:18]), function(j){
      ...
      cor.test(x = balance, y = feature, method = "spearman", use = "pairwise.complete.obs") %>% tidy() %>% mutate(chromatin = j)
...
```
#CONCLUSION: Data can still be improved but the patway balances are there. These are the highest in RPE1 wt cell line (this is the most picky cell). The balances seem to be smaller in RPEDef & RPEPro cells. Not proper chromatin annotation?
