...
for (j in unique(values_chromatin$cell_line)){
  for (i in unique(values_chromatin$condition)){
    ...
    set.seed(1)
    PCR_model_DDR_test <- pcr(mean_diff~ dam.H3K9me3+dam.H3K27me3+chip.H3K27ac+chip.H3K4me1+ chip.H3K36me3+chip.H3K4me3+chip.H3K4me2+dam.H3K9me2+dam.LMNB1+late_replicating, data=gene.library.dt , validation="CV") #Run principal component regression
...
cosine.cliques <- map_dfr(c("ATMi","DNAPKi"), function(x){
  ...
  cosine.d.cliques <- cosine(as.matrix(cosine.cliques.cells.dcast)) %>% reshape2::melt() %>% mutate(drug = x)
})
...
ggplot(values_chromatin %>% filter(condition == "ATMi"), aes(dam.H3K27me3,)) +
  geom_point() +
...
ggplot(values_chromatin %>% filter(condition == "DNAPKi"), aes(chip.H3K36me3,diff_bal)) +
  geom_point() +
  geom_smooth(se = F, method = "lm") + 
  ggpubr::stat_cor(method = "pearson") +
  facet_wrap(~ cell_line) +
