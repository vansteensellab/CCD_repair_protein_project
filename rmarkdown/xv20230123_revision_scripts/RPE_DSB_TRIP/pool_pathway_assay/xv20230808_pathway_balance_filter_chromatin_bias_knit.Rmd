...
chromatin_features_pools_bin <-  left_join(chromatin_features_pools_chip_bin, chromatin_features_pools_dam_bin, by = c("ID"="name")) %>%
  ...
  mutate(cell_line = case_when(iPCR_cell == "RPE1Deff" ~ "RPEDef",
                               ...
                               T ~ iPCR_cell)) %>%
  select(-pool, -iPCR_cell, -iPCR_transfection, -iPCR_complexity) %>%
  mutate(chrom_type = case_when(bin_LMNB1 ==1 & bin_H3K9me3 == 1 & bin_repli == 1 ~ "triple_het",
                                bin_H3K27me3 == 1 ~ "H3K27me3",
                                bin_LMNB1 == 1 | bin_H3K9me3 == 1 | bin_H3K9me2 == 1 | bin_repli == 1 | bin_LMNNB2 == 1 ~ "other_het",
                                T ~ "euchromatin"))
...
```{r, warning=F}
#RPE data
values_chromatin_RPE <- left_join(filtered_balance_75, chromatin_features_pools_5000)
...
#Join with balances and calculate the correlation (for U2OS)
```{r, warning=F}
#RPE data
values_chromatin_U2OS <- left_join(filtered_balance_75, chromatin_features_pools_U2OS)
...
spearman_coefficients <- map_dfr(chromatin_analysis_pair$cell_line, function(x){
  ...
  rho_table <- map_dfr(colnames(values_chromatin_U2OS[9:17]), function(j){
      balance <- filter_data$mean_bal
      feature <- as.numeric(as.matrix(filter_data[j]))
        cor.test(x = balance, y = feature, method = "spearman", use = "pairwise.complete.obs") %>% broom::tidy() %>% mutate(chromatin = j)
})
...
#Plot correlation coefficients
ggplot(spearman_coefficients %>% distinct()) + 
  geom_col(aes(fct_relevel(chromatin),estimate, fill = p.value < 0.05)) + 
  coord_flip() +
  theme_bw() + 
  xlab("Chromatin features") +
  ylab("Spearman's correlation coefficient of MMEJfreq.") + 
...
ggplot(values_chromatin_RPE, aes(dam_LMNB1, mean_bal)) + 
