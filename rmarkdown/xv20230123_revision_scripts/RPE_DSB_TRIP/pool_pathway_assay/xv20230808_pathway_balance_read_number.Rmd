...
#Calculat total read numbers
total_read_barcode <- indel_data_clean %>% 
  dplyr::group_by(barcode, cell_line, condition, bio_rep) %>% 
  dplyr::summarise(total_IPR = sum(counts, na.rm = T))

#Total sample number
total_read_sample <- indel_data_clean %>% 
  dplyr::group_by(cell_line, condition, bio_rep) %>% 
  dplyr::summarise(total_sample = sum(counts, na.rm = T))




#Only look at LBR2 vs. tracr
control_balance_bio <- indel_data_clean %>% filter(condition %in% c("LBR2") & outcome %in% c("ins_1","del_-7","wt_0")) %>% left_join(total_read_barcode) %>% left_join(total_read_sample) %>% select(barcode, outcome, cell_line, bio_rep, counts,total_IPR) %>% distinct()

#Look at read numbers
control_balance_bio_dcast <- control_balance_bio %>% 
  reshape2::dcast(barcode + cell_line + condition + bio_rep ~ outcome, value.var = "counts", fill = 1) %>% 
  left_join(total_read_barcode) %>%
  left_join(total_read_sample) %>% 
  mutate(fraction_IPR = total_IPR/total_sample)

#plot relationship between 
ggplot(control_balance_bio_dcast) +
  geom_point(aes(`del_-7`, fraction_IPR))
...
read_count <- c(0,0.00005,0.0001,0.00025,0.0005,0.001,0.005,0.01)

filter_read_count_test <- map_dfr(read_count , function(x) {
  pathway_metric_control_bio %>%
    filter(fraction_IPR > x) %>%
    mutate(IPR_percentage = x*100)
})

#Number of IPRs in each
IPR_summary <- filter_read_count_test %>% 
  dplyr::group_by(cell_line, condition, IPR_percentage, bio_rep) %>% 
  dplyr::summarize(IPR_n = n()) 

IPR_summary_dcast <- IPR_summary %>% 
  reshape2::dcast(cell_line + condition + bio_rep ~ IPR_percentage, value.var  = "IPR_n")
...
ggplot(IPR_summary %>% 
  ...
  geom_col(aes(x = bio_rep,
               y = IPR_n,
               fill = fct_relevel(as.character(IPR_percentage),as.character(read_count))),
           position = "dodge") +
  labs(fill = "IPR_percentage") +
...
ggplot(filter_read_count_test) + 
  geom_point(aes(log10(`del_-7`),log10(total_IPR))) +
  facet_grid(cell_line ~ IPR_percentage) + geom_abline() +
  theme_bw()
```
...
replicates_IPR <- filter_read_count_test %>%
  filter(condition == "LBR2") %>%
  dplyr::group_by(barcode,cell_line, IPR_percentage) %>%
  dplyr::summarise(rep_n = n()) %>%
  ungroup() %>%
  dplyr::group_by(cell_line, IPR_percentage,rep_n) %>%
  dplyr::summarise(capture_IPR = n())

total_IPR_rep <- filter_read_count_test %>%
  filter(condition == "LBR2") %>%
  select(cell_line, barcode, IPR_percentage) %>% 
  distinct() %>%
  dplyr::group_by(cell_line,IPR_percentage) %>%
...
#Absolute_numbers
IPR_reps <- replicates_IPR %>% reshape2::dcast(cell_line + rep_n ~ IPR_percentage)

#IPRs in multiple reps
ggplot(replicates_IPR) + 
  geom_col(aes(x = fct_relevel(as.character(IPR_percentage),as.character(read_count)),
               y = capture_IPR,
               fill = as.character(rep_n)),
           position = "dodge") +
  labs(fill = "IPR_reps") +
  facet_wrap(~ cell_line, scales = "free") + 
  theme_bw() + labs(x = "IPR_percentage")
...
read_counts_filter_reprod <- filter_read_count_test %>%
  filter(condition == "LBR2") %>%
  reshape2::dcast(barcode + cell_line + condition + IPR_percentage ~ bio_rep, value.var = "balance")
...
ggplot(read_counts_filter_reprod_summary, aes(R1,R2)) +
  ...
  facet_grid(cell_line ~ IPR_percentage, scales = "free") +
  theme_bw() +
...
#Test with U2OS & 50
analysis_pair <- read_counts_filter_reprod_summary %>% select(cell_line, IPR_percentage) %>% distinct()

replicate_comparisons <- tidyr::crossing(c(1:4),c(1:4))

tmp_f <- map2_dfr(analysis_pair$cell_line,analysis_pair$IPR_percentage, function(x,y){
  #Filter_data
  data_sel <- read_counts_filter_reprod_summary %>%
    filter(cell_line == x & IPR_percentage == y) %>%
    select(R1, R2, R3, R4)
...
read_counts_filter_reprod_TIF <- filter_read_count_test %>%
  filter(condition == "LBR2") %>%
  reshape2::dcast(barcode + cell_line + condition + IPR_percentage ~ bio_rep, value.var = "TIF")
...
filtered_balance <- read_counts_filter_reprod_summary %>%
  filter(reps > 1) %>%
  select(barcode, cell_line, mean_bal, sd_bal, reps, IPR_percentage)

#TIF data
filtered_balance_TIF <- read_counts_filter_reprod_summary_TIF %>%
  filter(reps > 1) %>%
  select(barcode, cell_line, mean_TIF, sd_TIF, reps, IPR_percentage)

#Number of IPRs per cell line and IPR_percentage
IPR_number <- filtered_balance %>% dplyr::group_by(cell_line,IPR_percentage) %>% dplyr::summarise(counts = n())
...
filtered_balance_75 <- filtered_balance %>% left_join(filtered_balance_TIF) %>% filter(IPR_percentage == 0.)
...
spearman_coefficients_all <- map2_dfr(chromatin_analysis_pair$cell_line, chromatin_analysis_pair$IPR_percentage, function(x,y){
  filter_data <- values_chromatin_all_filter %>% filter(cell_line == x & IPR_percentage == y)
  ...
  rho_table %>% mutate(cell_line = x, IPR_percentage = y) %>% distinct()
})
...
ggplot(spearman_coefficients_all %>% distinct()) + 
  ...
  xlab("Chromatin features") +
  ylab("Spearman's correlation coefficient of MMEJfreq.") + 
  facet_grid(fct_relevel(cell_line, c("RPE1","RPEDef","RPEPro")) ~ IPR_percentage)
```

#Spearman correlation over different filters
```{r, fig.width=15, fig.height=5}
ggplot(spearman_coefficients_all %>% distinct()) + 
  geom_col(aes(fct_relevel(as.character(IPR_percentage), as.character(read_count)), estimate, fill = cell_line)) + 
  theme_bw() + 
  xlab("Read filter") +
  ylab("Spearman's correlation coefficient of MMEJfreq.") + 
...
write_rds(filter_read_count_test %>% filter(IPR_percentage == 75), file = "~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/pools_balance/xv20230726_high_confidence_IPR_set.rds")
```
