...
ggplot(mean.diff.TIF.export) + geom_quasirandom(aes(no_effect,mean.diff.TIF)) + theme_bw()

```
#CONCLUSION: Proteins that have any type of effect tend to have lower editing efficiency. But, the effect sizes are very modest compared to changes in pathway balance. 
...
for (i in unique(WT.values$rep)) {
  for (x in unique(WT.values$barcode)) {
    single_wt_ratio <- filter(WT.values, rep == i & barcode == x) %>% pull(log2TIF)
...
fit.z.scores.TIF.IPR <- screen.data.names.tib %>% 
  ...
  na.omit() %>% #remove NA
  mutate(z.score.log2ratio = (log2TIF - wt.log2TIF)/sd.wt.log2TIF) %>% #calculate z-score value
...
print(paste("We empirically tested the FDR and found that ",samples_significant_CCDs[2,"counts"], " out of 33 mock KO samples (9%) were called significant confirming low FDR"))
```
...
```{r}
combination_tmp <- DR_KO_CCD_all_values %>% select(gene, "chrom_feature"=feature,"TIF_CCD_value" = CCD_value) %>% left_join(data_DR_screen) %>% filter(DR_effect_type %in% c("CCD","both"))

genes_CCD <- unique(combination_tmp$gene)  #68 out of 89 also have differences in TIF

cosine_distance_TIF_ratio <- map_dfr(genes_CCD, function(x) {
  TIF_CCD <- combination_tmp %>% filter(gene == x) %>% pull(TIF_CCD_value) %>% abs()
  ratio_CCD <- combination_tmp %>% filter(gene == x) %>% pull(CCD_synergy_score) %>% abs()
  
  tibble(gene = x, cosine_sim = as.numeric(cosine(TIF_CCD,ratio_CCD)))
})
