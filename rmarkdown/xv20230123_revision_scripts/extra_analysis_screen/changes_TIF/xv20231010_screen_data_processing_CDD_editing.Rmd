...
dcast_TIF <- screen.data.names.tib  %>% reshape2::dcast(plate+well+barcode+gene+sample ~ rep, value.var = "log2TIFratio")
...
#R1 vs. R2 (Fig. S1B)
pdf("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/supplementary_figures/figure_S1_general_controls/xv20231009_TIF_corplor_R1_R2.pdf",width = 4, height = 4)
ggplot(dcast_TIF) + 
  geom_point(aes(R1,R2, color = fct_relevel(sample,c("WT","POLQ"))))+ 
  geom_smooth(aes(R1,R2), method = "lm")+ 
  xlab("R1 (log2 TIF)") + ylab("R2 (log2 TIF)") + 
  theme_bw() + stat_cor(aes(R1,R2), label.y = -3, label.x = -2) +
  scale_color_manual(values = c("#949DCF","#465584","#DADADA")) +
  theme(legend.position = "top") + coord_fixed( xlim = c(-3.5,0), ylim = c(-3.5,0))
dev.off()

#R1 vs. R3 (Fig. S1C)
pdf("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/supplementary_figures/figure_S1_general_controls/xv20231009_TIF_corplor_R1_R3.pdf",width = 4, height = 4)
ggplot(dcast_TIF) + 
  ...
  theme_bw() + stat_cor(aes(R1,R3), label.y = -3, label.x = -2) +
  scale_color_manual(values = c("#949DCF","#465584","#DADADA")) +
  theme(legend.position = "top") + coord_fixed( xlim = c(-3.5,0), ylim = c(-3.5,0))
dev.off()

#R2 vs. R3 (Fig. S1D)
pdf("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/supplementary_figures/figure_S1_general_controls/xv20231009_TIF_corplor_R2_R3.pdf",width = 4, height = 4)
ggplot(dcast_TIF) + 
  ...
  theme_bw() + stat_cor(aes(R2,R3), label.y = -3, label.x = -2) +
  scale_color_manual(values = c("#949DCF","#465584","#DADADA")) +
  theme(legend.position = "top") + coord_fixed( xlim = c(-3.5,0), ylim = c(-3.5,0))
dev.off()
```
...
wt.set.editing <- filter(screen.data.names.tib , sample == "WT") %>% dplyr::group_by(barcode, rep) %>% dplyr::summarise(wt.TIF = mean(log2TIFratio, na.rm = T))
...
log2.distance.TIF <- screen.data.names.tib %>% 
  ...
  mutate(log2.foldchange = log2TIFratio - wt.TIF) %>% #Calculate differential of log2MMEJ:NHEJ 
...
effect_types <- data_DR_screen %>%
  mutate(pathway_involved = case_when(!is.na(DR_effect_type) & global_diff > 0 ~ "NHEJ",
           ...
           select(gene,DR_effect_type,pathway_involved, bal_global_diff = global_diff) %>% distinct()
...
#  filter(sample == "KO") %>%
  ...
  t_test(mean.log2foldchange ~ 1, mu = 0) %>% 
...
#
ggplot(mean.diff.TIF.export %>% 
         filter(DR_effect_type %in% c("both","CCD") & p.adj < 0.05),
       aes(mean.diff.TIF, bal_global_diff)) + 
  geom_point() + 
  stat_cor() +
  aes(mean.diff.TIF, bal_global_diff) +
  theme_bw() +
  ...
  coord_fixed(ylim = c(-1.25,2), xlim = c(-1.25,2))
...
#PLOT
pdf("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/supplementary_figures/figure_S1_general_controls/xv20231010_volcano_plot_TIF.pdf",width = 5, height = 3.5)
ggplot(test.mean.diff.TIF) + 
  geom_point(aes(mean.fc,-log10(p.adj))) + 
  geom_hline(yintercept = -log10(0.001), linetype = 2) +
  geom_vline(xintercept = 0, linetype = 2) +
  geom_text_repel(data = subset(test.mean.diff.TIF, mean.fc < -0.55), aes(mean.fc,-log10(p.adj), label = gene)) +
  geom_text_repel(data = subset(test.mean.diff.TIF, mean.fc > 0.25), aes(mean.fc,-log10(p.adj), label = gene)) +
  theme_bw() + 
  xlab("∆log2TIF") +
  ylab("-log10FDR") +
...
# Plot TIF differences by effect
pdf("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20220117_paper_figure_short/supplementary_figures/figure_S1_general_controls/xv20231010_hits_delta_TIF.pdf",width = 2.5, height = 4)
ggplot(mean.diff.TIF.export %>% filter(DR_effect_type %in% c("both","CCD")), aes(pathway_involved,mean.diff.TIF)) + 
  geom_hline(yintercept = 0, linetype = 2) +
  geom_quasirandom(aes(color = p.adj < 0.001)) + 
  theme_bw() +
