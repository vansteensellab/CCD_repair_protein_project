...
#Relation between log2TIF & log2TIR
```{r}
ggplot(screen.data.ddr) + geom_point(aes(log2TIF, log2(freqCut)))
...
#TIF CCDs
ggplot(combination_all %>% filter(gene %in% c("BRCA2","CHAF1A","POLL"))) + 
...
cosine_distance_TIF_ratio <- map_dfr(unique(cosine_distance_dt$gene), function(x) {
  ...
  ratio_CCD <- cosine_distance_dt %>% filter(gene == x) %>% mutate(effect_size = abs(CCD_synergy_score)) %>% pull(effect_size)
...
ggplot(clas_cosine_distance %>% filter(CCD_significance == "both_signif")) + 
  geom_col(aes(reorder(gene, cosine_sim), cosine_sim)) +
  coord_cartesian(ylim = c(-1,1)) +
  theme_bw() +
...
#Cosine distance calculation
signed_cosine_distance_TIF_ratio <- map_dfr(unique(cosine_distance_dt$gene), function(x) {
  TIF_CCD <-  cosine_distance_dt %>% filter(gene == x) %>% pull(TIF_CCD_value)
  ratio_CCD <- cosine_distance_dt %>% filter(gene == x) %>% mutate(effect_size = abs(CCD_synergy_score)) %>% pull(CCD_synergy_score)
...
#
signed_clas_cosine_distance <- signed_cosine_distance_TIF_ratio %>% left_join(significant_TIF_ratio)
```
