...
distance_exon <- map_dfr(unique(interaction(x$kgID, x$Exon.number, sep = "_")), function(y){
  gene <- as.character(str_extract(y, ".*(?=_)"))
  exon <- as.integer(str_extract(y, "(?<=_).*"))
  prev_exon_end <- filter(x, kgID == gene & Exon.number == (exon - 1)) %>% pull(Exon.end) %>% unique()
  exon_start <- x %>% filter(kgID == gene & Exon.number == exon) %>% pull(Exon.start) %>% unique()
  exon_end <- x %>% filter(kgID == gene & Exon.number == exon) %>% pull(Exon.end) %>% unique()
  next_exon_start <- filter(x, kgID == gene & Exon.number == (exon + 1)) %>% pull(Exon.start) %>% unique()
...
high_quality_gRNAs <- human_gRNA %>%
  ...
  dplyr::mutate(rel_ex_number = Exon.number/max(Exon.number)) %>%
  ungroup() %>%
  filter(kgID %in% kgID_to_use & Frameshift.... > 80 & rel_ex_number < 0.2) %>%
  distinct()
```
...
```{r}
# Filter high quality gRNAs in targetted regions
