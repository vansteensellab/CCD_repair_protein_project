...
high_confidence_IPR <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230905_DEF_viable_freq_0.008.rds")

#Import control data
control_data_DEF <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230907_DEF_control_data.rds")
...
##Run PCA component analysis with this positions in the genome
```{r}
PCA_analysis_dt <- inhibitor_effect_chromatin_RPE1 %>%
  filter(gRNA == "NTC" & binsize == 2000)
...
for (j in unique(pca_model_dt$cell_line)){
  ...
  for (i in unique(cell_line_df$gRNA)){
    ...
    set.seed(1)
    PCR_model_DDR_test <- pls::pcr(mean_diff~ dam_H3K27me3+chip_H3K36me3+dam_LMNB1+chip_H3K4me1+chip_H3K27ac+dam_H3K9me2+late_replicating, data=gene.library.dt, validation = "CV") #Run principal component regression
...
drug_CCD_all_values <- drug_CCD_slopes_RPE %>% 
  ...
  filter(!feature %in%  c("dam_LMNB2", "chip_H3K4me2", "chip_H3K4me3", "dam_H3K9me3")) %>%
...
#Export data table
```{r}
write_rds(drug_CCD_all_values, file = "~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230907_CCD_analysis_summary_DEF.rds")
