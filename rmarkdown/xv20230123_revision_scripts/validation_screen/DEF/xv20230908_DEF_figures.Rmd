...
#Processed data
DEF_CCD <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230907_CCD_analysis_summary_DEF_val_3reps.rds") %>% left_join(proteins_gRNA) %>% mutate(chrom_feature = str_extract(pattern = "(H3|l|L).*", feature))
...
```{r pressure, echo=FALSE}
#Global effect correlation data
CCD_effect_correlation <- DEF_CCD %>%
  filter(fdr < 0.1) %>%
