...
#Processed data
DEF_CCD <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230910_CCD_analysis_summary_DEF_val_3reps.rds") %>% left_join(proteins_gRNA) %>% mutate(chrom_feature = str_extract(pattern = "(H3|l|L).*", feature))
...
DEF_log2 <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230910_log2_balance_chromatin_DEF.rds") %>% left_join(proteins_gRNA)
...
ggplot(M_N_synergy_summary) +
  geom_col(aes(fct_relevel(pathway,c("MMEJ","NHEJ","both")),value)) +
