...
filtered_indel_data <- pathway_metric_control_bio %>%
  ...
  group_by(barcode, gRNA) %>%
  dplyr::mutate(mean_freq = mean(IPR_read_freq)) %>%
  filter(mean_freq > 0.0075 & mean_z > -1.65)
...
#Export data
```{r}
write_rds(filtered_indel_data, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230905_DEF_viable_mean_freq_0.008.rds")
