...
normalized_quantity <- import_dna_quantification %>%
  ...
  mutate(z_score_dna = scale(DNA_conc))

#Combined score
combined_data <- normalized_quantity %>%
  dplyr::group_by(gRNA, cell_line) %>%
  dplyr::summarise(mean_z = (sum(z_score_dna)/sqrt(n())))

ggplot(normalized_quantity, aes(gRNA, `z_score_dna`)) +
  stat_summary(geom = "bar") +
  geom_quasirandom(aes(color = bio_rep)) +
  theme_bw() + geom_hline(yintercept = 0, linetype = 2)

ggplot(combined_data %>% filter(cell_line != "U2OS")) +
  geom_col(aes(gRNA, mean_z, fill = mean_z < -1.65)) + 
...
#Make plots gRNA
ggplot(plot_example) +
  geom_quasirandom(aes(IPR_read_freq,sqrt(`del_-7`*ins_1))) + 
  facet_grid(cell_line~gRNA) +
  theme_bw() 
...
filter_read_count_test <- map_dfr(read_count , function(x) {
  #Perform barcode freq test
barcode_frequency_test <- pathway_metric_control_bio %>%
  dplyr::group_by(barcode, gRNA) %>%
  dplyr::mutate(reps = n()) %>%
  filter(reps == 3) %>%
  t_test(IPR_read_freq ~ 1, mu =  x, alternative = "greater") %>%
  mutate(fdr = p.adjust(p, method = "BH")) %>%
  ungroup() %>%
  filter(fdr < 0.05) %>% 
  select(gRNA, barcode)
  #Filter based on it
  pathway_metric_control_bio %>% right_join(barcode_frequency_test) %>% mutate(read_freq_filter = x)
})

#Number of IPRs in each
IPR_summary <- filter_read_count_test %>% 
  dplyr::group_by(cell_line, gRNA, read_freq_filter, bio_rep) %>% 
  dplyr::summarize(IPR_n = n()) 

IPR_summary_dcast <- IPR_summary %>% 
  reshape2::dcast(cell_line + gRNA + bio_rep ~ read_freq_filter, value.var  = "IPR_n")
...
replicates_IPR <- filter_read_count_test %>%
  dplyr::group_by(barcode,cell_line, read_freq_filter,gRNA) %>%
  dplyr::summarise(rep_n = n()) %>%
  ungroup() %>%
  dplyr::group_by(cell_line, read_freq_filter,rep_n,gRNA) %>%
  dplyr::summarise(capture_IPR = n())

total_IPR_rep <- filter_read_count_test %>%
  select(cell_line, barcode, read_freq_filter,gRNA) %>% 
  distinct() %>%
  dplyr::group_by(cell_line,read_freq_filter,gRNA) %>%
...
#Absolute_numbers
IPR_reps <- replicates_IPR %>% reshape2::dcast(cell_line + gRNA + rep_n ~ read_freq_filter)
...
ggplot(freq_IPR_capture_total) + 
  geom_col(aes(x = fct_relevel(as.character(read_freq_filter),as.character(read_count)),
               y = freq_capture,
               fill = as.character(rep_n)),
           position = "dodge") +
  labs(fill = "IPR_reps") +
  facet_wrap(~ cell_line) + 
  theme_bw() + labs(x = "read_freq_filter")
...
ggplot(n2_IPRs) + 
  geom_col(aes(x = gRNA,
               y = freq_capture_n2,
               fill = fct_relevel(as.character(read_freq_filter),as.character(read_count))),
           position = "dodge") +
  labs(fill = "read_freq_filter") +
  facet_wrap(~ cell_line) +
  theme_bw() +
  theme(legend.position = "top")
```
...
read_counts_filter_reprod <- filter_read_count_test %>%
  reshape2::dcast(barcode + cell_line + gRNA + read_freq_filter ~ bio_rep, value.var = "log2_bal")
...
ggplot(read_counts_filter_reprod_summary, aes(R1,R2)) +
  ...
  ggpubr::stat_cor() +
  facet_grid(gRNA ~ read_freq_filter, scales = "free") +
  theme_bw()
...
ggplot(read_counts_filter_reprod_summary, aes(R2,R3)) +
  ...
  geom_smooth(method = "lm") +
  ggpubr::stat_cor() +
  facet_grid(cell_line ~ read_freq_filter, scales = "free") +
  theme_bw()
...
# Test for the barcode frequency
```{r}
barcode_frequency_test <- pathway_metric_control_bio %>%
  dplyr::group_by(barcode, gRNA) %>%
  dplyr::mutate(reps = n()) %>%
  filter(reps == 3 & gRNA == "NTC") %>%
  t_test(IPR_read_freq ~ 1, mu =  0.001, alternative = "greater") %>%
  mutate(fdr = p.adjust(p, method = "BH"))
...
filtered_indel_data <- pathway_metric_control_bio %>%
  ...
  filter(IPR_read_freq > 0.0075 & mean_z > -1.65)
...
#Export data
```{r}
write_rds(filtered_indel_data, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230905_DEF_viable_freq_0.008.rds")

#Create data table for controls
#Filter barcodes with reads
controls_filter_data <- pathway_metric_control_bio %>%
  filter(IPR_read_freq > 0.0075 & gRNA %in% c("LBR2","NTC"))

write_rds(controls_filter_data, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230907_DEF_control_data.rds")
