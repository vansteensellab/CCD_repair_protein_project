...
normalized_quantity <- import_dna_quantification %>%
  ...
  mutate(z_score_dna = as.numeric(scale(DNA_conc))) 
...
filtered_indel_data <- pathway_metric_control_bio %>%
  ...
  dplyr::mutate(mean_freq = mean(IPR_read_freq),
                ...
                reps = n()) %>%
  filter(IPR_read_freq > 0.0075 & mean_z > -1.65 & reps == 3 & is.finite(log2_bal))
...
ggplot() +
  ...
  ggtitle("Frequency of IPR vs. log2 MMEJ:NHEJ", subtitle = "RPE1-hTERT p53-BRCA1 dKO DSB-TRIP pool")
```
#Plot after calculating the mean over three replicates
```{r}
#Make plots gRNA
ggplot() +
  geom_point(data=filtered_indel_data %>% filter(gRNA != "empty" & bio_rep == "R1"), 
             aes(mean_freq,mean_bal, color = gRNA == "NTC")) + 
  theme_bw() +
  ylab("log2 MMEJ:NHEJ") + xlab("IPR frequency in pool") +
  ggtitle("Average frequency IPR vs. log2 MMEJ:NHEJ", subtitle = "RPE1-hTERT p53-BRCA1 dKO DSB-TRIP pool")
```
...
#Export data
```{r}
write_rds(filtered_indel_data, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230909_DEF_viable_mean_freq_0.008.rds")
