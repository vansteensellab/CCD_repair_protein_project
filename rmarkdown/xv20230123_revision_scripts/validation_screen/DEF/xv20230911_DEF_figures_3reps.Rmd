...
#Processed data
DEF_CCD <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230911_CCD_analysis_summary_DEF_val_3reps.rds") %>% left_join(proteins_gRNA) %>% mutate(chrom_feature = str_extract(pattern = "(H3|l|L).*", feature))
...
global_effect_correlation <- DEF_CCD %>% 
  select(gene,gRNA,DEF_global = global_diff) %>% 
...
ggplot(global_effect_correlation,aes(DEF_global,K562_global)) + 
...
#Plot this global effect
ggplot(global_effect_correlation,aes(RPE1_global,DEF_global)) + 
  geom_smooth(method = "lm", se = F) +
  geom_point() +
  ggpubr::stat_cor(method = "spearman") +
  theme_bw() + 
...
#Join CCD_scores
K562_RPE_distances <- K562_RPE1_distance_all %>% dplyr::left_join(DEF_CCD %>% select(gene, gRNA,fdr)) 

ggplot(K562_RPE_distances %>% distinct() %>% filter(gene != "ATR")) +
  ...
  ggtitle("CCD pattern similarity", 
           subtitle = "Comparison between RPE1-hTERT p53/BRCA1 dKO & K562") +
...
combined <- bind_rows(RPE1_modified, K562_modified, DEF_modified)

#Plot ATM-KO vs. LMNB1
#ATMko and LMNB1
ggplot(combined %>% filter(gene == "ATM" & cell_line != "RPE1"),aes(LMNB1, mean_diff, color = cell_line)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = F) +
  geom_hline(yintercept = 0, linetype = 2) +
  theme_bw() +
...
#FANCD2ko and LMNB1
ggplot(combined %>% filter(gene == "FANCD2" & cell_line != "RPE1"),aes(LMNB1, mean_diff, color = cell_line)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = F) +
  geom_hline(yintercept = 0, linetype = 2) +
  theme_bw() +
...
ggplot(combined %>% filter(gene == "POLL" & cell_line != "RPE1"),aes(H3K36me3, mean_diff, color = cell_line)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = F) +
  geom_hline(yintercept = 0, linetype = 2) +
  theme_bw() +
...
combined_order <- combined %>% filter(cell_line == "DEF") %>% na.omit() %>% dplyr::group_by(gene,gRNA) %>% dplyr::summarise(mean = mean(mean_diff)) %>% arrange(mean) %>% pull(gene) 

ggplot(combined %>% na.omit() %>% filter(cell_line != "RPE1")) +
  stat_summary(aes(fct_relevel(gene,combined_order),mean_diff,fill = fct_relevel(cell_line, c("DEF","K562"))),  position = position_dodge2(preserve = "single"), geom = "bar", fun.data = "mean_sd") + 
  stat_summary(aes(fct_relevel(gene,combined_order),mean_diff, group = fct_relevel(cell_line, c("DEF","K562"))), position = position_dodge2(preserve = "single"), geom = "errorbar", fun.data = "mean_sd",width = 0.5) + 
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90,
                                   hjust = 1,
                                   vjust = 0.5),
