...
filtered_indel_data <- pathway_metric_control_bio %>%
  ...
  filter(IPR_read_freq > 0.0075) %>% #In this file, I will remove the viability note
  group_by(barcode, gRNA) %>%
  dplyr::mutate(reps = n()) %>%
  ungroup() %>%
  filter(reps == 3)
...
#Export data
```{r}
write_rds(filtered_indel_data, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230912_DEF_all_mean_freq_0.0075_.rds")
