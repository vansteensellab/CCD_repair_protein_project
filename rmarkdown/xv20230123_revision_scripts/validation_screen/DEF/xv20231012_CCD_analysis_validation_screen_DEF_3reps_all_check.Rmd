...
high_confidence_IPR <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/inhibitors/xv20230919_inhibitors_freq_0.005_.rds") %>%
  filter(is.finite(log2_bal)) %>% mutate(cell_line = case_when(cell_line == "RPEDef" ~ "DEF",
                                                               cell_line == "RPEPro" ~ "PRO",
                                                               T ~ cell_line))
...
chromatin_data_RPE1 <- readRDS_proof("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen","chromatin_data_RPE1") %>%
  filter(cell_line == "DEF" & binsize == 2000) #Chromatin
...
#Plot correlation coefficients
#pdf("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20230307_revision_figures/RPE_PRO_DEF/xv20230918_DEF_chromatin_correlation.pdf", width = 3.5,height = 4)
...
inhibitor_effect_chromatin_RPE1 <- mean_gRNA_effect %>%
  left_join(chromatin_data_RPE1 %>% distinct()) %>%
  filter(reps > 2) %>% na.omit()
```
...
ggplot(summary_IPR_KO) + 
  ...
  xlab("DNA repair protein KO") + facet_wrap(~ cell_line)
...
```{r}
#PCA model
pca_model_dt <- inhibitor_effect_chromatin_RPE1 %>% filter(binsize == 2000)
...
for (j in c("PRO","DEF")){
  ...
  for (i in unique(cell_line_df$gRNA)){
    ...
    set.seed(1)
    PCR_model_DDR_test <- pls::pcr(mean_diff~ dam_H3K27me3+chip_H3K36me3+dam_LMNB1+chip_H3K4me1+chip_H3K4me2+chip_H3K4me3+dam_H3K9me2+chip_H3K27ac+dam_H3K9me2+late_replicating, data=gene.library.dt) #Run principal component regression
