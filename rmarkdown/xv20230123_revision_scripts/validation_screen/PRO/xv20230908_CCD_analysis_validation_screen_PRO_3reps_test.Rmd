...
diff_gRNA_RPE1 <- high_confidence_IPR %>% 
  ...
  dplyr::summarise(mean_diff_gRNA = mean(log2_bal),
                   mean_diff_ntc = mean(dmso_bal),
                   reps = n()) %>%
  mutate(mean_diff = mean_diff_gRNA - mean_diff_ntc) %>%
...
mean_gRNA_effect <- diff_gRNA_RPE1 %>%
  dplyr::group_by(barcode, gRNA) %>%
  dplyr::summarise(mean_diff_gRNA = mean(gRNA_diff),
                   reps = n())
```
...
inhibitor_effect_chromatin_RPE1 <- diff_gRNA_RPE1 %>%
...
for (j in unique(pca_model_dt$cell_line)){
  ...
  for (i in unique(cell_line_df$gRNA)){
    ...
    set.seed(1)
    PCR_model_DDR_test <- pls::pcr(mean_diff~ chip_H3K4me1+chip_H3K36me3+dam_LMNB1+chip_H3K4me2+chip_H3K27ac+dam_H3K9me2+dam_H3K9me3+late_replicating+dam_H3K27me3, data=gene.library.dt) #Run principal component regression
