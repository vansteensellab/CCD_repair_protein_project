...
read_count <- c(0,0.001,0.0015,0.002,0.005,0.01)
...
filtered_indel_data <- pathway_metric_control_bio %>%
  ...
  filter(IPR_read_freq > 0.002 & dna_score > 0.5) %>%
  group_by(barcode, gRNA) %>%
  dplyr::mutate(reps = n()) %>%
  ungroup() %>%
  filter(reps == 3)
...
filtered_data_plot <- pathway_metric_control_bio %>%
  ...
  dplyr::mutate(mean_freq = mean(IPR_read_freq),
                ...
                reps = n()) %>%
  filter(IPR_read_freq > 0.002 & reps == 3 & is.finite(log2_bal) & gRNA %in% c("LBR2","NTC","36","64")) %>%
...
#Export data
```{r}
write_rds(filtered_indel_data, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230911_PRO_viable_mean_freq_0.002.rds")
...
write_rds(controls_filter_data, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230911_PRO_control_data.rds")
