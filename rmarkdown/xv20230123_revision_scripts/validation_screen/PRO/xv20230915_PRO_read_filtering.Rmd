...
filtered_indel_data <- pathway_metric_control_bio %>%
  ...
  filter(IPR_read_freq > 0.0025 & is.finite(log2_bal)) %>%
  group_by(barcode, gRNA) %>%
  dplyr::mutate(reps = n()) %>%
  ungroup() %>%
  filter(reps == 3)
...
#Make plots gRNA
ggplot() +
  geom_point(data=pathway_metric_control_bio_freq %>% filter(!gRNA %in% c("empty","LBR2","halfLBR2")), 
...
#Export data
```{r}
write_rds(filtered_indel_data, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230915_PRO_all_mean_freq_0.0025.rds")
