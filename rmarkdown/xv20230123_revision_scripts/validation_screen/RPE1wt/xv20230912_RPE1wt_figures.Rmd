...
#Processed data
RPE1wt_CCD <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230912_CCD_analysis_summary_RPE1_5reads_n3.rds") %>% left_join(proteins_gRNA)  %>% mutate(chrom_feature = str_extract(pattern = "(H3|l|L).*", feature))
...
#All log2_values
RPE1wt_log2 <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230912_log2_balance_chromatin_RPE1_5reads.rds") %>% left_join(proteins_gRNA)
...
global_effect_correlation <- RPE1wt_CCD %>% 
  select(gene,gRNA,RPE_global = global_diff) %>% 
...
M_N_synergy_summary <- RPE1wt_CCD %>% 
  na.omit() %>%
...
#Join CCD_scores
K562_RPE_distances <- K562_RPE1_distance_all %>% dplyr::left_join(RPE1wt_CCD %>% select(gene, gRNA,fdr)) 
