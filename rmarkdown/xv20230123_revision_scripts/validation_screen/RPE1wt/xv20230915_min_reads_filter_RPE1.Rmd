...
#Bind both
all_IPRs <- bind_rows(RPE_IPR) %>% mutate(pool = paste(iPCR_cell, iPCR_transfection,iPCR_complexity, sep = "_")) %>% filter(pool %in% selected_pools)
```
...
ggplot(plot_example ) +
  ...
  theme_bw() +
  ggtitle("Frequency of IPR", subtitle = "RPE1-hTERT p53 KO DSB-TRIP pool") +
...
read_count <- c(0,0.0005,0.001,0.002,0.004,0.01)
...
filtered_indel_data <- pathway_metric_control_bio %>%
  ...
  filter(`del_-7` > 5 & ins_1 > 5 & is.finite(log2_bal)) %>%
...
filtered_data_plot <- pathway_metric_control_bio %>%
  ...
  dplyr::mutate(mean_freq = mean(IPR_read_freq),
                ...
                reps = n()) %>%
  filter(`del_-7` > 5 & ins_1 > 5 & reps == 3 & is.finite(log2_bal) & gRNA %in% c("LBR2","NTC","36","64")) %>%
...
ggplot(filtered_data_plot) +
  ...
  ggtitle("log2 MMEJ:NHEJ balance", subtitle = "RPE1-hTERT DSB-TRIP pool")
...
#Export data
```{r}
write_rds(filtered_indel_data, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230912_RPE1_filtered_balance_data.rds")

#Create data table for controls
#Filter barcodes with reads
controls_filter_data <- pathway_metric_control_bio %>%
  filter(gRNA %in% c("halfLBR2","NTC","LBR2"))

write_rds(controls_filter_data, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230912_RPE1_control_data.rds")
