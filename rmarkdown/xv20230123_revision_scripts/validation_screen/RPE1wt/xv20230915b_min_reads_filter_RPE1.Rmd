...
indel_data <- clone_pools_pathway_assay %>% 
  ...
  filter(cell_line == "RPE1") %>% left_join(viability_data)
```
...
ggplot(filtered_data_plot) +
  ...
  ggtitle("log2 MMEJ:NHEJ balance", subtitle = "RPE1-hTERT p53 KO DSB-TRIP pool")
...
ggplot() +
  geom_point(data=pathway_metric_control_bio %>% filter(!gRNA %in% c("empty","LBR2","halfLBR2")), 
             aes(IPR_read_freq,log2(`del_-7`/ins_1))) + 
  geom_point(data=filtered_indel_data %>% filter(!gRNA %in% c("empty","LBR2","halfLBR2")), 
...
#Export data
```{r}
write_rds(filtered_indel_data, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230915_RPE1_min_read_5.rds")

#Create data table for controls
#Filter barcodes with reads
controls_filter_data <- pathway_metric_control_bio %>%
  filter(`del_-7` > 5 & ins_1 > 5 & gRNA %in% c("halfLBR2","NTC","LBR2"))

write_rds(controls_filter_data, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230915_RPE1_control_data.rds")
