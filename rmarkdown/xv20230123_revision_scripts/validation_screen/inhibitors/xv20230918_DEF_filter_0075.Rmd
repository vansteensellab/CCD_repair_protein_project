...
indel_data <- clone_pools_pathway_assay %>% 
  ...
  dplyr::summarise(counts = sum(count, na.rm = F)) %>% left_join(all_IPRs %>% select(barcode, pool)) %>% filter(cell_line != "U2OS")
```
...
filtered_indel_data <- pathway_metric_control_bio %>%
  ...
  filter(IPR_read_freq > 0.005 & gRNA != "tracr") %>% #In this file, I will remove the viability note
...
filtered_data_plot <- pathway_metric_control_bio %>%
  ...
  dplyr::mutate(mean_freq = mean(IPR_read_freq),
                ...
                reps = n()) %>%
  filter(IPR_read_freq > 0.005 & reps > 2 & is.finite(log2_bal))
...
ggplot(combined_diff_data %>% filter(reps > 2 &!gRNA %in% c("NTC","LBR2","halfLBR2","empty"))) +
  stat_summary(aes(gRNA,mean_diff_balance, fill = cell_line),  position = position_dodge2(preserve = "single"), geom = "bar", fun.data = "mean_sd") + 
  stat_summary(aes(gRNA,mean_diff_balance, fill = cell_line), position = position_dodge2(preserve = "single"), geom = "errorbar", fun.data = "mean_sd",width = 0.5) + 
  theme_bw() +
...
wilcox_test <- combined_diff_data %>% 
  filter(!gRNA %in% c("NTC","LBR2","halfLBR2","empty") & reps > 2) %>%
...
#Export data
```{r}
write_rds(filtered_indel_data, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/inhibitors/xv20230919_inhibitors_freq_0.005_.rds")
