...
#dmso_reps
ntc_barcodes <- dmso_balance %>% dplyr::group_by(barcode) %>%
  summarise(mean_bal = mean(dmso_bal),
            reps = n()) %>%
  filter(reps == 3) %>%
  pull(barcode)
```
...
```{R}
RPE1_high_confidence_reps <- indel_data %>% 
  filter(cell_line == "RPE1" & 
           barcode %in% ntc_barcodes & 
           outcome %in% c("del_-7", "ins_1")) %>%
  reshape2::dcast(barcode + gRNA+ bio_rep ~ outcome, value.var = "counts", fill = 0) %>%
  mutate(log2_bal = log2((`del_-7` + 1) /(ins_1 + 1)))

#Extract ntc
control_value <-  RPE1_high_confidence_reps %>%
  filter(gRNA == "NTC") %>%
  select(barcode,bio_rep,ntc_bal = log2_bal)

#Calculate differential
diff_gRNA_RPE1 <- RPE1_high_confidence_reps %>%
  left_join(control_value) %>%
  mutate(gRNA_diff = log2_bal - ntc_bal) %>%
...
#Create U2OS dataframe (not updated yet)
...
#Quick plot
ggplot(inhibitor_effect_chromatin_RPE1, aes(dam_LMNB1,mean_diff)) + 
     geom_point() + 
     geom_smooth(method = "lm", se = F) + 
     ggpubr::stat_cor(method = "pearson") +
     facet_grid(binsize ~ gRNA) + theme_bw()
```
...
for (j in unique(inhibitor_effect_chromatin_RPE1$cell_line)){
  for (i in unique(inhibitor_effect_chromatin_RPE1$gRNA)){
    ...
    set.seed(1)
    PCR_model_DDR_test <- pls::pcr(mean_diff~ dam_H3K9me3+dam_H3K27me3+ chip_H3K36me3+chip_H3K4me3+chip_H3K4me2+dam_H3K9me2+dam_LMNB1+late_replicating + dam_LMNB2, data=gene.library.dt , validation="CV") #Run principal component regression
...
for (h in unique(inhibitor_effect_chromatin_RPE1$gRNA)) {
  ...
  for (j in colnames(inhibitor_effect_chromatin_RPE1)[4:14]) { #Run this function for each of the 25 high quality chromatin features
...
ggplot(drug_CCD_all_values) + 
  geom_tile(aes(gRNA,feature, fill = slope)) +
