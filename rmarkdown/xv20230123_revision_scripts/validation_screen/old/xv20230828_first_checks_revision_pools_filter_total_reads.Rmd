...
control_balance_bio <- control_samples %>% filter( outcome %in% c("ins_1","del_-7","wt_0"))
...
#Plot balance
ggplot(filtered_measures) + 
  geom_quasirandom(aes(gRNA,mean_TIF)) +
  facet_grid(cell_line ~ read_filter)

#Correlate balance and TIF
ggplot(filtered_measures,aes(mean_TIF, mean_bal)) +
  geom_point() +
  ggpubr::stat_cor() +
  geom_smooth(method = "lm") +
  facet_grid(cell_line ~ read_filter)
