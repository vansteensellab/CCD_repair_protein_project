...
#Aim: I will test how every sample looks and see if the data looks reproducible also for other conditions
...
ggplot(indel_data %>% filter(outcome %in% c("del_-7","ins_1","wt_0"))) + 
  geom_quasirandom(aes(bio_rep, log10(counts), color = outcome)) + 
  facet_grid(~ cell_line)+
  theme_bw()

#Plot read numbers (only signature indels)
ggplot(indel_data %>% filter(outcome %in% c("del_-7","ins_1"))) + 
  geom_quasirandom(aes(bio_rep, log10(counts), color = outcome)) + 
  facet_grid(~ cell_line) +
  theme_bw()
...
control_balance_bio_dcast <- control_balance_bio %>% reshape2::dcast(barcode + cell_line + gRNA + bio_rep ~ outcome, value.var = "counts", fill = 1, fun.aggregate = mean)
...
read_count <- c(0,1,5,10,20,50,100) 
...
read_counts_filter_reprod_TIF<- filter_read_count_test %>%
  reshape2::dcast(barcode + cell_line + gRNA + read_filter ~ bio_rep, value.var = "TIF")
...
```{r}
#Plot balance
ggplot(filtered_measures %>% filter(cell_line == "DEF"),aes(gRNA,mean_bal)) + 
  geom_quasirandom() +
  facet_wrap(~ read_filter, ncol = 1) +
...
#Conclusion: I am not sure what is the best way for filtering the data out! When it comes for reads in both, I think 5 reads are as far as we can go.
