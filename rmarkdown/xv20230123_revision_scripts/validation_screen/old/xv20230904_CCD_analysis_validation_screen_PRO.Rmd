...
high_confidence_IPR <- readRDS("~/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230905_PRO_DEF_pathway_balance_10reads_per_indel.rds") %>% 
  filter(!gRNA %in% c("empty"))
...
#Combine all different binsizes and export chromatin table
```{r}
...
#Export data frame
write_rds(chromatin_pools_binsize, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230806_DSB_TRIP_pool_chromatin_data.rds")
```
...
inhibitor_effect_chromatin_RPE1 <- mean_gRNA_effect %>%
  left_join(chromatin_pools_binsize %>% filter(cell_line %in% c("PRO","DEF")) %>% distinct())
```
...
#Quick plot
ggplot(inhibitor_effect_chromatin_RPE1 %>% filter(gRNA %in% prots_with_8_IPR & binsize == "20000"), aes(dam_LMNB1,mean_diff)) + 
     geom_point() + 
     geom_smooth(method = "lm", se = F) + 
     ggpubr::stat_cor(method = "pearson") +
     facet_grid(cell_line ~ gRNA) + theme_bw()

#Quick plot
ggplot(inhibitor_effect_chromatin_RPE1 %>% filter(gRNA %in% prots_with_8_IPR & binsize == "20000"), aes(chip_H3K36me3,mean_diff)) + 
     geom_point() + 
     geom_smooth(method = "lm", se = F) + 
     ggpubr::stat_cor(method = "pearson") +
     facet_grid(cell_line ~ gRNA) + theme_bw()

ggplot(inhibitor_effect_chromatin_RPE1 %>% filter(gRNA %in% prots_with_8_IPR & binsize == "20000"), aes(dam_H3K27me3,mean_diff)) + 
     geom_point() + 
     geom_smooth(method = "lm", se = F) + 
     ggpubr::stat_cor(method = "pearson") +
     facet_grid(cell_line ~ gRNA)+ theme_bw()

ggplot(inhibitor_effect_chromatin_RPE1 %>% filter(gRNA %in% prots_with_8_IPR & binsize == "20000"), aes(chip_H3K4me1,mean_diff)) + 
     geom_point() + 
     geom_smooth(method = "lm", se = F) + 
     ggpubr::stat_cor(method = "pearson") +
     facet_grid(cell_line ~ gRNA)+ theme_bw()
