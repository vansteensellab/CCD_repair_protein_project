...
toxic_conditions <- tibble(cell_line = c(rep("PRO",3)), gRNA = c("38","68","74"))

#Filter by replicate
indel_data <- clone_pools_pathway_assay %>% 
  filter(cell_line == "PRO") %>%
...
#Plot barcode frequency per replicate
```{R}
#Dcast
tmp <- plot_example %>% reshape2::dcast(barcode +cell_line+gRNA ~ bio_rep, value.var = "IPR_read_freq")
...
filtered_indel_data <- pathway_metric_control_bio %>%
  filter(IPR_read_freq > 0.001 & `del_-7` > 5 & bio_rep != "R1")
...
ggplot(combined_diff_data %>% filter(!gRNA %in% c("NTC","LBR2","halfLBR2","empty") & cell_line == "PRO")) +
...
#Export data
```{r}
write_rds(filtered_indel_data, file = "/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/data/revisions/validation_screen/xv20230905_PRO_pathway_bal_only_R3.rds")
