...
### calculation using MMEJfreq instead of balance
```{r}
...
screen.data.ddr <- filtered_screen_data %>% 
  ...
  mutate(freqCut = 1 - pct_wt,
         MMEJfreq = (del_7 / proccessed_read),
         NHEJfreq = (ins_1 / proccessed_read),
...
wt.set.ratio <- filter(screen.data.names.tib , gRNA == "NTC") %>% 
  dplyr::group_by(barcode, replicate) %>% 
  dplyr::summarise(wt.MMEJ = mean(MMEJfreq, na.rm = T),
                   wt.NHEJ = mean(NHEJfreq, na.rm = T),
                   wt.cut = mean(freqCut, na.rm = T),
                   wt.log2ratio = mean(log2MMEJNHEJratio))
...
log2.distance.ratio <- screen.data.names.tib %>% 
  ...
  mutate(log2.foldchange = log2MMEJNHEJratio - wt.log2ratio,
         log2MMEJfreq = log2(MMEJfreq/wt.MMEJ),
         log2NHEJfreq = log2(NHEJfreq/wt.NHEJ),
         log2cut = log2(freqCut/wt.cut)) %>% #Calculate differential of log2MMEJ:NHEJ 
  na.omit() %>%
  dplyr::group_by(gene,barcode,gRNA, cell_line) %>% #Group by sample
  dplyr::summarise(mean.log2foldchange = mean(log2.foldchange),
                   mean.MMEJchanges = mean(log2MMEJfreq),
                   mean.NHEJchanges = mean(log2NHEJfreq),
                   mean.cut = mean(log2cut),
                   n_rep = n()) %>% #Compute differential of log2MMEJ:NHEJ
  ungroup() %>% filter(n_rep == 3)
```

#Plot changes in MMEJfreq, NHEJfreq and freqCut
```{r}
ggplot(log2.distance.ratio %>% 
         filter(gRNA != "empty"),
       aes(gRNA, mean.cut)) + 
  ggbeeswarm::geom_quasirandom() + 
  stat_summary(geom = "point", color = "red") + theme_bw() + 
  geom_hline(yintercept = 0, linetype = 2)  + ylim(-1.5,1.2)


ggplot(log2.distance.ratio %>% 
         filter(gRNA != "empty"),
       aes(gRNA, mean.MMEJchanges)) + 
  ggbeeswarm::geom_quasirandom() + 
  stat_summary(geom = "point", color = "red") + theme_bw() + 
  geom_hline(yintercept = 0, linetype = 2)+ ylim(-1.5,1.2)
...
DR_KO_CCD_all_values <- RPE1_KO_CCD_slopes %>% 
  ...
  left_join(CCDs_dt_RPE1_adjs %>% 
              dplyr::select(cell_line,gene, model_p.value = p.value, model_p.adj = fdr), by = c("gene")) %>% #Join with CCDs p_value data table and change some names
  left_join(pathway_summary, by = c("gene","cell_line")) %>% #Join with individual IPR significance calls (z-test)
...
export_screen_result_TS4 <- log2.distance.ratio %>% 
  mutate(cell_line = "RPE1_p53_BRCA1_dKO") %>%
...
IPR_locations <- read_delim("/home/x.vergara/XV_P3_ChromDSBScreen/xv_CRISPR_screen_figures/export/xv20230307_revision_figures/DSB_TRIP_mapping/xv20230519_RPE_pools_mapping.tsv") %>%
  filter(grepl("RPE1Deff_Low_1000", name)) %>% separate(name, into =c("barcode", NA, NA, NA))
...
#write.xlsx(export_screen_result_TS4, file = "data/supplementary_tables/xv20231123_Table_S4b_p53_BRCA1_dKO_delta_log2_MMEJ_NHEJ.xlsx")

#Table S6: Genomic coordinates, chromatin feature scores and barcodes of 19 IPRs in K562 clone 5
#write.xlsx(export_screen_result_TS6, file = "data/supplementary_tables/xv20231117_Table_S6c_p53_BRCA1_dKO_chromatin_features.xlsx", keepNA = T)

#Table S7: Chromatin context dependent effects of proteins in the screen.
#write.xlsx(export_screen_result_TS7, file = "data/supplementary_tables/xv20231123_Table_S7c_p53_BRCA1dKO_global_CCD_MMEJ_NHEJ_results.xlsx", keepNA = T)
